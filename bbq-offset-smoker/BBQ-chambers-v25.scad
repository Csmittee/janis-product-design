...
// Version: v25
// Date: 2026-07-25
// Source: BBQ-chambers-v24.scad
// Changes: direct-cc real redesign, Janis's own instruction after
// reviewing the hinge bracket against a real off-shelf part: "we might
// need to redo the design, including the door parting line on face CD,
// this must calculate and go as one unify system." Real structural
// change (first time this round touches the ridge's own ownfixed/lid
// split, previously untouched/frozen since v2):
//
// NEW `RIDGE_SPLIT_Y` constant — the ridge (C-D face) has ALWAYS had a
// real fixed/lid split, same as the side wall does (`NEW_SPLIT_Z`), but
// it was hardcoded to the exact ridge midpoint (`chamber_W/2`, = the
// SAME value as `DATUM_Y_CENTER`, coincidentally, not by real design
// intent) in `fixed_side_wedge()`/`lid_side_wedge()`/
// `lid_closed_panels()`/`lid()`'s own rotation point. Janis's own
// explicit instruction: move this split closer to apex C, to let the new
// pivot bracket (on the fixed side) sit closer to C too, per the real
// off-shelf hinge part now being used. `RIDGE_SPLIT_Y = chamfer + 30` =
// 208.665mm (was chamber_W/2 = 305mm) — a real, cc-chosen judgment call
// (Janis's own instruction gives the DIRECTION and rough intent, not an
// exact number for this specific constant; the 30mm margin beyond apex C
// leaves a small, real lid-side ridge remnant for a genuine weld seam,
// matching this project's own small-overlap convention like
// `LID_OVERLAP`). Flagged, open to adjustment.
//
// R-009 duplication check: `chamber_W/2`/`DATUM_Y_CENTER` used as the
// ridge-split reference in exactly 4 places (`fixed_side_wedge()`,
// `lid_side_wedge()`, `lid_closed_panels()`'s own ridge-cap panel width,
// `lid()`'s own rotation point) — all 4 updated to `RIDGE_SPLIT_Y` in
// this pass, not just the one that would have been visually obvious.
// `DATUM_Y_CENTER` itself is UNCHANGED and NOT touched elsewhere — it
// remains the chamber's real lateral centerline (610/2=305mm, used by
// `exhaust_room()`/`chimney_pipe()`/the front bracket/etc. for genuinely
// unrelated reasons) — only the ridge-split USE of that same numeric
// value is being separated out into its own real, independently-tunable
// constant. No other content in this file touched.
//
// v24 kept unchanged, on record. v24's own original header follows,
// UNCHANGED, kept as real history:
//
// BBQ Offset Smoker — Chambers
// Version: v24
// Date: 2026-07-24
// Source: BBQ-chambers-v23.scad
// Changes: prompts/bbq-lid-hinge-v7-sync-pivot-margin-cc-prompt.md, TASK
// 1. ONE real change: `lid_open_deg`'s top-level default declaration
// moved into a `/* [Hidden] */` Customizer group (OpenSCAD's own
// recognized syntax). Root cause fixed (Section 2, Problem 1 of the
// source prompt): `lid_open_deg` is a bare top-level assignment in this
// file AND `BBQ-offset-smoker-base-v7.scad` reassigns it after the
// include chain (the "last top-level assignment wins" mechanism) --
// OpenSCAD's Customizer auto-detects EVERY top-level assignment across
// the full include tree and generates its own exposed control for each,
// injecting a `-D` command-line override at render time for whatever it
// tracks. A `-D` override wins over any in-file reassignment regardless
// of textual position -- so Customizer's own default for THIS file's
// `lid_open_deg` could silently defeat the base file's real driver
// (`door_open_deg`). FIX: `/* [Hidden] */` excludes it from Customizer's
// UI/override list entirely, leaving `door_open_deg` (base file) as the
// only real control reaching this file's own `lid()` call. The default
// declaration itself is NOT deleted -- this file must still render
// standalone on its own (existing convention, confirmed via this file's
// own DEBUG TOGGLES/ASSEMBLY section, unchanged) -- only its Customizer
// VISIBILITY changed. `firebox_door_open_deg` moved to its own new
// `/* [Firebox] */` Customizer group immediately after, purely to CLOSE
// the `[Hidden]` grouping (Customizer groups apply to every following
// variable until the next group marker) -- it remains a real, visible,
// independently-driven control, UNCHANGED in every other respect.
// R-009 duplication check: grep-confirmed `lid_open_deg` has exactly
// TWO real top-level assignments in the full include tree (this file's
// own default, and the base file's own reassignment) -- no other copies
// found in BBQ-understructure-v17.scad or elsewhere. No other content in
// this file touched -- every other module/constant/value below is
// BYTE-IDENTICAL to v23 (confirmed via diff before committing). v23 kept
// unchanged, on record.
//
// v23's own original header follows, UNCHANGED, kept as real history:
//
// BBQ Offset Smoker — Chambers
// Version: v23
// Date: 2026-07-24
// Source: BBQ-chambers-v22.scad
// Changes: prompts/bbq-lid-hinge-three-rib-v2-cc-prompt.md, Section 7.5.
// ONE real change, R-009 duplication check stated explicitly per this
// project's own CC INTRO convention: `lid_hardware()` (module),
// `LEVER_ARM`/`COUNTERWEIGHT_KG` (its two exclusive constants), the
// `show_lid_hardware` toggle, and its ASSEMBLY call line DELETED
// entirely. R-009 check: grep-confirmed zero real consumers of
// LEVER_ARM/COUNTERWEIGHT_KG anywhere else in this file or in
// BBQ-understructure-v15.scad/BBQ-offset-smoker-base-v5.scad — this was
// the pre-v2 end-hinged-lid concept's own stub, `show_lid_hardware=false`
// since the v2 ridge-hinge redesign (2026-07-14), confirmed dead not a
// live duplicate. The REAL counterbalance mechanism (CB1, three-rib
// design) is now built in BBQ-offset-smoker-base-v6.scad, in the
// Accessories branch, not here — this stale stub was a genuine R-009
// duplication risk (two unrelated "counterbalance" concepts in the
// codebase, one dead) now that the real one exists. No other content in
// this file touched — every other module, constant, and value below is
// BYTE-IDENTICAL to BBQ-chambers-v22.scad (confirmed via diff before
// committing). v22 kept unchanged, on record.
//
// v22's own original header follows, UNCHANGED, kept as real history:
//
// BBQ Offset Smoker — Chambers
// Version: v22
// Date: 2026-07-22
...
module lid(lid_open_deg = 0) {
    translate([0, RIDGE_SPLIT_Y + LID_HINGE_GAP, DATUM_Z_RIDGE + LID_HINGE_GAP])   // v25: rotation point now RIDGE_SPLIT_Y, was DATUM_Y_CENTER
        rotate([-lid_open_deg, 0, 0])
        translate([0, -RIDGE_SPLIT_Y - LID_HINGE_GAP, -DATUM_Z_RIDGE - LID_HINGE_GAP])
        lid_closed();
}

...
// Version: v14
// Date: 2026-07-30
// Source: BBQ-offset-smoker-base-v13.scad
//
...
// stale narrative.
//
// v14 real changes from v13 (all per Janis's own direct catches from a
// render): (1) tray skirt moved off the hinge-side edge (where it
// visibly sank into the hinge block, a real defect no manifold check
// catches since it's just overlapping solid material) onto the tip and
// both left/right side edges instead — the hinge side now carries zero
// skirt, on purpose. (2) the folding link (`tray_link()`) is now real,
// visible geometry (2 rod segments meeting at an elbow) at its deployed
// reference position, not just a computed length — see
// docs/tray-relocation-bracket.md for the real, disclosed kinematic
// finding on why a simple fixed-length 2-bar link can't follow the tray
// through the FULL stow sweep from this anchor. (3) `HINGE_OUT` widened
// +15mm for real physical clearance around the link's own hardware.
//
...
module tray(x0, angle_deg) {
    ...
    translate([x0, -HINGE_PIVOT_OFFSET, HINGE_Z]) rotate([angle_deg, 0, 0]) {
        ...
        // a coincident face.
        translate([0, -TRAY_D-e-TRAY_T, -TRAY_SKIRT_H])            // tip (outer) edge
            cube([TRAY_L, 2*TRAY_T, TRAY_SKIRT_H + TRAY_T]);
        translate([-e, -TRAY_D-e-TRAY_T, -TRAY_SKIRT_H])           // left side edge
            cube([2*TRAY_T, TRAY_D+2*TRAY_T, TRAY_SKIRT_H + TRAY_T]);
        translate([TRAY_L-TRAY_T, -TRAY_D-e-TRAY_T, -TRAY_SKIRT_H]) // right side edge
            cube([2*TRAY_T, TRAY_D+2*TRAY_T, TRAY_SKIRT_H + TRAY_T]);
    }
}
...
// tray's own real tip (deployed Y-span reaches -305.01mm), confirmed
// inside not past it, exactly as Janis called for.
...
TRAY_LINK_LEN = norm(TRAY_LINK_TT - TRAY_LINK_TS);                  // real length to source a folding link part with, see docs/tray-relocation-bracket.md for the current live number

// REAL, DISCLOSED FINDING, 2026-07-30 (checked in Python before
// modeling, not assumed): a genuine 2-bar pin-jointed link of FIXED
// total length (divide 50/50, per Janis's own spec) can only close the
// gap between the fixed `ts` and the tray's own `tt` at angles where
// their real distance is <= TRAY_LINK_LEN. Checked across the full
// stow sweep (tray0_angle_deg -90..0) via real circle-circle
// intersection: the ts-to-tt distance is EXACTLY TRAY_LINK_LEN at
// deployed (0°, tangent circles, the link fully straight) but GROWS
// well past it as the tray folds toward stowed (-90°) -- `ts` sits low
// (near apex H, well below the tray's own hinge Z), so `tt` swings
// AWAY from it, not toward it, as the tray folds up. A fixed-length
// rigid link genuinely cannot follow the tray through the full stow
// range from this anchor -- this is a real geometric fact, not a
// modeling shortcut, and matches why real hardware for this exact job
// (a folding shelf/table stay) commonly uses a SLOTTED, not a fixed,
// pivot at one end. Modeled here at the DEPLOYED (0°) reference
// position only, where it IS fully determined (elbow = the tangent
// point, exactly midway between ts and tt since both segments are the
// same length) -- real, visible geometry, not a placeholder.
TRAY_LINK_ELBOW = (TRAY_LINK_TS + TRAY_LINK_TT) / 2;
TRAY_LINK_ROD_OD = 10;   // mm, real judgment call -- no exact size given
module tray_link(x_center) {
    translate([x_center,0,0]) {
        hull() {
            translate([0,TRAY_LINK_TS[0],TRAY_LINK_TS[1]]) sphere(r=TRAY_LINK_ROD_OD/2, $fn=16);
            translate([0,TRAY_LINK_ELBOW[0],TRAY_LINK_ELBOW[1]]) sphere(r=TRAY_LINK_ROD_OD/2, $fn=16);
        }
        hull() {
            translate([0,TRAY_LINK_ELBOW[0],TRAY_LINK_ELBOW[1]]) sphere(r=TRAY_LINK_ROD_OD/2, $fn=16);
            translate([0,TRAY_LINK_TT[0],TRAY_LINK_TT[1]]) sphere(r=TRAY_LINK_ROD_OD/2, $fn=16);
        }
    }
}

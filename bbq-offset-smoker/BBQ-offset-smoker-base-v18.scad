...
// Version: v18
// Date: 2026-07-31
// Source: BBQ-offset-smoker-base-v17.scad
//
...
// stale narrative.
//
// v17 (prior round) fixed the tray's fold DIRECTION: LOCKED convention,
// angle_deg 0=deployed (horizontal) .. +90=stowed (tip toward ground),
// clockwise about X viewed from +X when deploying — v16 had this
// backwards. v17 also rebuilt the link's kinematics for the corrected
// direction (`ts` a true fixed anchor at `al`, `lc` the real folding
// joint) but left 2 real, disclosed gaps, both closed this round:
//
// v18 real changes from v17, both from Janis's own direct teaching
// after reviewing the v17 render:
//
// (1) PIVOT RELOCATED to the hinge block's own real OUTER TIP
// (Y=-HINGE_OUT), not the inner point (Y=-HINGE_PIVOT_OFFSET) v17 still
// used. Janis's own catch: "it seem like you put in on the apex a and
// al plane not the tip of the small hinge notch" — matching a real door
// hinge's own knuckle position (at the leaf's outer edge). This was the
// root cause of v17's disclosed near-full-stow collision (only ~3mm of
// real depth existed behind the folded tray before this fix). Plate
// mounts flush at the same point now — `TRAY_MOUNT_GAP` retired
// entirely, no longer needed.
//
// (2) LINK LENGTH SIZING FIXED: `TRAY_LINK_C_HALF` is now exactly half
// the deployed reach (no lap-overlap padding). Checked in Python before
// writing this: with c1=c2 exactly half the max chord, the fold angle
// at the middle joint (`lc`) is mathematically guaranteed 180° (fully
// straight) at 0° — proven via the law of cosines — and stays smooth
// across the ENTIRE sweep, unlike v17's padded sizing which grew
// unstable near full stow. The real slop this hardware needs for an
// actual pin/lap joint is what Janis's own SLOT correction provides
// instead of a fixed pad: the mid-joint pivot rides in a real slot
// (schematic here, not modeled as a literal mechanism, matching
// hinge_u()'s own "not fab-precise" convention) rather than a rigid
// pin — same real idea, better conditioned.
//
// Re-verified via a real isolated intersection() probe (link vs. tray
// plate): EMPTY (no collision) across the ENTIRE 0-90° sweep — closes
// v17's disclosed 87-90° gap completely. DISCLOSED, NOT PERFECT
// (Janis's own call, this link isn't critical): the fold angle at `lc`
// reaches ~11° at full 90° stow, not a literal 0° — the real slotted
// pivot (not modeled precisely) would take up this last bit.
//
...
// CONSTANT TRAY_LINK_C_HALF away from it -- a genuine
// circle(ts,C_HALF)/circle(tt,C_HALF) intersection, not an
// approximation. Two roots exist; picking the one CLOSER to apex A is
// Janis's own direct correction (checked against a labeled triangle
// simulation before writing this) -- matches the real hardware
// description: as the tray folds toward stowed, `lc` genuinely travels
// toward apex A while `tt` tucks up under the tray and `ts` stays put
// at the wall. DISCLOSED, NOT PERFECT (Janis's own call, this link
// isn't critical): the fold angle at `lc` reaches ~11° at full 90°
// stow, not a literal 0° -- the real slotted pivot (not modeled as a
// literal mechanism here) would take up this last bit of play; not
// modeled precisely, matching hinge_u()'s own schematic convention.

...
// Version: v19
// Date: 2026-07-31
// Source: BBQ-offset-smoker-base-v18.scad
//
...
// overlap padding), closing v18's disclosed 87-90° collision — but left
// `lc` (the middle joint) landing 80-110mm away from both `ts` and `tt`
// at full fold, since a rigid circle/circle solve has no choice but to
// bow out when the link's own half-length vastly exceeds the shrunk
// ts-to-tt gap. v19 closes this, and a second real defect Janis caught
// directly from a render of the bracket cross-section:
//
// (1) `ts`'s OWN HINGE WAS SINKING INTO THE BRACKET. Janis's catch,
// after being shown an isolated bracket+hinge render: `al` (where `ts`
// lives) is literally one of `tray_bracket()`'s own 3 triangle
// vertices, so `hinge_u(ts,...)`, centered exactly on that vertex,
// genuinely overlapped the bracket's own solid wedge — confirmed via a
// real intersection() probe (22 vertices, 3 volumes, a real overlap,
// not a touch). This was never caught earlier because prior collision
// probes only checked the link against the TRAY PLATE, never against
// `tray_bracket()` itself.
//
// (2) `lc` REBUILT, per Janis's own direct resolution after being shown
// the real numbers (a free-body diagram: forcing `lc`'s Y within the
// desired band leaves the OTHER half's own real length ~21mm short of
// itself — physically impossible for a truly rigid bar): "we do not
// change the link length, we allow the pivot pin to slide along the
// link to compensate the diff of length need at each require angle."
// `lc` is now simply the MIDPOINT of `ts` and `tt` at every angle. This
// keeps `ts`-`lc`-`tt` exactly COLLINEAR (a straight, shortening strut,
// not a bowing 2-bar link) and puts `lc`'s own Y automatically BETWEEN
// `ts` and `tt`'s own Y at every angle, by construction — which, as a
// direct consequence, also keeps `lc` on the correct (non-bracket) side
// of the wall at every angle, since `tt`'s own Y never crosses positive.
//
// FIX for (1) and (2) together: the link's OWN internal depth-split
// (`TRAY_LINK_X_SPLIT`, from v18) is RETIRED — it existed to keep 2
// bowing, coplanar bars from interpenetrating each other, which no
// longer applies now that the link is a straight, collinear strut
// (verified: `Simple: yes`, no self-intersection, with both halves at
// the identical X). What both problems actually needed was the WHOLE
// link (not split internally) moved clear of the bracket's own solid
// X-span — `TRAY_LINK_BRACKET_CLEAR` does this. Re-verified via real
// isolated probes (link vs. bracket, link vs. tray plate, link's own
// self-manifold-ness): EMPTY / `Simple: yes` at every angle, 0-90°.
//
...
// shared boundary (a real, expected, zero-volume touch, same as
// hinge_u()'s own intentional touch on tray_bracket()) -- never a real
// volumetric overlap, at any swept angle. `TRAY_MOUNT_GAP` RETIRED (was
...
module tray(x0, angle_deg) {
    ...
    // direction + kinematics corrected in v17/v18, see tray_link()'s own
    ...
    // tray about the tray's own real pivot (Y=-HINGE_OUT, Z=HINGE_Z);
    // `ts` is a TRUE FIXED anchor at `al`; the link's own middle
    // lap-joint (`lc`) is what genuinely folds, via a real circle/circle
    // intersection -- see tray_link_lc() above.
...
// v19 REAL REBUILD -- `lc` rederived per Janis's own direct correction
// after inspecting where v18's circle/circle `lc` actually landed: "you
// should check tt and ts hinge alignment first... hinge must overlap
// each other" and, once shown the real numbers (ts/tt only ~6mm apart
// in Y at full fold, but the rigid circle/circle solve put `lc` 80-110mm
// away from BOTH), Janis's own resolution: "we do not change the link
// length, we allow the pivot pin to slide along the link to compensate
// the diff of length need at each require angle" -- a real telescoping
// slot at the mid-joint, not a fixed-length rigid 2-bar link. `lc` is
// now simply the MIDPOINT of `ts` and `tt` at every angle: this keeps
// `ts`-`lc`-`tt` exactly COLLINEAR (a straight, shortening strut, not a
// bowing 2-bar link) and `lc`'s own Y automatically stays BETWEEN `ts`
// and `tt`'s own Y at every single angle (proven, not just observed --
// a midpoint's own coordinate is always between its two endpoints') --
// closing both the "hinges must align" requirement AND (as a direct
// consequence, since `tt`'s Y never crosses positive) the bracket-sink
// finding, without any circle-intersection ill-conditioning at all. At
// 0° (deployed) this lands exactly on the old TRAY_LINK_C_HALF value by
// construction (half the deployed reach) -- matches the original
// "180° at deployed" reference exactly. The real slack this now asks of
// the slot is large (each half's own real length stays fixed at
// TRAY_LINK_C_HALF, but the WORKING ts-to-lc/tt-to-lc distance shrinks
// to ~11mm at full 90° stow) -- disclosed, not modeled as a literal
// slot mechanism, matching hinge_u()'s own schematic convention.
function tray_link_lc(angle_deg) =
    (TRAY_LINK_TS_REF + tray_link_tt(angle_deg)) / 2;
...
// documented reference; NOT used as a geometric depth-split anymore as
// of v19 -- see below for why.
TRAY_LINK_STACK_T = 10;
// v19 REAL FIX -- Janis's own direct catch, after being shown the real
// bracket cross-section: `ts`'s own hinge (and the flat link approaching
// it) sat centered exactly ON the bracket's own vertex (`al` IS one of
// `TRAY_BRACKET_OUTLINE`'s three corners), so it genuinely sank into the
// bracket's solid wedge -- confirmed via a real
// intersection(hinge_u(ts),tray_bracket()) probe (22 vertices, 3
// volumes, a real overlap, not a touch). Since `ts`-`lc`-`tt` are now
// exactly collinear (v19's midpoint rebuild, see tray_link_lc() above)
// the whole link no longer needs an INTERNAL depth-split between its
// own 2 halves (verified: `Simple: yes`, no self-intersection, with
// both halves at the SAME X) -- the earlier `TRAY_LINK_X_SPLIT` idea
// was solving the OLD bowing-link's own self-interference, which no
// longer exists once the link is a straight, shortening strut. What
// IS still needed: clearing the WHOLE link away from the bracket's own
// solid X-span (`TRAY_BRACKET_W`=30mm) plus `hinge_u()`'s own extra
// (TRAY_LINK_T+6)=9mm thickness -- re-verified via a real isolated
// probe (link vs. bracket, link vs. tray plate, both swept 0-90°):
// EMPTY at every angle with this clearance, not assumed.
...
module tray_link(x_center, angle_deg) {
    ...
    hinge_u(ts, xc);
    hinge_u(tt, xc);
    translate([xc,lc[0],lc[1]]) rotate([0,90,0])

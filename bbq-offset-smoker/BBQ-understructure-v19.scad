...
//
// Wheels, axles, fender/track-width, support frame, and the include
// chain to BBQ-chambers-v26.scad. Most version bumps to this file are
// pure pointer-only chain-break fixes (this file's own `include` moved
// forward to keep the chambers file's own real changes from being
// silently orphaned from the render chain) — full history of every
// prior bump and the small number of real geometry rounds (fender/
// wheel-arch profile, level-drop companion, etc.) lives in
// cc_chat_log.md, not here.

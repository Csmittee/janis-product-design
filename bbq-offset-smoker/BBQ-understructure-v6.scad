...
// Version: v6
// Date: 2026-07-20
// Source: BBQ-understructure-v5.scad
// Changes: bbq-understructure-v6-fitment-fixes. 4 real fitment fixes from
// Janis's live OpenSCAD-desktop review of v5, PLUS a full numbers recompute
// against BBQ-chambers-v15.scad (the just-built square-shell/cylinder
// firebox redesign).
//
...
// ═══════════════════════════════════════════════════════════════════
// TASK 3 — REAR FENDER, v6 REBUILT AS A SHORT FLARED WING (was a "long
// straight panel" per Janis's live review). FENDER_GAP(150mm radial
// clearance)/FENDER_T(4mm)/FENDER_W(240mm rolling-direction length)/
// FENDER_ARC_PAST_EDGE(25deg) all UNCHANGED from v5 (none of those were
// the real problem, see file header). Real fix: the flat mounting
// portion is now a genuinely SHORT weld tab — the arc itself starts at
// the REAL angle where its own inner radius naturally reaches the
// firebox's own wall, live-computed via acos(), not a fixed 90deg
// top-of-wheel start.
// ═══════════════════════════════════════════════════════════════════
...
// rear_fender() -- v6 TASK 3 REBUILT. outward_sign: -1 for the LEFT
// wheel (outward = -Y), +1 for the RIGHT wheel (outward = +Y). d = real
// signed Y-distance from the wheel's own center to the firebox's own
// wall -- a_start = acos(d/FENDER_R_IN) is the REAL angle (live-computed,
// works symmetrically for both signs via acos()'s own [0,180] range,
// confirmed via echo: 58.11deg left / 121.89deg right at TASK 2's new
// 980mm track) where the arc's own inner surface naturally reaches the
// firebox wall -- the flat tab is now just FENDER_WELD_OVERLAP(1.5mm,
// UNCHANGED real weld overlap into the firebox's wall_t(3mm) band)
// beyond that, not a long run to the wheel's own centerline.
...
module rear_fender(wheel_y, firebox_edge_y, outward_sign) {
    d = firebox_edge_y - wheel_y;
    a_start = acos(d / FENDER_R_IN);   // real live angle, see file header (58.11/121.89deg at v6's own track width)
    ...
    z_in  = REAR_AXLE_Z + FENDER_R_IN  * sin(a_start);
    z_out = REAR_AXLE_Z + FENDER_R_OUT * sin(a_start);
    y_in  = wheel_y + FENDER_R_IN  * cos(a_start);   // == firebox_edge_y by construction
    y_out = wheel_y + FENDER_R_OUT * cos(a_start);
    translate([REAR_AXLE_X - FENDER_W/2, 0, 0]) rotate([0, 90, 0])
        linear_extrude(height = FENDER_W, convexity = 6) {
            fender_arc_2d(wheel_y, REAR_AXLE_Z, FENDER_R_OUT, FENDER_R_IN, a_start, a_end);
            polygon(points=[
                hex_pt(z_in, weld_edge_y),
                hex_pt(z_out, weld_edge_y),
                hex_pt(z_out, y_out),
                hex_pt(z_in, y_in),
            ]);
        }

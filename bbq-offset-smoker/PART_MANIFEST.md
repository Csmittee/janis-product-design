...
#
# Version: 1.17 — 2026-07-20 (bbq-chambers-v15-square-shell-cylinder-
...
## BBQ-chambers-v15.scad

| Module | What it IS | What it is NOT (only if real confusion risk exists) | Toggle |
|---|---|---|---|
| `chamber_shell()` | UNCHANGED CODE (chamber's own shape frozen, DO NOT TOUCH) — fixed portion of the octagon, full chamber_L length, wall_t hollow. Real world position UNCHANGED this round (chamber_floor_z=771.335mm, untouched — TASK 1's height change is firebox-only). Rear wall carries the REBUILT `firebox_passage()` (v15: real CIRCLE, TASK 3); front end-cap carries exhaust_room_opening() | the lid's own territory for X=[LID_X0,LID_X1] — a SEPARATE part (`lid()`) | `show_chamber_shell` |
| `lid_territory_margin_fill()` | UNCHANGED CODE. Real, confirmed contact vs `outer_shell()`'s own flange at the chamber's rear-territory zone (X=[893.5,915]) — SAME real weld contact as every version since v14 (byte-identical facet/volume count confirmed via CGAL, zero change from firebox_x0 staying pinned at 913.5mm this round) | a NEW conflict introduced by TASK 1/2's firebox growth — confirmed unchanged/pre-existing, not a regression | (none — sub-part of `chamber_shell()`, no separate toggle) |
| `firebox_passage()` | v15 TASK 3 REBUILT as a plain CIRCLE (`firebox_passage_profile()`) — was v14.2's trapezoid, derived from the octagon's taper to match a rectangular duct; that derivation has no basis now the duct is round. Real target area (0.008 x the NEW real fire volume): 46.242in²=29,833.5mm², diameter 194.898mm. REAL FINDING: the fire cylinder's own true center now sits BELOW chamber_floor_z (a direct consequence of TASK 1's height growth) — passage repositioned biased to the cylinder's own real top (5mm margin, judgment call, flagged). Real CGAL measurement (STL volume, not estimated): 88.7% of the circle's own area is genuinely cut through real chamber material, ~11.3% a real confirmed chord below the floor (matches this project's own v13/v14 precedent for circular-passage findings, not a novel deviation) | a placeholder — the target AREA is a locked project rule (0.008x rule), but this round's real SHAPE/position choice (vs. a trapezoid) is a stated reasoning call, not a placeholder either | (none — sub-part of `chamber_shell()`, no separate toggle) |
...
| `firebox(firebox_door_open_deg, ash_tray_out_pct)` | v15 REWORK — TWO fully independent welded assemblies (fire cylinder + outer shell, see next rows), SAME real architecture v14 established. `firebox_door()` UNCHANGED CODE, real dimensions stay 580mm wide (FIREBOX_W unchanged), now mounted on a taller 580mm-high shell (was 428.6mm) — flagged real consequence. `ash_tray()` width formula REBUILT for the round cylinder's own real geometry (see its own row) | one shared assembly — deliberately two separate, non-touching solids | `show_firebox` |
| `fire_cylinder()` / `fire_cylinder_end_cap()` | v15 TASK 2, NEW — REPLACES `inner_duct()`/`inner_duct_end_cap()` (RETIRED, R-009). Round, 456mm diameter (62mm wall clearance vs the outer shell, Janis's own explicit choice, real OPEN AIR gap), full `FIREBOX_L`(580mm, was 460mm) span, open both ends (same convention as the retired rectangular duct). Own end cap welds DIRECTLY to the chamber's own octagon end cap (NOT to the outer shell) — real full circle minus the SAME passage-hole shape `firebox_passage()` cuts (shared 2D module). Real CGAL seal-weld: non-empty contact vs the chamber's own solid wall, confirmed; zero-contact vs `outer_shell()` confirmed EMPTY | connected to or touching the outer shell anywhere — confirmed via a real mandatory zero-contact CGAL check | (none — sub-part of `firebox()`, no separate toggle) |
| `fire_cylinder_partition()` | v15 TASK 4, NEW. Square end plate (matches `outer_shell_footprint_2d()`'s own real 580x580 exterior footprint, R-009 reuse) minus a circle matching the cylinder's own real diameter (+2e real clearance, avoids an exact-diameter coincident-surface touch) — physically locates the cylinder within the square outer shell (per Janis's reference photo). Positioned at the door/front end (X=[firebox_x1-wall_t-e,firebox_x1-e] — real, found+fixed via CGAL: landing exactly at firebox_x1 produced a real Simple:no coincident-face result against `firebox_door()`, pulled back by e, re-verified Simple:yes/empty). Real CGAL: non-empty contact vs `outer_shell()`'s own wall, EMPTY vs the cylinder and the door | welded/fused to the cylinder itself — a separate locating collar, real clearance not material overlap | (none — sub-part of `firebox()`, no separate toggle) |
| `outer_shell()` / `outer_shell_end_cap()` | v15 TASK 1: real 580x580x580 CUBE (FIREBOX_H 428.6->580mm, FIREBOX_W unchanged, FIREBOX_L now 580mm too — all three real dimensions equal). Flange/end-cap construction UNCHANGED CODE (`outer_shell_footprint_2d()`/`outer_shell_flange_cut_2d()`, both reused as-is, automatically consume the new FIREBOX_H/FIREBOX_L live). Real CGAL re-verification (mandatory, not assumed): the flat "no material below the floor" construction still Simple:yes at the new, much larger range (firebox now extends 351.335mm below chamber_floor_z, was 199.935mm) | a redesigned construction — same code, real values just recompute automatically off the new FIREBOX_H/L | (none — sub-part of `firebox()`, no separate toggle) |
...
| Module | What it IS | What it is NOT (only if real confusion risk exists) | Toggle |
|---|---|---|---|
| `rear_axle()` | TASK 3 REPOSITION. WHEEL_D/WHEEL_R/TREAD_W UNCHANGED (457.2/228.6/200, 18"). REAR_AXLE_Z UNCHANGED (=WHEEL_R=228.6, wheel bottom still real world Z=0). REAR_AXLE_X UNCHANGED (1143.5, real firebox X-midpoint, v14.2 preserves it). REAR_WHEEL_Y_LEFT/RIGHT now driven by the NEW shared `TRACK_WIDTH`(1080mm, TASK 1) = -235/845mm (was -150/760 under v4+v14.2-pointer-alone) — real, confirmed 150mm gap to the firebox's own outer-shell edge each side (closer than v4's old 200mm FIREBOX_CLEARANCE gap, per this round's own spec). REAR_BRACKET_H UNCHANGED VALUE (342.8mm — both its own inputs, firebox_floor_z and REAR_AXLE_Z, are unchanged from v4). Strut/spacer/beam construction UNCHANGED (DO NOT TOUCH), real spacer lengths widen automatically (379.5mm each, was 308mm) | a swivel/steerable mechanism — static, no kinetic parameter | `show_rear_axle` |
| `rear_fenders()` | NEW, TASK 3. Flat steel panel welded to the firebox's own OUTER SHELL wall (`firebox_y0`/`firebox_y1`), real 150mm radial clearance (`FENDER_GAP`) maintained continuously by a concentric arc (`fender_arc_2d()`, same pie-slice-vs-ring masking technique `true_octagon_profile()`/`fixed_side_wedge()` already use) centered on the wheel's own axle, curving `FENDER_ARC_PAST_EDGE`(25deg) past the wheel's own outer edge. `FENDER_T`(4mm)/`FENDER_W`(240mm) are judgment calls (no numeric spec given beyond the 150mm gap), flagged. Real CGAL: EMPTY vs `rear_wheels()` (no collision, maintains clearance throughout the curve, not just at the endpoints), NON-EMPTY vs `outer_shell()` (real weld contact, `FENDER_WELD_OVERLAP`=1.5mm into the wall_t=3mm band) | a full wheel-well enclosure — this is a single flat+curved panel per side, open underneath, per this task's own spec | `show_rear_fenders` |
| `front_wheel_support(steer_deg, handle_fold_deg)` | `front_bracket()`'s own MAIN SHAPE (TOP_W/BOT_W/MID_H, the 2-trapezoid-leg construction) UNCHANGED (DO NOT TOUCH, TASK 4) — real diff is `LEG_DROP` only, now a live formula (`chamber_floor_z-firebox_floor_z`=199.935mm, was a 150mm literal) so the leg's own bottom edge lands EXACTLY at `firebox_floor_z`(571.4mm), confirmed via echo. `front_caster_plate()`/`FRONT_SPACER_LEN` recompute automatically (336.8mm, was 215.4mm). `front_stub_axle()`/`front_wheels()` TASK 1/5 respan from the retired `DUAL_WHEEL_OFFSET`(110mm) to the SAME shared `TRACK_WIDTH`/2(540mm) TASK 1 gives the rear — front wheel is now a real COPY of the rear wheel. *** REAL, CGAL-CONFIRMED: THE STANDING v4 ~6mm FRONT-WHEEL/BRACKET COLLISION IS RESOLVED ***, TASK 7 — the wider stance alone puts the near tire face 224mm clear of the bracket's own widest edge (was overlapping); confirmed via a real `intersection()` probe (front_wheels() vs front_bracket()+front_caster_plate()+tow_triangle()) returning EMPTY, not assumed from the wider spacing alone | still colliding — this round's own TRACK_WIDTH change is what fixes it, confirmed not assumed | `show_front_wheel_support` |
| `tow_triangle()` / `tow_handle(handle_fold_deg)` | TASK 2 (tip) + TASK 6 (length/angle). Triangle: sharp pyramid apex REBUILT round via `hull()` (150mm dia boss, `TIP_D`, this task's own explicit minimum — judgment call: read as the rounded boss's own real diameter, flagged) — forward reach (`TRIANGLE_APEX_X`=-300) UNCHANGED, so `HINGE_X`'s own formula is untouched, but now lands inside genuine round material instead of v4's thin near-point sliver (real, flagged improvement). Handle: `HANDLE_UPRIGHT_LEN`(400mm literal) RETIRED, replaced by live `TBAR_LEN` = `DATUM_Z_RIDGE`(roof)-`WHEEL_R`-`TBAR_BRACKET_THICKNESS`(50) = 1102.735mm. `handle_fold_deg` default FIXED 0->90 (vertical storage — was shipping flat/towing by default, a standing v12/v4 QA defect, now corrected and real-render-confirmed). *** REAL, FLAGGED VARIANCE, NOT SILENTLY ABSORBED ***: TBAR_LEN's own formula implicitly assumes a wheel-height pivot; this file's real (unchanged, out-of-scope-this-round) `TRIANGLE_Z` hinge sits 100mm higher, so at the new 90deg default the tip's real Z (1431.335mm) exceeds `DATUM_Z_RIDGE`(1381.335mm) by 50mm on paper — a real CGAL check (`intersection()` vs the full built chamber_shell()/lid()/firebox()/exhaust_room()/chimney_pipe()) confirms this causes NO actual collision (the T-bar at 90deg sits forward of/outside the chamber's own X-footprint entirely), but the numeric margin itself is genuinely negative, flagged for the record, not silently resolved either direction | independent of the front caster (unchanged from v3/v4) — still rigidly coupled | (none — sub-parts of `front_wheel_support()`'s own `front_swivel_assembly()`, no separate toggle, unchanged from v4) |
| `prep_shelves(shelf_deployed)` | UNCHANGED from v4 (DO NOT TOUCH this round). Mount point (`GRATE_Z`) is chamber-frozen; real value moves to 950mm (v14.2, was 778.665mm under v13) purely as an automatic consequence of TASK 0's pointer bump, not this round's own scope | | `show_shelves` |

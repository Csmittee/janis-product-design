...
# check): (1) `ts`'s own hinge was genuinely SINKING into
# `tray_bracket()`'s own solid wedge -- `al` (where `ts` lives) is
# literally one of the bracket's own 3 triangle vertices, so
# hinge_u(ts,...), centered exactly on that vertex, overlapped the
# bracket's own material (real intersection() probe: 22 vertices, 3
# volumes -- genuine overlap). Root QA gap: prior collision probes only
# ever checked the link against the tray PLATE, never against
# tray_bracket() itself. (2) `lc` (the middle lap-joint) was landing
# 80-110mm away from BOTH `ts` and `tt` at full fold instead of visibly
# aligning with them, per Janis's own explicit requirement ("hinge must
# overlap each other"). Proven via a free-body diagram: at 90 degrees,
# ts-to-tt shrinks to ~22mm but each rigid link half stays fixed at
# ~110.8mm -- forcing lc's Y into the desired band leaves the OTHER
# half ~21mm SHORT of its own fixed length, physically impossible for a
...
# not a fixed-length rigid 2-bar link. FIX: `lc` is now simply
# `midpoint(ts, tt)` at every angle -- keeps ts-lc-tt exactly collinear
# (a straight, shortening strut, not a bowing 2-bar link) and puts lc's
# Y automatically between ts/tt's own Y at every angle by construction,
# which as a direct consequence also keeps lc clear of the bracket. The
# v18-era internal depth-split (TRAY_LINK_X_SPLIT, for 2 bowing coplanar
# bars) is RETIRED -- no longer bows; instead the WHOLE link (both
# halves, uniformly) is offset clear of the bracket's real X-span via a
# new TRAY_LINK_BRACKET_CLEAR constant. Re-verified across the entire
# 0-90 degree sweep using the real code: full-assembly Simple:yes, link
# vs tray_bracket() empty, link vs tray plate empty, link vs
# front_wheel_support() empty, and the link's own self-manifold-ness
# clean (Simple:yes, no self-intersection at the now-collinear lc).
# Disclosed: the real slot magnitude this now needs is large (~100mm of
# working travel per side; each half's own fixed length is unchanged) --
# still schematic, not modeled as a literal slot mechanism, matching
# hinge_u()'s own convention. `v16.scad` archived (last-3-live:
# v17/v18/v19). Full summary: docs/handoff-2026-07-30.md (updated),
# docs/tray-relocation-bracket.md (updated, "v19" section).
# Previous: 1.50 — 2026-07-31 (bbq-lid-hinge-v18, relocate tray pivot +

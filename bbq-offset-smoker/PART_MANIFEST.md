...
#
# Version: 1.18 — 2026-07-20 (bbq-understructure-v6-fitment-fixes):
...
## BBQ-understructure-v6.scad

| Module | What it IS | What it is NOT (only if real confusion risk exists) | Toggle |
|---|---|---|---|
| `rear_axle()` | TASK 0/2/4 recompute. WHEEL_D/WHEEL_R/TREAD_W UNCHANGED (457.2/228.6/200). REAR_AXLE_X formula UNCHANGED but real value shifts +60mm (1203.5, was 1143.5) since v15's own firebox_x1 grew while firebox_x0 stayed pinned. REAR_WHEEL_Y_LEFT/RIGHT now driven by TASK 2's narrower `TRACK_WIDTH`(980mm) = -185/795mm (was -235/845mm) — real, confirmed 100mm gap to the firebox's own outer-shell edge each side (was 150mm, firebox insulated, wheel sits closer). REAR_BRACKET_H recomputes AUTOMATICALLY via v15's new firebox_floor_z (191.4mm, was 342.8mm) — zero code change, same unchanged formula | a swivel/steerable mechanism — static, no kinetic parameter | `show_rear_axle` |
| `rear_fenders()` | TASK 3 REBUILT as a short flared wing (was "a long straight panel," Janis's live review finding). Real cause found via direct computation: the flat mounting portion spanned the ENTIRE distance from the firebox wall to the wheel's own vertical centerline (250mm flat run) before the curve began. Fixed: flat portion is now a genuinely SHORT weld tab (`FENDER_WELD_OVERLAP`=1.5mm, unchanged) — the arc itself starts almost immediately, at the real angle (`a_start = acos(d/FENDER_R_IN)`, live-computed, works symmetrically both sides) where it naturally reaches the firebox wall. Real arc sweep now ~147° (was 115°). *** REAL BUG FOUND+FIXED VIA CGAL ***: this wider sweep broke `fender_wedge_2d()`'s own 3-point wedge-mask technique (the chord between its 2 far points cut INSIDE the fender's own ring at this wider angle, producing two disconnected thin slivers instead of a band — confirmed via real render, not assumed) — fixed with a real multi-point fan mask, robust to any total sweep angle, re-verified as a clean continuous band. `FENDER_GAP`(150mm)/`FENDER_T`(4mm)/`FENDER_W`(240mm)/`FENDER_ARC_PAST_EDGE`(25deg) all UNCHANGED. Real CGAL: EMPTY vs `rear_wheels()`, NON-EMPTY vs `outer_shell()` (real weld contact) | a full wheel-well enclosure — a single flat+curved panel per side, open underneath | `show_rear_fenders` |
...
| `prep_shelves(shelf_deployed)` | TASK 1 REBUILT mounting (kinematics UNCHANGED — `rotate([deg,0,0])` about the X-axis was already a proper hinge-LINE rotation for every X along the shelf, not just a point). Real problem was PHYSICAL SUPPORT: only ONE `tray_mount_bracket()` existed near the shelf's own near end while the shelf spans ~320mm — a real ~290mm unsupported cantilevered far end (Janis's live review finding). Fixed: `tray_mount_brackets()` now calls 3 real brackets (`TRAY_BRACKET_COUNT`, judgment call, flagged) evenly spread across the shelf's own real X span, same hinge line, real distributed weld support (confirmed via CGAL, all 3 individually contact real chamber material) — sits level/flush when folded down instead of drooping | a kinematic/rotation fix — the rotation was already correct, this is a support/fabrication fix only | `show_shelves` |

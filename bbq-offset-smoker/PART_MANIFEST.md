...
#
# Version: 1.11 — 2026-07-17 (bbq-chambers-v12-firebox-rebuild-understructure-
...
## BBQ-chambers-v12.scad

| Module | What it IS | What it is NOT (only if real confusion risk exists) | Toggle |
|---|---|---|---|
| `chamber_shell()` | UNCHANGED v12 (chamber frozen, DO NOT TOUCH this round) — fixed portion of the octagon (floor + RIGHT wall + right chamfer + half ridge), full chamber_L length, wall_t hollow. Real 8-point `true_octagon_profile()` hollowed FIRST, then clipped via `fixed_side_wedge()`. Rear wall carries `firebox_passage()` (auto-follows the v12 firebox rebuild, code unchanged); front end-cap carries exhaust_room_opening() | the lid's own territory for X=[LID_X0,LID_X1] — a SEPARATE part (`lid()`) | `show_chamber_shell` |
...
| `firebox(firebox_door_open_deg, ash_tray_out_pct)` | v12 REBUILT — independent FIREBOX_L/W/H=460/510/428.6mm (was a uniform 457mm cube), world Z=[571.4,1000] (LOCKED spec numbers). Wraps `firebox_shell()`, `firebox_near_wall_closure()`, `firebox_upper_wall_seal()`, NEW `fuel_cylinder()` (v12 TASK 2, replaces `fire_grate()`), `ash_tray()` (height reduced 80->12mm, flagged), `firebox_door()`. Real, CGAL-confirmed ~1.5mm solid overlap with `chamber_outer_tube()`'s own end cap at the new (X-midpoint-preserving) position — flagged, not a manifold defect (union of overlapping solids stays valid) | the old uniform-cube firebox — this version has independent per-axis dimensions | `show_firebox` |
| `firebox_near_wall_closure()` | UNCHANGED CODE v12 (Y-span auto-follows FIREBOX_W) — solid wall_t panel closing the real gap below chamber_floor_z. Real remaining band now only 28.6mm (was 200mm through v11) since the new firebox floor sits much closer to chamber_floor_z | the passage's own opening (stays open) OR the region ABOVE chamber_floor_z (`firebox_upper_wall_seal()`, below) | (none — sub-part of `firebox()`, no separate toggle) |
| `firebox_upper_wall_seal()` | UNCHANGED CODE v12 (auto-follows the new firebox_square_2d() rectangle, R-009) — still closes the "triangle leak" region above chamber_floor_z | | (none — sub-part of `firebox()`, no separate toggle) |

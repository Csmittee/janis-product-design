...
| Lid hinge/handle/CB1 (base-v12, 2026-07-30) | closed (`door_open_deg=0`) | open (`door_open_deg=90`) | CB1 lateral link (`cb1_link_2d()`) built and wired in on ALL 3 ribs via a `with_cb1` boolean param. Shared pivot `FC_Y`/`FC_Z` = `HINGE_PIVOT_Y`/`HINGE_PIVOT_Z` (`BBQ-chambers-v26.scad`), read live by both the visual `lid()` and the rib assembly so they always rotate about the exact same center — `door_open_deg` drives both directly as an explicit module argument, no shared/reassigned global variable involved. Full labeled geometry reference: `docs/rib-cb1-terminology.png`. Re-verified via fresh CGAL `Simple: yes` at 0/90° with all 3 ribs present, before and after this round's sanitization pass (identical vertex/facet counts). Real moment analysis (`docs/lid-hinge-moment-analysis.md`, 2026-07-30): current design meets only 1 of Janis's 4 stated criteria (3.6kgf startup lift in range); the gravity moment never crosses negative across 0-90°, so the door does not self-hold open at 90° — root cause is the absence of a real counterbalancing mass (current CB1 is a stopper/bracket, not a counterweight). *** REAL DEFECT, STILL NOT FIXED *** rib0/rib2 (X=200/715) still fall inside tray0/tray1's own stow X-span — same real interference as every version since v6, unchanged, needs a follow-up decision/round |

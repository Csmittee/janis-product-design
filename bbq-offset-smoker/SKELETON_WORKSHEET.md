...
| Lid hinge/handle/CB1 (base-v13, 2026-07-30) | closed (`door_open_deg=0`) | open (`door_open_deg=90`) | CB1 lateral link (`cb1_link_2d()`, bracket only) built and wired in on ALL 3 ribs via a `with_cb1` boolean param. CB1's real counterweight mass, `cb1_pipe()` (4" sq. tube, both ends capped, real computed mass ~8kg), restored this round — its cross-section corners are frozen individually via `freeze_from_open()` so it stays aligned with the bracket's own DE-tangent frame (fixed a real angle bug Janis caught). Shared pivot `FC_Y`/`FC_Z` = `HINGE_PIVOT_Y`/`HINGE_PIVOT_Z` (`BBQ-chambers-v26.scad`), read live by both the visual `lid()` and the rib assembly. Re-verified `Simple: yes` at 0/45/90° with all 3 ribs + pipe present. Real moment analysis (`docs/lid-hinge-moment-analysis.md`): with the pipe included, meets 1 of Janis's 4 stated criteria outright (2.5kgf startup lift, close to the 3-7kgf target) and the moment now DOES cross negative before 90° (a real improvement over no-counterweight), but lands near-neutral equilibrium at full open rather than the ~5kgf self-holding target — a tuning decision (more counterweight leverage), not a code defect. *** REAL DEFECT, STILL NOT FIXED, RE-CONFIRMED *** rib0/rib2 (X=200/715) still fall inside tray0/tray1's own stow X-span — direct CGAL intersection confirms real overlap at door=0 with tray stowed/mid-fold, same interference as every version since v6, needs a follow-up decision/round. Prep trays also relocated this round (`docs/tray-relocation-bracket.md`) to clear the grab handle. |

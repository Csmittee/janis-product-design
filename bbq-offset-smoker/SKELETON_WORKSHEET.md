...
> render: `flat_link(ts,lc,xc)`'s rendered length is literally
> `norm(lc-ts)`, and since v19's `lc` was `midpoint(ts,tt)`, that length
> shrank continuously (110.76mm at deployed down to 10.92mm at 90°
> stow) — a real steel bar cannot change its own physical length.
> Co-developed over several rounds of Python simulation before writing
> any `.scad` code (free-tip paths, an orbit-boundary check Janis caught
> being violated on a first discarded attempt, a mandatory CW/CCW
> rotation-sense verification). Real fix: `ts`'s own bar keeps a TRUE
> FIXED length (110.76mm, never varies), sweeping smoothly and
> monotonically CLOCKWISE (viewed from +X) from the historical 0°
> bearing to exactly 90° (straight up, `pin.Y=0`) at full stow. `tt`'s
> own bar is ALSO a true fixed length (same nominal stock) but carries
> a real SLOT in its outer section — per Janis's own explicit call
> ("just one link need to have slot") — sized directly from a sampled
> sweep to ~30mm, not guessed. Re-verified: bar lengths constant at
> every sampled angle (`echo()`-confirmed, not assumed), pin never
> crosses into the bracket's +Y territory, pin never exceeds `tt`'s own
> constant orbit radius (a check Janis caught me failing on an earlier
> attempt), full assembly `Simple: yes`, link vs. bracket empty, link
> vs. tray plate empty, link's own self-manifold-ness clean. At 90°
> stow: `ts.Y=0`, `tt.Y=-6.0`, pin.Y=0.0, `tt`'s own bar tip `.Y=+1.4` —
> all four within 7mm, satisfying Janis's own alignment requirement.
> Disclosed, not addressed: `ts`'s own hinge still sits on the
> bracket's own vertex, kept collision-free only by
> `TRAY_LINK_BRACKET_CLEAR`'s existing X-offset (unchanged from v19),
> which Janis has separately flagged as making `ts` look shifted to an
> asymmetric corner. `v17.scad` archived. Full summary:
> `docs/handoff-2026-07-30.md`, `docs/tray-relocation-bracket.md`
> ("v20" section).
> Previous: 1.39 — 2026-07-31
> Changes: bbq-lid-hinge-v19. Active base assembly pointer is now
> `BBQ-offset-smoker-base-v19.scad` (source v18). Two real defects Janis
> caught directly from inspecting the v18 render, neither caught by any
> prior collision check: (1) `ts`'s own hinge was genuinely SINKING into
> `tray_bracket()`'s own solid wedge — `al` is literally one of the
> bracket's own 3 triangle vertices, so `hinge_u(ts,...)`, centered
> exactly on that vertex, overlapped the bracket's own material (real
> probe: 22 vertices, 3 volumes). Root QA gap: prior probes only ever
> checked the link against the tray plate, never against
> `tray_bracket()` itself. (2) `lc` (the middle lap-joint) was landing
> 80-110mm away from BOTH `ts` and `tt` at full fold, per Janis's own
> alignment requirement. Proven via a free-body diagram: at 90°,
> ts-to-tt shrinks to ~22mm but each rigid link half stays fixed at
> ~110.8mm — forcing lc's Y into the desired band leaves the other half
> ~21mm SHORT of its own fixed length, physically impossible for a
> rigid bar. Janis's own resolution: "we do not change the link length,
> we allow the pivot pin to slide along the link to compensate the diff
> of length need at each require angle" — a telescoping-slot mid-joint.
> FIX: `lc` is now `midpoint(ts, tt)` at every angle — keeps ts-lc-tt
> exactly collinear (a straight, shortening strut, not a bowing 2-bar
> link) and puts lc's Y automatically between ts/tt's own Y at every
> angle by construction, which also keeps lc clear of the bracket. The
> v18-era internal depth-split (`TRAY_LINK_X_SPLIT`) is RETIRED (link no
> longer bows); the WHOLE link is instead offset clear of the bracket's
> real X-span via a new `TRAY_LINK_BRACKET_CLEAR`. Re-verified across
> the entire 0-90° sweep: full-assembly `Simple: yes`, link vs.
> `tray_bracket()` empty, link vs. tray plate empty, link vs.
> `front_wheel_support()` empty, link's own self-manifold-ness clean.
> Disclosed: the real slot travel needed is large (~100mm per side).
> `v16.scad` archived. Full summary: `docs/handoff-2026-07-30.md`,
> `docs/tray-relocation-bracket.md` ("v19" section).
> Previous: 1.38 — 2026-07-31
...
> DIRECTION, LOCKED, never to change: `angle_deg` -90=stowed actually
> swung the tray tip UP past apex A, not down toward the ground —
> confirmed via a real colored render. Fixed convention:
> `tray0_angle_deg`/`tray1_angle_deg` 0=deployed .. +90=stowed (tip
> down), clockwise about X viewed from +X when deploying. (2) Link
> kinematics rebuilt via a real taught 2-bar-link exercise (a triangle
> a/b/c, checked against a labeled Python simulation first): under the
> corrected direction the fixed-`ts`-to-rotating-`tt` distance genuinely
> shrinks as the tray folds, so `ts` reverts to a TRUE FIXED anchor at
> `al` (no slider); the link's own middle joint (`lc`) now does the real
> folding, via a circle/circle intersection picking the root closer to
> apex A; the 2 link halves are split in depth (not coplanar, same fix
> real hinges/scissors use — checked via a rectangle-overlap simulation
> first) with the combined stack held to a real 10mm total (nested, not
> additive, per Janis's own correction). Re-verified: empty (no
> collision) from 0° to about 86° — the entire normal range, a large
> improvement over v16. Disclosed, not fully resolved: a real, small
> collision from about 87° to 90° (near-degenerate `ts`/`tt` geometry,
> not a selection-rule bug) — accepted as a practical ~86° fold limit.
> `v14.scad` archived. Full summary: `docs/handoff-2026-07-30.md`,
> `docs/tray-relocation-bracket.md`.
> Previous: 1.36 — 2026-07-30
> Changes: bbq-lid-hinge-v16. Active base assembly pointer is now
> `BBQ-offset-smoker-base-v16.scad` (source v15). Real fix, Janis's own
> direct catch after reviewing the v15 render ("you got to wire it"):
> `tray_link()` was real geometry but STATIC, always drawn at its
> deployed (0°) reference position regardless of `tray0_angle_deg`/
> `tray1_angle_deg`. Now genuinely angle-dependent for both trays: `tt`
> rotates rigidly with the tray about its own real pivot; `ts` (the
> slider) re-solves every call via a real circle/line intersection,
> picking the physically-valid root (verified in Python first). Isolated
> collision probe re-verified: empty from 0° to about -82° (the normal
> operating range). New, disclosed, NOT-yet-fixed finding: a real, small
> collision from about -83° to -90° (last ~7-8° approaching full
> vertical stow) — the folded plate's near face and the fixed `Y=0`
> slider line are only ~3mm apart at full stow, too tight for the
> link's 15mm flat strip with no real slot modeled; confirmed symmetric
> on both trays; needs a decision (accept -85° as the practical fold
> limit, or a real notch/slot at the wall face). `v13.scad` archived.
> Full summary: `docs/handoff-2026-07-30.md`,
> `docs/tray-relocation-bracket.md`.
> Previous: 1.35 — 2026-07-30
> Changes: bbq-lid-hinge-v15. Active base assembly pointer is now
> `BBQ-offset-smoker-base-v15.scad` (source v14). Real folding-link
> hardware rebuild per Janis's detailed spec: `TRAY_MOUNT_GAP`=16mm
> fixes the tray plate overlapping the widened hinge block (no real gap
> existed before); link rebuilt as a flat 15mm steel strip with a real
> ~10mm lap overlap at the middle pivot, U-hinge+pin at both ends, `tt`
> pulled clear of the tray surface. `v12.scad` archived. Full summary:
> `docs/handoff-2026-07-30.md`, `docs/tray-relocation-bracket.md`.
> Previous: 1.34 — 2026-07-30
> Changes: bbq-lid-hinge-v13, REAL FILE VERSION BUMP. Active base
> assembly pointer is now `BBQ-offset-smoker-base-v13.scad` (source
> v12) — every change since the CB1 lateral link round (sanitization,
> CB1 pipe restore, moment analysis, tray relocation bracket/link/skirt,
> CB1 pipe rotation fix) had been made in place to the file still named
> v12, never bumped; v12 is now restored to its real frozen state as of
> that round and kept as historical record. Full current-state summary:
> `docs/handoff-2026-07-30.md`.
> Previous: 1.32 — 2026-07-30
> Changes: bbq-lid-hinge-v12, sanitization + real moment analysis round.
> PART C's "Lid hinge/handle/CB1" row rewritten to drop stale v7-era
> pivot/Customizer narrative and add this round's real moment-analysis
> result (`docs/lid-hinge-moment-analysis.md`) — full history:
> cc_chat_log.md's 2026-07-30 entries.
>
> Previous: 1.31 — 2026-07-30, bbq-lid-hinge-v12 CB1 lateral link,
> FINAL/COMPLETE (consolidates several same-day rework passes). PART C's
> "Lid hinge/handle/CB1" row updated: `cb1_link_2d()` now applies to ALL
> 3 ribs (widened from the source prompt's own original middle-rib-only
> scope). Bracket (Ua/Ub/Uc) + neck (50mm wide) + T4U/t6be/t7/t7u connect
> it into the existing t4/t5 rib body as one solid filled region.
> `t1`-`t6` profile itself untouched (locked). Full labeled reference:
> `docs/rib-cb1-terminology.png`. Full `--render` (CGAL) `Simple: yes`
> at 0/45/90° with all 3 ribs present, DXF 3-contour check clean.
> Previous: 1.29 — 2026-07-27
...
| Lid hinge/handle/CB1 (base-v20, 2026-07-31) | closed (`door_open_deg=0`) | open (`door_open_deg=90`) | CB1 lateral link (`cb1_link_2d()`, bracket only) built and wired in on ALL 3 ribs via a `with_cb1` boolean param. CB1's real counterweight mass, `cb1_pipe()` (4" sq. tube, both ends capped, real computed mass ~8kg), restored, cross-section aligned to the bracket's own DE-tangent frame. Shared pivot `FC_Y`/`FC_Z` = `HINGE_PIVOT_Y`/`HINGE_PIVOT_Z` (`BBQ-chambers-v26.scad`). Real moment analysis (`docs/lid-hinge-moment-analysis.md`): meets 1 of Janis's 4 stated criteria outright (2.5kgf startup lift); lands near-neutral equilibrium at full open rather than the ~5kgf self-holding target — a tuning decision, not a code defect. *** REAL DEFECT, STILL NOT FIXED, RE-CONFIRMED *** rib0/rib2 (X=200/715) still fall inside tray0/tray1's own stow X-span. Prep trays relocated (`docs/tray-relocation-bracket.md`): v15 rebuilt the folding link as real hardware-style geometry; v16 wired the link to genuinely track the tray's own angle; v17 fixed a real fold-DIRECTION bug (LOCKED convention: 0=deployed .. +90=stowed tip-down) and rebuilt the link kinematics to match, disclosing a real 87-90° collision. v18 traced that collision to the tray's own rotation pivot sitting at the hinge block's OLD inner point instead of its real outer tip (`Y=-HINGE_OUT`) and fixed the link's own length sizing, but its own "fully closed" claim was premature — it never checked the link against `tray_bracket()` itself. v19 fixed the bracket-sink defect (`ts`'s own hinge was genuinely sinking into `tray_bracket()`'s own solid wedge) but its own `lc = midpoint(ts,tt)` fix turned out to be a NON-PHYSICAL shrinking bar — Janis's own catch reading the code: the link's rendered length literally shrank from 110.76mm to 10.92mm across the fold. v20 replaced this outright: `ts`'s own bar keeps a TRUE FIXED length, sweeping clockwise (viewed from +X, mandatory sense); `tt`'s own bar, also fixed length, carries a real ~30mm slot (per Janis's own explicit call, "just one link need to have slot") so the shared pin can ride at a variable reach. Re-verified empty/`Simple: yes` across the ENTIRE 0-90° range, including link-vs-bracket, the link's own self-manifold-ness, AND real bar-length constancy via `echo()` at every sampled angle (no shrink, confirmed not assumed). `TRAY_MOUNT_GAP` retired (v18). Still not perfect (Janis's own call): `ts`'s own hinge still sits on the bracket's own vertex, kept collision-free only by an X-offset Janis has separately flagged as making `ts` look off-center — not resolved yet. |

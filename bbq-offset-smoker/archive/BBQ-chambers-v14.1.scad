...
// firebox_passage_profile() — v14.1 TASK 2 REBUILT. v14's
// passage_circle_2d()/PASSAGE_D/PASSAGE_Y_CENTER/PASSAGE_Z_CENTER
// (independently-sized 197mm circle, RETIRED this round, R-009 confirmed
// zero other real consumers) produced a real, flagged chord-shaped cut
// (did not fully clear real chamber material). Janis's direction: stop
// treating the passage as duct-shaped -- it is a hole through the
// CHAMBER's own octagon end cap only, any shape is fine as long as it
// stays within the inner duct's own real footprint (what it welds
// against). New shape: a real trapezoid, DERIVED DIRECTLY from
// true_octagon_profile()'s own real boundary at local h=0 and h=chamfer
// (the SAME hex_pt(h,w) 2D space/closed-form boundary GRATE_Y0/GRATE_Y1
// already use -- not an invented taper).
// *** THIS TRAPEZOID IS JANIS'S REAL, LOCKED DESIGN CHOICE (heat-rises /
// ash-avoidance reasoning: narrow at the floor where ash collects, wide
// higher up where heat naturally rises toward the duct) -- NOT A
// PLACEHOLDER, unlike v14's now-retired circle. Do not treat these
// proportions as adjustable defaults. ***
// TRAP_MARGIN=wall_t+10=13mm: SAME real weld-lip margin formula
// GRATE_Y_SAFETY already uses elsewhere in this file, not a new
// convention.
TRAP_MARGIN = wall_t + 10;                                    // 13mm
// Bottom edge, local h=0 (world Z=chamber_floor_z): real octagon floor
// width = chamber_W-2*chamfer = 252.670mm, inset by TRAP_MARGIN each side.
TRAP_BOT_W = (chamber_W - 2*chamfer) - 2*TRAP_MARGIN;         // 226.670mm
TRAP_BOT_H = 0;
// Top edge, local h=chamfer (world Z=chamber_floor_z+chamfer=950mm).
// REAL CROSS-CHECK: 950mm = GRATE_Z = apex A EXACTLY (chamber_floor_z=
// 950-chamfer by construction, so chamber_floor_z+chamfer collapses to
// 950 algebraically) -- confirmed, not a coincidence to re-derive
// differently. At h=chamfer the real octagon reaches its FULL chamber_W
// (610mm) width (both chamfer corners complete simultaneously, a regular-
// octagon symmetry property) -- but the top edge must also stay within
// the inner duct's own real footprint (DUCT_W=540mm, what this passage
// actually welds against) -- computed live, not assumed:
TRAP_TOP_W = min(chamber_W, DUCT_W);                          // 540mm -- duct governs, not the octagon
TRAP_TOP_H = chamfer;

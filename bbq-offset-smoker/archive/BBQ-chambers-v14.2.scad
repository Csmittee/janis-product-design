...
// firebox_passage_profile() — v14.2 TASK 1 RESIZED + REPOSITIONED (was
// v14.1 TASK 2's own build). Real trapezoid, still DERIVED DIRECTLY from
// true_octagon_profile()'s own real boundary conventions (hex_pt(h,w) 2D
// space) -- shape/taper direction UNCHANGED (top wider than bottom,
// following the octagon's real widening direction), only the real SIZE
// and TOP Z-position change this round.
// *** THIS TRAPEZOID IS JANIS'S REAL, LOCKED DESIGN CHOICE (heat-rises /
// ash-avoidance reasoning) -- NOT A PLACEHOLDER. Do not treat these
// proportions as adjustable defaults. ***
// TRAP_MARGIN=wall_t+10=13mm: SAME real weld-lip margin formula
// GRATE_Y_SAFETY uses elsewhere in this file -- kept only as the basis
// for v14.1's own bottom/top values below, which this round's real
// target-area scale is derived FROM (preserves the same proportional
// taper, per this round's own explicit instruction), not re-derived
// independently.
TRAP_MARGIN = wall_t + 10;                                    // 13mm
TRAP_V141_BOT_W = (chamber_W - 2*chamfer) - 2*TRAP_MARGIN;    // 226.670mm -- v14.1's real bottom, taper basis only
TRAP_V141_TOP_W = min(chamber_W, DUCT_W);                     // 540mm -- v14.1's real top, taper basis only
// TASK 1 (v14.2) -- real 0.008-of-fire-volume opening-area target, this
// project's own standing sizing rule (was never checked against it in
// v14.1, a real, flagged oversight this round fixes):
FIRE_VOLUME_IN3 = DUCT_W * DUCT_H * FIREBOX_L / 16387.064;    // 5,890.51in3 -- UNCHANGED, re-stated live not re-derived differently
TRAP_TARGET_AREA_IN2 = FIRE_VOLUME_IN3 * 0.008;               // 47.124in2
TRAP_TARGET_AREA_MM2 = TRAP_TARGET_AREA_IN2 * 645.16;         // 30,402.57mm2
// TOP Z-POSITION -- real, DIRECT deviation from this round's own written
// prompt (which said height/Z-position stay frozen, only widths change)
// per Janis's own explicit chat clarification, which supersedes it,
// flagged here not silently reconciled: bottom edge STAYS at
// chamber_floor_z (unchanged anchor); top edge RISES from v14.1's
// chamfer(=apex A=950mm) to 20mm below the inner duct's own real top
// wall (DUCT_Z_CENTER+DUCT_H/2), a real weld-clearance margin -- NOT tied
// to apex A anymore (real, stated consequence: 960mm != 950mm, v14.1's
// "top edge = apex A" coincidence does not carry over here by design).
TRAP_WELD_GAP = 20;                                           // real weld-clearance margin, Janis's own direct spec this round
TRAP_BOT_H = 0;                                                // world Z = chamber_floor_z, UNCHANGED anchor
// duct top wall world Z = DUCT_Z_CENTER+DUCT_H/2, but those constants are
// declared later in the file (dependency-order) -- same real value
// re-derived here from firebox_z0/firebox_z1 (both declared earlier,
// DUCT_Z_CENTER's own formula is (firebox_z0+firebox_z1)/2, DUCT_H is
// FIREBOX_H-2*DUCT_MARGIN, both algebraically identical, confirmed via
// echo): DUCT_Z_CENTER+DUCT_H/2 = (firebox_z0+firebox_z1)/2 +
// (FIREBOX_H-2*DUCT_MARGIN)/2 = firebox_z1 - DUCT_MARGIN (since
// FIREBOX_H=firebox_z1-firebox_z0, the terms collapse) = 980mm.
TRAP_TOP_H = (firebox_z1 - DUCT_MARGIN) - TRAP_WELD_GAP - chamber_floor_z;  // 188.665mm (world Z=960mm)
// WIDTHS -- scaled down from v14.1's own real bottom/top values,
// preserving the SAME proportional taper, solved against the NEW height
// to hit the real target area (not re-derived from the margin formulas
// independently -- this round's own explicit instruction):
TRAP_SCALE = TRAP_TARGET_AREA_MM2 / ((TRAP_V141_BOT_W + TRAP_V141_TOP_W)/2 * TRAP_TOP_H);
TRAP_BOT_W = TRAP_SCALE * TRAP_V141_BOT_W;                    // 95.287mm
TRAP_TOP_W = TRAP_SCALE * TRAP_V141_TOP_W;                    // 227.004mm
module firebox_passage_profile() {
    polygon(points = [
        hex_pt(TRAP_BOT_H, DATUM_Y_CENTER - TRAP_BOT_W/2),
        hex_pt(TRAP_BOT_H, DATUM_Y_CENTER + TRAP_BOT_W/2),
        hex_pt(TRAP_TOP_H, DATUM_Y_CENTER + TRAP_TOP_W/2),
        hex_pt(TRAP_TOP_H, DATUM_Y_CENTER - TRAP_TOP_W/2),
    ]);
}

...
FLANGE_LEN       = 20;                                          // outer shell's near-end structural flange, ADDITIVE (not a trim) -- UNCHANGED
FIREBOX_SHELL_L  = FIREBOX_L + FLANGE_LEN;                       // 600mm (v15, was 480mm) -- outer shell's own TOTAL physical length (real, separate from FIREBOX_L)
...
// firebox_passage_profile() — v15 TASK 3 REBUILT as a plain CIRCLE (was
// v14.2's own trapezoid, DERIVED from true_octagon_profile()'s taper to
// match a RECTANGULAR duct's footprint -- that derivation has no natural
// basis now the duct is round; a circle matches the fire cylinder's own
// symmetry directly, not a forced re-fit of the old trapezoid logic. See
// file header for the full real reasoning + the real chord finding.
// v14.2's own TRAP_MARGIN/TRAP_V141_BOT_W/TRAP_V141_TOP_W/TRAP_SCALE/
// TRAP_BOT_W/TRAP_TOP_W/TRAP_WELD_GAP/TRAP_BOT_H/TRAP_TOP_H all RETIRED
// (R-009, zero remaining consumers -- the trapezoid-specific derivation
// chain has no successor, replaced wholesale by the circle math below).
//
// Real fire volume -- v15 TASK 2, cylinder-based (was DUCT_W*DUCT_H-based
// rectangular-duct math through v14.2):
FIRE_VOLUME_IN3 = PI * CYL_R * CYL_R * FIREBOX_L / 16387.064;    // 5,780.25in3 (v15, was 5,890.51in3) -- real, live-computed
PASSAGE_TARGET_AREA_IN2 = FIRE_VOLUME_IN3 * 0.008;                // 46.242in2 -- same 0.008-of-fire-volume rule v14.2 established
PASSAGE_TARGET_AREA_MM2 = PASSAGE_TARGET_AREA_IN2 * 645.16;       // 29,833.5mm2
// PASSAGE_D -- real diameter if built as a FULL, unclipped circle at the
// real target area (194.898mm) -- built at this real size regardless of
// the real chord-clip found below (same "build the real target size,
// report the real resulting containment honestly" convention v13/v14
// both established for their own circular passage attempts, not a novel
// choice here).
PASSAGE_D = sqrt(4 * PASSAGE_TARGET_AREA_MM2 / PI);               // 194.898mm
PASSAGE_R = PASSAGE_D / 2;                                        // 97.449mm
// PASSAGE_TOP_MARGIN -- real judgment call, flagged: real weld/edge
// clearance kept below the fire cylinder's own real top boundary (see
// file header for the full real positioning reasoning -- the cylinder's
// own true center sits BELOW chamber_floor_z entirely this round, so the
// passage is biased toward the cylinder's own top, the only real zone
// that actually overlaps chamber material).
PASSAGE_TOP_MARGIN = 5;
PASSAGE_Y_CENTER = CYL_Y_CENTER;                                  // 305, UNCHANGED
cyl_top_z = CYL_Z_CENTER + CYL_R;                                 // 938mm (world Z) -- fire cylinder's own real top
PASSAGE_Z_TOP_WORLD = cyl_top_z - PASSAGE_TOP_MARGIN;             // 933mm (world Z)
PASSAGE_Z_CENTER_WORLD = PASSAGE_Z_TOP_WORLD - PASSAGE_R;         // 835.551mm (world Z)
...
// below) -- NOT an absolute world Z, same convention TRAP_BOT_H/TRAP_TOP_H
// used through v14.2. Converted here, once, not re-derived per use.
PASSAGE_Z_CENTER = PASSAGE_Z_CENTER_WORLD - chamber_floor_z;      // 64.216mm (local h)
// Real, confirmed chord: passage's own real bottom (world Z=738.102mm)
// sits 33.233mm below chamber_floor_z(771.335mm) -- ~17% of the
// 194.898mm diameter has zero real chamber material to cut (matches this
// file's own standing "no material below the floor" convention, not a
// defect, see file header for the full real CGAL containment result).
...
// fire_cylinder_end_cap_2d() / fire_cylinder_end_cap() — SAME real role
// as v14.2's own inner_duct_end_cap(): cylinder's own full real circle
// minus the SAME passage-hole shape TASK 3 cuts through the chamber wall
// (shared module, not re-derived) -- keeps the cylinder's interior
// actually connected to the chamber. Welds DIRECTLY to the chamber's own
// octagon end cap, NOT to the outer shell.
...
// outer_shell_flange_cut_2d() — v14.2 TASK 2, NEW. Real, confirmed defect
// found via an actual CGAL ray-probe (not a visual read): v14.1's TASK 1
// made the flange/end-cap a PLAIN, UNPERFORATED rectangle (correctly
// fixing the visible step) but never cut the passage hole through it --
// the flange's own footprint (580x428.6mm) is LARGER than the duct's own
// opening and sits directly in the line of sight through the passage, so
// even though the chamber's own rear wall and the inner duct's own end
// cap both have a real trapezoid hole, this solid flange fully blocks the
// view/airflow behind them (matches Janis's own "very large but not cut
// through"/"thin film" report -- the hole's own outline is visible via
// the OTHER two real cuts, but this one was never made). Same shared
// `firebox_passage_profile()` module, not re-derived or resized --
// cutting the SAME real shape through every layer it needs to pass
// keeps them all aligned by construction.
...
module outer_shell() {
    union() {
        ...
        // near-end flange, ADDITIVE 20mm, SOLID (no interior cavity), FLAT
        // full-height plane (v14.1 TASK 1 -- no octagon-clip zone, see
        // outer_shell_footprint_2d() comment above). Spans EXACTLY
...
// SAME flat full-height footprint as the flange it caps (v14.1 TASK 1 --
// no octagon-clip zone) -- STRUCTURAL, bears real load from the chamber's
// rear edge (Janis's explicit design intent). Positioned FLANGE_LEN(20mm)
// away in X from the inner duct's own start (firebox_x0) -- confirmed via
// real CGAL zero-contact check below that it never touches the inner duct
// or the duct's own end cap.
...
// ash_tray() — v15 REQUIRED real fix (R-009 consequence): the duct's own
// rectangular DUCT_W/DUCT_Y_CENTER (v14.2 basis) are retired along with
// the rectangular duct itself -- the tray now sits inside a ROUND fire
// cylinder, a rectangular tray's own safe width is governed by the
// circle's real chord width at the tray's own FARTHEST-from-center Z
// (the tray's own BOTTOM face, since the tray sits below the cylinder's
// vertical center -- NOT the top face, a real reasoning error caught and
// fixed via CGAL before finalizing, not assumed: checking only the top
// corner first suggested 171.6mm was safe, but the bottom face is 220mm
// from center vs the top's 208mm, so the bottom is the real binding
// constraint). Real check (see cc_chat_log.md): inner radius
// (CYL_D/2-wall_t=225mm) at ASH_TRAY_GAP(5mm, real clearance above the
// cylinder's own real inner-bottom surface, judgment call, flagged)
// leaves a real safe max width of 94.34mm at the tray's own BOTTOM
// corners (live-computed, the true binding constraint). ASH_TRAY_W=80mm
// chosen with real margin (~14.3mm total), confirmed via CGAL fully
// contained within fire_cylinder()'s own hollow interior, zero collision
// (the first CGAL pass at 150mm found a real, substantial 49,100mm³
// overlap with the cylinder's own solid wall -- fixed, re-verified
// empty). Length formula UNCHANGED, real value now 534mm (was 414mm,
// grows automatically with the longer FIREBOX_L, expected).
ASH_TRAY_H = 12;
ASH_TRAY_L = FIREBOX_L - 2*wall_t - 40;       // 534mm (v15, was 414mm)
ASH_TRAY_GAP = 5;                              // real clearance above the cylinder's own inner-bottom surface, judgment call, flagged
ASH_TRAY_W = 80;                               // v15, real chosen value with margin vs the 94.34mm safe max at the tray's own bottom (was 514mm under the old rectangular duct)
...
module ash_tray(ash_tray_out_pct = 0) {
    ...
    tray_z = CYL_Z_CENTER - (CYL_D/2 - wall_t) + ASH_TRAY_GAP;   // 490mm (world Z), real bottom
    translate([ASH_TRAY_X_IN + slide, CYL_Y_CENTER - ASH_TRAY_W/2, tray_z])
        difference() {
...
module firebox(firebox_door_open_deg = 0, ash_tray_out_pct = 0) {
    color("#8B2E2E") fire_cylinder();
    color("#8B2E2E") fire_cylinder_end_cap();

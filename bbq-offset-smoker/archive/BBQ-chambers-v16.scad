...
// outer_shell_flange_footprint_2d() — v16 TASK C REBUILT TWO-ZONE (Janis's
...
// cube fire box profile"). v14.1 flattened outer_shell_footprint_2d() to a
// single unclipped square (fixing a real visible width-jump STEP at the
// time, see the comment above this module, kept as real history) -- that
// flattening RETIRES here at the flange/end-cap call site specifically
// (R-009), NOT at outer_shell_footprint_2d() itself (still used plain/
// unclipped by fire_cylinder_partition_2d() at the DOOR end, firebox_x1 --
// there is no real chamber octagon anywhere near the door end to clip
// against, clipping there would be physically meaningless). This new
// module is the flange/end-cap's OWN footprint: h>=0 (world
// Z>=chamber_floor_z) clips FLUSH to the real octagon (deliberate, no
// margin -- a structural tuck-under weld, full real contact is the
// intent, Janis's own "it give stronger structure support"); h<0 keeps the
// native full square. Real, live-confirmed: for h>=163.665mm (world
// Z>=935mm) the octagon is already wider than the 580mm square everywhere
// across it, so this intersection is a no-op there too -- the two-zone
// taper is real but confined to world Z=[771.335,935], not the whole
// flange height. Combined with TASK A's real passage fix (real chamber
// material now sits behind wherever the passage hole is cut through this
// footprint), the "rat hole"/unsupported-opening defect Janis flagged
// (Image 2) should not recur -- re-verify visually below, not assumed.

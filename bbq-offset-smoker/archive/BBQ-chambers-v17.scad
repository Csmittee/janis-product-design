...
// fire_cylinder_end_cap_2d() / fire_cylinder_end_cap() — v16 TASK B
// REBUILT TWO-ZONE (Janis's own words: "the inner cylinder backend or
// endcap have the top part copy octagon frame of the chamber while the
// rest copy the circular shape of the cylinder"). For h>=0 (world
// Z>=chamber_floor_z, real chamber territory) the end cap's own boundary
// clips to true_octagon_profile(); for h<0 (below chamber_floor_z, the
// cylinder's own real center CYL_Z_CENTER=710mm sits well below the floor,
// no chamber material exists there to reference) it keeps its native full
// circle. Same shared passage-hole cut TASK A rebuilt (not re-derived).
// Welds DIRECTLY to the chamber's own octagon end cap, NOT to the outer
// shell (UNCHANGED role from v14.2/v15).
...
// chamber_hollow_cavity_2d() — v17 NEW REPLACEMENT for v17's own first-pass
// `chamber_wall_ring_or_open_below_2d()` attempt (RETIRED same round it was
// added, R-009 -- caught by re-rendering before shipping, not left in).
// That first attempt clipped the flange's own OUTER boundary down to the
// octagon's own true outer edge for h>=0 -- fixed the "wall inside the
// chamber" defect (below), but a real re-render found it created a NEW,
// separate real gap: the chamber has ZERO material of its own (neither
// wall nor interior) OUTSIDE the octagon's true boundary, so wherever the
// firebox's 580mm square is wider than the octagon at a given height (most
// of the flange's real height range, see `outer_shell_flange_footprint_2d()`
// below), clipping the SQUARE down to the narrower octagon left a real,
// literal hole with NO material from either part on either side of the
// seam -- exactly Janis's own "missing firebox outer shell end cap that
// should stretch to fuse with the chamber shell" finding (green arrow,
// screenshot), confirmed by re-rendering the isolated flange+chamber
// before finalizing, not assumed. FIX (this module): stop clipping the
// flange's own OUTER boundary at all -- keep it the FULL 580mm square,
// flush and continuous with the main outer-shell body on every side (same
// silhouette all the way through, zero notches, zero missing material) --
// and instead cut ONLY the chamber's own real HOLLOW INTERIOR shape
// (`offset(delta=-wall_t) true_octagon_profile()`, the SAME real wall_t
// this file already uses) out of it, so the flange can never plug the
// chamber's own real bore (the ACTUAL defect Janis's "i didnt ask for
// this wall!" flagged) without also carving a fake notch into its own
// visible exterior. This shape only exists for h>=0 (true_octagon_profile()
// has no geometry below h=0), so h<0 is automatically left fully solid/
// untouched by this cut, no separate "or open below" padding trick needed
// for a difference() (unlike the earlier intersection()-based attempt).
module chamber_hollow_cavity_2d() {
    offset(delta = -wall_t) true_octagon_profile();
}
// outer_shell_flange_footprint_2d() — v16 TASK C ORIGINAL INTENT (Janis's
// own words: "the fire box shell end cap that tuck under the chamber have
// the top zone copy octagon shape while the bottom zone copy the square
// cube fire box profile so there should never be a visible hole") -- v17
// REBUILT to actually deliver the STATED GOAL (never a visible hole) via a
// different, real mechanism than the literally-proposed one, after
// confirming via CGAL+re-render that the literal "clip the square down to
// the octagon" approach creates exactly the hole it's meant to prevent
// (see `chamber_hollow_cavity_2d()` above for the full real finding).
// Real result: the flange's own outer silhouette is now the FULL 580mm
// square for its ENTIRE real height (both h>=0 and h<0 -- no two-zone
// step at chamber_floor_z either, so v14's own original visible-step
// defect can't recur here now, a real bonus not the goal), continuous and
// flush with the main outer-shell body -- with a hole cut through it
// matching the chamber's own real hollow bore (h>=0 only) so it can never
// block the chamber's own interior. Real CGAL (re-verified below, not
// assumed): EMPTY vs the chamber's own hollow cavity (2mm real margin,
// not just an exact touch), NON-EMPTY vs the chamber's own real wall
// material (genuine structural weld contact), and the flange's own outer
// boundary confirmed IDENTICAL to `outer_shell_footprint_2d()`'s own
// (zero notch, by construction).
...
// partition()`/`ash_tray()`/`firebox_door()` UNCHANGED, no new toggle
// (not part of Janis's own explicit ask this round, R-009: keep scope to
// what was asked).

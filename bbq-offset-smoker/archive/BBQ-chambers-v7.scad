...
// PARAMETERS — all mm
// ───────────────────────────────
GRATE_Z          = 750;                  // v6: REPOSITIONED (was 700) -- aligned to the lid parting line
                                          // (chamber_floor_z + chamfer = 600+150 = 750, bottom of the lid's
                                          // own vertical wall panel), per Janis's explicit spec. Still the
                                          // MASTER CONTROL VALUE the chain runs from -- not re-derived from
                                          // chamber_floor_z, just re-VALUED to the correct real feature line.
grate_clearance  = 150;                  // v6: 100->150 in lockstep with GRATE_Z, so chamber_floor_z
                                          // (750-150=600) stays EXACTLY UNCHANGED -- only the grate's own
                                          // seat height moves, nothing else in the chain shifts.
wall_t           = 3;
...
// ───────────────────────────────
DATUM_GRATE_Z    = GRATE_Z;                                   // 750 (v6, was 700)
...
// v7: THIS is the exact profile that caused "reads solid, not hollow" --
// extruding this SOLID (fake-diagonal-closed) shape the FULL chamber_L
// length turned the diagonal into a real spanning panel. `chamber_outer_tube()`/
// `chamber_inner_cavity()` NO LONGER call this module (see
// `true_octagon_profile()`/`fixed_side_wedge()` below) -- confirmed via a
// full-file grep (R-009) that this module has exactly ONE real remaining
// caller, `firebox_passage_profile()` (via `intersection()`), which is an
// explicit separate DO-NOT-TOUCH open item -- KEPT UNCHANGED for that
// caller only, not deleted.
...
// v7 TASK 2 REBUILD: chamber_outer_tube()/chamber_inner_cavity() no longer
// use fixed_shell_profile() (see that module's own header, v7 note) --
// rebuilt from true_octagon_profile() + fixed_side_wedge() instead, per
// the CONTEXT root-cause fix (build the real hollow ring FIRST, clip to
// the fixed side SECOND -- never bake the cut into the profile itself).
// ───────────────────────────────
// chamber_inner_cavity() — v7 REBUILT: same outer-minus-inset-inner
// pattern as before (real solid end caps at [wall_t, chamber_L-wall_t],
// v2 TASK 1 closure fix preserved), just sourced from the TRUE octagon
// now instead of fixed_shell_profile(). Used internally by
// chamber_outer_tube() below as the "inner" solid to hollow out the ring
// with -- no longer subtracted a second time from chamber_shell() (that
// would be a redundant no-op now that the ring is already hollow).
...
// chamber_outer_tube() — v7 REBUILT per TASK 2: (1) full wall_t hollow
// ring of true_octagon_profile() (outer solid minus chamber_inner_cavity(),
// same X-range as before, real solid end caps), THEN (2) intersection()
// with fixed_side_wedge() (also extruded across the same chamber_L range)
// to keep only the fixed (non-lid) side of the real apex-to-ridge-midpoint
// cut line. Step (1) before step (2) is the whole fix -- clipping a
// SOLID profile (old behavior) leaves a spanning panel at the cut; clipping
// an already-HOLLOW ring leaves only the wall's own true cross-section.
module chamber_outer_tube() {
    intersection() {
        difference() {
            translate([0, 0, chamber_floor_z]) rotate([0, 90, 0])
                linear_extrude(height = chamber_L) true_octagon_profile();
...
// lid_territory_end_caps() — v6 REBUILT (v5's own version was a SOLID
// fill using lid_profile() -- a real design gap in the v5 fix itself, see
// file header Finding 1: a solid opaque-looking block sitting exactly in
// an open-lid viewer's sightline reads as "solid mass" even though the
// rest of the shell is correctly hollow).
// chamber_outer_tube()/chamber_inner_cavity() ALREADY cover the FIXED-
// side (fixed_shell_profile()) continuously across the FULL chamber_L --
// that was never the gap. The ONLY thing missing at the 2 end zones
// (X=[DATUM_X_FRONT,LID_X0], X=[LID_X1,DATUM_X_REAR]) is the LID-SIDE
// profile material lid() itself would normally supply -- so this module
// adds ONLY that, as a proper wall_t-thick HOLLOW shell (same
// outer-minus-inset-inner construction chamber_outer_tube()/
// chamber_inner_cavity() already use), using lid_profile() itself so the
// shell's own exterior traces the real octagon edges exactly, matching
// the closed lid's own outer surface where they meet at X=LID_X0/LID_X1.
// Solid only for the true wall_t-thick end cap at the outermost tip
// (X=DATUM_X_FRONT or X=DATUM_X_REAR), same end-cap pattern
// chamber_inner_cavity() already uses.
// NO overlap past LID_X0/LID_X1 into the lid's own kinetic X-range --
// confirmed deliberately: rotate() only moves material within the Y-Z
// cross-section, never across X, so as long as this fixed end-zone's own
// X-range stays strictly disjoint from lid()'s X-range (exactly true
// here, touching at most), the open/swinging lid can never intersect it
// at any angle -- verified via a full CGAL kinetic sweep after this
// change, not assumed.
module lid_territory_end_caps() {
    if (LID_X0 > DATUM_X_FRONT)
        difference() {
            translate([DATUM_X_FRONT, 0, chamber_floor_z]) rotate([0, 90, 0])
                linear_extrude(height = LID_X0 - DATUM_X_FRONT) lid_profile();
            translate([DATUM_X_FRONT + wall_t, 0, chamber_floor_z]) rotate([0, 90, 0])
                linear_extrude(height = (LID_X0 - DATUM_X_FRONT) - wall_t, convexity = 4)
                    offset(delta = -wall_t) lid_profile();
        }
    if (DATUM_X_REAR > LID_X1)
        difference() {
            translate([LID_X1, 0, chamber_floor_z]) rotate([0, 90, 0])
                linear_extrude(height = DATUM_X_REAR - LID_X1) lid_profile();
            translate([LID_X1, 0, chamber_floor_z]) rotate([0, 90, 0])
                linear_extrude(height = (DATUM_X_REAR - LID_X1) - wall_t, convexity = 4)
                    offset(delta = -wall_t) lid_profile();
        }
}
...
module chamber_shell() {
    // v7: chamber_inner_cavity() subtraction REMOVED here -- chamber_outer_tube()
    // is already hollow internally (TASK 2 rebuild), subtracting it again
    // would be a redundant no-op. firebox_passage()/exhaust_room_opening()
    // re-applied exactly as before, unchanged (TASK 2 constraint).
    difference() {
        union() {
            chamber_outer_tube();

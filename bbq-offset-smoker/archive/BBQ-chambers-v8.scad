...
// Exhaust room datums — v8 TASK 4: RECOMPUTED per Claude Web's prior max-
// flush decision (room stays flush against the flat wall, never extends
// into the chamfer) -- was centered in the trough_h band
// (chamber_floor_z + (trough_h-ROOM_H)/2 = 705/805 under the old chamfer).
// New formula puts ROOM_TOP_Z exactly at the wall/chamfer boundary --
// confirmed: chamber_floor_z+trough_h = 852.670, matches ROOM_TOP_Z exactly.
ROOM_BASE_Z      = chamber_floor_z + trough_h - ROOM_H;         // 752.670mm (v8, was 705)
ROOM_TOP_Z       = ROOM_BASE_Z + ROOM_H;                         // 852.670mm (v8, was 805) -- exactly the wall/chamfer boundary
...
// NO LONGER calls this module (see `true_octagon_profile()`/`fixed_side_wedge()`
// below) -- confirmed via a full-file grep (R-009, re-confirmed v8) that
// this module has exactly ONE real remaining caller, `firebox_passage_profile()`
// (via `intersection()`), which is an explicit separate DO-NOT-TOUCH open
// item -- KEPT UNCHANGED for that caller only, not deleted. (v8:
// `chamber_inner_cavity()`, the OTHER v7 caller this note originally
// referred to, is itself now retired as a standalone module -- see
// `octagon_ring()`.)

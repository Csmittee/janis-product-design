...
// Version: v10
// Date: 2026-07-25
// Source: BBQ-offset-smoker-base-v9.scad
// Changes: direct-cc, Janis's own 3-step counterbalance-branch spec --
// CB1/counterbalance/stopper, deferred since v6, finally rebuilt. Full
// detail: docs/lid-hinge-counterbalance-calc.md Section 16. Summary:
//
// 1. CB1 PIPE PLACEMENT (Janis's step 1 -- "place the counter balance
//    pipe... it may fly in the air above DE surface... make sure aligned
//    flat parallel to face DE"): UNCHANGED formula from v6 -- CB1_OPEN
//    was already built this way (D + CB1_EDGE_DIST along DE_DIR, +
//    CB1_STANDOFF along DE_NORM, i.e. parallel to DE, standing off it by
//    a real air gap) -- confirmed correct against the new pivot, not
//    re-derived (CB1_EDGE_DIST/CB1_MASS_KG remain LOCKED, physics
//    recompute still explicitly deferred).
// 2. LINK REBUILT (Janis's step 2 -- "trace the CD, DE form to reach
//    this balancer pipe, becareful on apex D... let the link fly above
//    the ridge CD, DE around 25mm to be safe"): the old v7-era single
//    "bow waypoint" (push the naive path's closest-approach point 50mm
//    away from D -- explicitly flagged EVERY round since v7.2 as NOT
//    achieving a real verified margin) is RETIRED. Replaced with an
//    exact offset-polygon construction: `RIDGE_HOVER` traces the CD
//    ridge at a real, constant standoff; `D_HOVER` rounds the real
//    convex corner at apex D via the SAME correct `miter_point()`
//    formula already used at apex B, giving the true point equidistant
//    from BOTH the CD and DE faces at once (no arc/sweep needed). REAL,
//    SIGNIFICANT SIMPLIFICATION made possible by v9's shared pivot: apex
//    D belongs to the LID (Y > RIDGE_SPLIT_Y), which now rotates about
//    the exact same center as the rib -- so D's position relative to the
//    rib is CONSTANT at every door_open_deg, and this is a single
//    native-frame check, not an angle-dependent sweep. Verified in
//    Python before writing this file (per R-014): net real solid
//    clearance to apex D is exactly 25.000mm (centerline offset 45mm =
//    Janis's 25mm + the blade's own 20mm minimum half-width), matching
//    Janis's own number AND clearing rules-bbq-fab.md's own 20mm
//    apex-clearance floor with a real 5mm margin -- not asserted, the
//    exact number.
// 3. PRONG/STOPPER (Janis's step 3 -- "create the prong or wrench hand
//    to grab this balancer... let the side act as the stopper... touch
//    the DE face"): UNCHANGED from v6 -- `cb1_wrap_cut_2d()`'s own
//    half-wrap (the "index finger", grips CB1) plus the RIB_SPINE's own
//    final segment (CB1_CLOSED -> CONTACT_CLOSED, the "thumb", reaching
//    further to a real literal contact point ON the DE face) already
//    matches this description exactly -- confirmed against the new link
//    path, not rebuilt.
//
// *** REAL ISSUE FOUND, NOT YET FIXED, FLAGGED NOT HIDDEN *** -- Janis's
// own real OpenSCAD screenshot of v9 at an open angle shows the ribs'
// handle-side V-shapes floating visibly above the real open door panel,
// not tracing it. Root cause, found this round: apex B/C (and the real
// parting-line split point) are on the FIXED side of RIDGE_SPLIT_Y (Y<
// 242.665) -- they are NOT part of the lid panel at all. But the rib's
// handle-side spine (RIB_SPLIT_OFFSET/RIB_B_OFFSET) was built FROM those
// fixed-side points, even though the ENTIRE rib is one rigid body that
// rotates WITH the lid (`lid_rib_assembly()`). At door_open_deg=0 this
// coincidentally looks right (fixed and lid geometry coincide when
// closed), but past 0 the rib swings away from B/C with the lid while
// the real fixed structure stays put AND the real lid panel never
// extended back that far to begin with -- so that whole arm ends up
// tracing NOTHING real once open, exactly the floating gap in Janis's
// own screenshot. This is a DIFFERENT, unresolved problem from the CB1/
// apex-D fix above (D is lid-side, so the shared-pivot single-check
// applies there; B/C are fixed-side, so the handle arm's real clearance
// requirement is an angle-dependent sweep against the FIXED structure,
// not a native-frame trace of a surface that doesn't exist there once
// open). NOT fixed in this commit -- Janis's own explicit direction
// requested before touching this code again (see cc_chat_log.md/this
// round's chat for the live discussion). CB1/D-side geometry above (1-3)
// IS real and independently correct regardless of how this gets resolved.
//
...
// ─── v9 REAL REBUILD -- "get back the original rib" (Janis's own
// explicit instruction, this file's own header). With the door and rib
// now sharing ONE real rotation center (`HINGE_PIVOT_Y`/`HINGE_PIVOT_Z`,
// BBQ-chambers-v26.scad), their relative geometry is identical at every
// angle by construction -- the whole class of sweep-drift sink/float
// bugs v8's later passes fought (miter formulas, multi-angle sweeps,
// ridge-flat detour waypoints) no longer applies, so this round reverts
// to v6.1's own original, simple single-normal standoff technique,
// checked once at the closed state (not a full sweep -- there is no
// longer a sweep-dependent risk to check).
// REAL, ELEGANT CONSEQUENCE found by direct calculation before touching
// this code (confirmed to Janis, not assumed): apex B, apex C, and the
// new pivot are EXACTLY collinear -- `HINGE_PIVOT_Y`-chamfer = 64 =
// `HINGE_PIVOT_Z`-`DATUM_Z_RIDGE`, matching the B-C wall's own real 45°
// slope exactly (chamfer construction on a regular octagon is inherently
// 45°). The door-side arm is therefore ONE straight run from B all the
// way to the pivot -- apex C sits ON that line, needing no separate
// corner treatment of its own; the only REAL corner left is at B
// (between the vertical AB wall and this 45° diagonal). ───
...
BC_NORM = [-BC_DIR[1], BC_DIR[0]];                  // (-0.7071,0.7071), outward -- confirmed points away from chamber interior, SAME direction as B->pivot (collinear)
// miter_point(V,n1,n2,d) -- the correct 2D polygon-offset miter formula
// (kept from v8's own 5th-pass real fix, not the earlier wrong "average
// direction, flat push" technique) -- used once, at apex B, the one real
// remaining corner. miter_point = V + d*(n1+n2)/(1+n1.n2), the point
// perpendicular distance d from BOTH adjacent walls simultaneously.
function miter_point(V,n1,n2,d) = let(dot=n1[0]*n2[0]+n1[1]*n2[1], k=d/(1+dot))
    [V[0]+k*(n1[0]+n2[0]), V[1]+k*(n1[1]+n2[1])];

MIN_HALF_W = 20;   // mm -- 40mm minimum blade width (Section 5), moved up from its old spot below RIB_SPINE so the CB-link block just below can read it live (R-009: not duplicated as a second literal)

SPLIT_STANDOFF = 15;                                // mm -- back to v6.1's own original value
B_STANDOFF     = 20;                                // mm -- real miter offset at the one remaining corner (AB wall + the B-pivot diagonal)
...
// ─── v10 REAL REBUILD -- CB-side link, Janis's own step 2 ("trace the
// CD, DE form to reach this balancer pipe, becareful on apex D... let
// the link fly above the ridge CD, DE around 25mm to be safe"). Retires
// the old v7-era single "bow waypoint" (push the closest-approach point
// 50mm away from D -- flagged every round since v7.2 as NOT achieving a
// real verified margin). Real exact offset-polygon construction instead:
// `RIDGE_HOVER` traces the CD ridge at a real constant standoff;
// `D_HOVER` rounds the true convex corner at D via the SAME
// `miter_point()` formula used at apex B above -- the point
// simultaneously offset by the SAME distance from BOTH the CD and DE
// faces at once, no arc/sweep needed. LINK_CLEARANCE(25mm) is the REAL
// material-to-surface clearance Janis asked for (matching how CB1's own
// pipe "flies... above the DE surface" -- a real physical standoff, not
// an abstract centerline distance) -- since the blade itself has a real
// half-width at these points (`MIN_HALF_W`, moved up above so it can be
// read live here, R-009 checked), the CENTERLINE must be offset
// LINK_CLEARANCE + MIN_HALF_W = 45mm for the SOLID surface to clear by
// the real 25mm Janis asked for. CD_NORM is the ridge's own real outward normal
// (the ridge is horizontal, chamber interior below -- outward is +Z).
// Verified in Python before writing this file (per R-014): net real
// solid clearance to apex D and to both the CD/DE surfaces is exactly
// 25.000mm along the tightest segment -- clears rules-bbq-fab.md's own
// 20mm apex-clearance floor with a real 5mm margin. Elegant simplifi-
// cation made possible by v9's shared pivot: apex D belongs to the LID
// (Y > RIDGE_SPLIT_Y), which now rotates about the exact same center as
// the rib, so this is a single native-frame check, not an angle-swept
// one -- checked once at the closed state. ───
CD_NORM        = [0, 1];                            // ridge (CD face) outward normal -- horizontal face, interior below, outward is +Z
LINK_CLEARANCE = 25;                                 // mm -- real material-to-CD/DE-surface clearance, Janis's own explicit number this round
LINK_OFFSET    = LINK_CLEARANCE + MIN_HALF_W;        // 45mm -- centerline offset needed to realize a real 25mm solid clearance (MIN_HALF_W read live, not duplicated)

RIDGE_HOVER = [FC_Y, DATUM_Z_RIDGE + LINK_OFFSET];               // hovers above the ridge, directly below the pivot's own Y
D_HOVER     = miter_point(RIB_REF_D, CD_NORM, DE_NORM, LINK_OFFSET);  // rounds the real corner at D, 45mm (centerline) from BOTH faces at once

// ─── Full rib spine: handle -> split-point -> B -> pivot (apex C sits
// ON this same straight line, a pass-through tapering point, not a
// separate waypoint) -> ridge-hover -> D-hover -> CB1 -> prong tip.
// "dont let the rib exert from its door length any sharp turn, fill it
// with some nice spline or arc" (Janis's own explicit spec): the
// hull-of-circles ("capsule chain") technique already fillets every
// consecutive pair of spine points smoothly (reused directly, this
// project's own standing convention) -- with apex C's own corner
// eliminated by the collinear geometry above, the only real direction
// changes left on this whole arm are at B and at the new D-hover point,
// both naturally rounded by the hull-of-circles. ───
RIB_SPINE = concat(
    [[HANDLE_Y,HANDLE_Z], RIB_SPLIT_OFFSET, RIB_B_OFFSET, [FC_Y,FC_Z]],
    [RIDGE_HOVER, D_HOVER],
...
// bore, this project's own standing convention). v9: dist FC-D is now
// 199.23mm (the pivot's own real new position, this file's own header)
// -- net pad clearance ~170.7mm (199.23 - 28.5), still comfortably
// clear. The CB1 BRANCH's own clearance to D is v10's own real,
// resolved concern -- see RIDGE_HOVER_HALF_W/D_HOVER_HALF_W below.
...
RIDGE_HOVER_HALF_W = MIN_HALF_W;         // 20mm -- matches the LINK_OFFSET derivation above (LINK_CLEARANCE + this same MIN_HALF_W)
D_HOVER_HALF_W     = MIN_HALF_W;         // 20mm -- same

RIB_SPINE_R = concat(
    [HANDLE_WRAP_R, WELD_HALF_W_SPLIT, WELD_HALF_W_B, AXLE_HALF_W],
    [RIDGE_HOVER_HALF_W, D_HOVER_HALF_W],

...
// Version: v15
// Date: 2026-07-30
// Source: BBQ-offset-smoker-base-v14.scad
//
...
// stale narrative.
//
// v15 real changes from v14 (Janis's own detailed hardware spec, after
// reviewing a v14 render): (1) `TRAY_MOUNT_GAP` — the tray plate's own
// inner edge was still flush with the pivot line, overlapping the
// (already-widened) hinge block by ~16mm, so no real gap ever existed
// for the folded link to hide behind the tray — fixed, the plate now
// starts flush with the hinge block's own outer tip. (2) the folding
// link is rebuilt as a real flat steel strip (15mm wide, was a round
// rod), with a genuine ~10mm lap overlap at the middle pivot (link
// length is no longer simply half the ts-to-tt span), a U-shaped clevis
// + pin at BOTH ts and tt (not just implied), and `tt` pulled clear of
// the tray's own underside by a real clearance so it no longer visibly
// penetrates the tray surface. `ts` remains a conceptual slider along
// the apex-A/al face (Janis's own call, no rail modeled). Re-verified
// no collision between the link and the tray plate; Simple:yes at
// 0/45/90° door + -90/0° tray angles; interference sweep vs
// front_wheel_support() still empty.
//
...
module tray_hinges(x0) {
    ...
    // real geometry, not just a length -- shown at its deployed (0°)
    // reference position, see this file's own note further down for why
    // it isn't (and can't be, as a simple fixed-length rigid link) shown
    // following the tray through the full stow sweep
    tray_link(x0 + HINGE_OFFSET);
    tray_link(x0 + TRAY_L - HINGE_OFFSET);
}
...
TRAY_LINK_TS      = TRAY_AL;                                            // "ts" = al itself, the real fixed anchor. Conceptually a SLIDER along the apex-A/al plane (Y=0), per Janis's own explicit call -- no rail modeled, real hardware handles this the same way a real slotted stay does.
...
TRAY_LINK_TT      = [TRAY_LINK_TT_RAW[0], TRAY_LINK_TT_RAW[1] - TRAY_LINK_CLEARANCE];   // real "tt" pin location, confirmed clear of the tray surface
...
TRAY_LINK_SPAN    = norm(TRAY_LINK_TT - TRAY_LINK_TS);                  // real ts-to-tt distance (post-clearance)
TRAY_LINK_HALF_LEN = (TRAY_LINK_SPAN + TRAY_LINK_OVERLAP) / 2;          // each link's own real length, including its half of the lap overlap
TRAY_LINK_DIR     = (TRAY_LINK_TT - TRAY_LINK_TS) / TRAY_LINK_SPAN;
TRAY_LINK_LEN     = TRAY_LINK_SPAN;                                     // real reference length (ts-to-tt working span), see docs/tray-relocation-bracket.md for the current live number

// REAL, DISCLOSED FINDING, 2026-07-30 (checked in Python before
// modeling, not assumed): a genuine 2-bar pin-jointed link of FIXED
// total length can only close the gap between the fixed `ts` and the
// tray's own `tt` at angles where their real distance is <=
// TRAY_LINK_SPAN. Checked across the full stow sweep (tray0_angle_deg
// -90..0) via real circle-circle intersection: the ts-to-tt distance is
// exactly TRAY_LINK_SPAN at deployed (0°, fully straight) but GROWS well
// past it as the tray folds toward stowed (-90°) -- `ts` sits low (near
// apex H, well below the tray's own hinge Z), so `tt` swings AWAY from
// it, not toward it, as the tray folds up. A fixed-length rigid link
// genuinely cannot follow the tray through the full stow range from
// this anchor -- matches why real hardware for this exact job commonly
// uses a SLOTTED pivot at one end (Janis's own call, confirmed: `ts`
// slides along the apex-A/al plane instead of staying pinned to one
// point). Modeled here at the DEPLOYED (0°) reference position only.
TRAY_LINK_END1 = TRAY_LINK_TS + TRAY_LINK_DIR * TRAY_LINK_HALF_LEN;                 // link 1: ts -> past the midpoint by half the overlap
TRAY_LINK_END2 = TRAY_LINK_TT - TRAY_LINK_DIR * TRAY_LINK_HALF_LEN;                 // link 2: tt -> past the midpoint the other way
TRAY_LINK_MID  = TRAY_LINK_TS + TRAY_LINK_DIR * (TRAY_LINK_SPAN / 2);               // pivot pin, centered in the real overlap region
...
module tray_link(x_center) {
    flat_link(TRAY_LINK_TS, TRAY_LINK_END1, x_center);
    flat_link(TRAY_LINK_TT, TRAY_LINK_END2, x_center);
    hinge_u(TRAY_LINK_TS, x_center);
    hinge_u(TRAY_LINK_TT, x_center);
    translate([x_center,TRAY_LINK_MID[0],TRAY_LINK_MID[1]]) rotate([0,90,0])

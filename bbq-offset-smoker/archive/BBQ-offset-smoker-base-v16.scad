...
// Version: v16
// Date: 2026-07-30
// Source: BBQ-offset-smoker-base-v15.scad
//
...
// stale narrative.
//
// v16 real changes from v15, Janis's own direct catch after reviewing
// the v15 render: "when i turn tray degree, look like the link doesnt
// link to the the degree turn yet, you got to wire it, thats it!" The
// folding link (`tray_link()`) was real geometry but STATIC — always
// drawn at its deployed (0°) reference position regardless of the
// tray's own angle_deg. Fixed: `tt` (rigidly part of the tray) now
// rotates WITH the tray about its own real pivot; `ts` (the slider) now
// re-solves every call, via a real circle/line intersection, as the
// point on the fixed apex-A/al line (Y=0) that keeps the link's own
// real span constant at every angle — picking the physically-valid
// "lower" root (checked in Python first: this root stays on the real
// [TRAY_AL.z, RIB_REF_A.z] segment across the full -90..0° sweep; the
// other root is off the segment entirely and is discarded). Wired for
// BOTH tray0 and tray1 (each keeps its own independent angle). Moved the
// `tray_link()` calls out of `tray_hinges()` (which has no angle) and
// into `tray(x0, angle_deg)` directly.
//
// Re-verified via a real isolated intersection() probe (link vs. tray
// plate, both trays, not assumed clean from the code alone): Simple:yes
// holds at every swept angle. EMPTY (no collision) across the whole
// normal operating range, 0° down to about -82°. NEW, DISCLOSED FINDING
// past that point, NOT fixed here, flagged for Janis's own decision (see
// docs/tray-relocation-bracket.md "v16 wiring" section for the full
// numbers): from about -83° to -90° (last ~7-8° approaching full
// vertical stow) the link's own real 15mm-wide flat strip genuinely
// clips the folded tray plate's near edge — root cause is geometric, not
// a wiring bug: at full -90° stow the folded plate's own near face
// always lands exactly at Y=-HINGE_PIVOT_OFFSET+TRAY_T (real rotation
// math, independent of any other constant), leaving only ~3mm clear to
// the fixed Y=0 slider line the link's own hardware has to thread
// through — too tight for a flat 15mm strip with no real slot/rail
// modeled (tried widening HINGE_PIVOT_OFFSET as a fix; made the overlap
// WORSE at the same angle, since it shifts both the plate AND the
// rotating `tt` pin together, not a simple 1-parameter fix). Confirmed
// the same threshold on BOTH tray0 and tray1 (symmetric, not a
// per-tray bug).
//
...
module tray(x0, angle_deg) {
    tray_hinges(x0);
    // folding link -- v16 REAL FIX, Janis's own direct catch after
    // reviewing the v15 render: "when i turn tray degree, look like the
    // link doesnt link to the the degree turn yet, you got to wire it."
    // The link's `tt` pin is rigidly part of the tray (bolted to its
    // underside), so it now rotates WITH the tray about the tray's own
    // real pivot (Y=-HINGE_PIVOT_OFFSET, Z=HINGE_Z) instead of staying
    // frozen at the deployed (0°) reference position. `ts` re-solves
    // every call as the point on the fixed apex-A/al slider line (Y=0)
    // that keeps the link's own real span constant -- a real circle/line
    // intersection, not an approximation -- see tray_link_ts() below.
...
// step 5: folding link -- al (== ts, the real fixed anchor now pinned to
// apex H's own Z) is the anchor, NOT a point merely close to it --
// project the 45° line the OTHER way, FROM al back up to the tray's own
// underside (Z=HINGE_Z), to find "tt" instead.
TRAY_LINK_TS_REF  = TRAY_AL;                                            // "ts" reference at deployed (0°) -- al itself. Real SLIDER along the apex-A/al plane (Y=0), per Janis's own explicit call -- no rail modeled, real hardware handles this the same way a real slotted stay does.
...
TRAY_LINK_TT_REF  = [TRAY_LINK_TT_RAW[0], TRAY_LINK_TT_RAW[1] - TRAY_LINK_CLEARANCE];   // "tt" pin location AT DEPLOYED (0°), rigidly fixed to the tray -- rotates with it at other angles, see tray_link_tt() below
TRAY_LINK_W       = 15;   // mm, flat steel strip width -- Janis's own explicit spec (was a round rod, corrected)
TRAY_LINK_T       = 3;    // mm, real judgment call -- strip thickness, no exact number given
TRAY_LINK_OVERLAP = 10;   // mm, real judgment call -- Janis's own explicit spec: the 2 links must overlap (a real lap joint) at the middle pivot, not just meet at a point, so a total link length equal to ts-to-tt is NOT correct -- each half is longer by half the overlap
TRAY_LINK_SPAN    = norm(TRAY_LINK_TT_REF - TRAY_LINK_TS_REF);          // real, CONSTANT ts-to-tt distance (post-clearance) -- the link's own fixed physical length, true at every angle since `ts` re-solves to keep it so, see below
TRAY_LINK_HALF_LEN = (TRAY_LINK_SPAN + TRAY_LINK_OVERLAP) / 2;          // each link's own real length, including its half of the lap overlap
function perp2(v) = [-v[1], v[0]];

// v16 REAL FIX, 2026-07-30, Janis's own direct catch after reviewing the
// v15 render: "when i turn tray degree, look like the link doesnt link
// to the the degree turn yet, you got to wire it." The v15 build only
// ever drew tray_link() at its deployed (0°) reference position,
// regardless of the tray's own real angle_deg -- this section makes it
// genuinely angle-dependent, for BOTH tray0 and tray1.
//
// `tt` is rigidly part of the tray (its own real pin, bolted to the
// tray's underside) -- so at angle A it's just TRAY_LINK_TT_REF rotated
// about the tray's own real pivot (Y=-HINGE_PIVOT_OFFSET, Z=HINGE_Z),
// same rotation the tray plate itself gets in tray() below. Verified in
// OpenSCAD's own convention (checked live, not assumed):
// rotate([a,0,0]) maps a point's local (y,z) to
// (y*cos(a)-z*sin(a), y*sin(a)+z*cos(a)).
function tray_link_rot(p, angle_deg) =
    let(rel = p - [-HINGE_PIVOT_OFFSET, HINGE_Z], c = cos(angle_deg), s = sin(angle_deg))
    [rel[0]*c - rel[1]*s, rel[0]*s + rel[1]*c] + [-HINGE_PIVOT_OFFSET, HINGE_Z];
function tray_link_tt(angle_deg) = tray_link_rot(TRAY_LINK_TT_REF, angle_deg);

// `ts` is NOT rigid with the tray -- it's the real slider, free to move
// along the fixed apex-A/al line (Y=0, Z between TRAY_AL.z and
// RIB_REF_A.z). For any rotated `tt`, `ts` is the point on that line a
// real, constant TRAY_LINK_SPAN away -- a genuine circle(center=tt,
// r=TRAY_LINK_SPAN) vs line(Y=0) intersection, not an approximation.
// Two roots exist; DISCLOSED FINDING, checked in Python before writing
// this (not assumed): swept the full stow range (tray angle 0..-90°)
// and confirmed the LOWER root (tz - h) stays on the real physical
// segment the whole way (671.335mm at 0° -- exactly matching the old
// fixed `al`, confirming this formula is correct at the reference
// config -- rising smoothly to ~773.64mm at -90°), while the upper root
// (969-1215mm) sits above apex A entirely, off the real segment, and
// must be discarded.
function tray_link_ts(angle_deg) =
    let(tt = tray_link_tt(angle_deg), ty = tt[0], tz = tt[1],
        h = sqrt(TRAY_LINK_SPAN*TRAY_LINK_SPAN - ty*ty))
    [0, tz - h];
...
module tray_link(x_center, angle_deg) {
    ts  = tray_link_ts(angle_deg);
    tt  = tray_link_tt(angle_deg);
    dir = (tt - ts) / TRAY_LINK_SPAN;
    end1 = ts + dir * TRAY_LINK_HALF_LEN;
    end2 = tt - dir * TRAY_LINK_HALF_LEN;
    mid  = ts + dir * (TRAY_LINK_SPAN / 2);
    flat_link(ts, end1, x_center);
    flat_link(tt, end2, x_center);
    hinge_u(ts, x_center);
    hinge_u(tt, x_center);
    translate([x_center,mid[0],mid[1]]) rotate([0,90,0])
        cylinder(h=TRAY_LINK_T*2+12, r=TRAY_HINGE_PIN_R, center=true, $fn=16);
}

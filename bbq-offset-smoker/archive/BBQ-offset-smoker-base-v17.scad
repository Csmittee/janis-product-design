...
// Version: v17
// Date: 2026-07-31
// Source: BBQ-offset-smoker-base-v16.scad
//
...
// stale narrative.
//
// v17 real changes from v16, both from Janis's own direct review of the
// v16 render:
//
// (1) FOLD DIRECTION FIXED, LOCKED, NEVER CHANGE: v16's angle_deg
// convention (-90=stowed) actually swung the tray tip UP past apex A,
// confirmed wrong via a real colored render (v16's render had washed-out
// colors that hid this at first). Janis's own locked spec: default/
// stowed is tip-toward-ground (vertical); "activating" (deploying) is a
// CLOCKWISE rotation about X viewed from +X, ending flat/horizontal.
// Fixed convention: angle_deg 0=deployed (unchanged) .. +90=stowed
// (tip down). Verified via local render at 0/45/90° before writing this.
//
// (2) LINK KINEMATICS REBUILT under the corrected direction, following
// a real taught 2-bar-link exercise (triangle a/b/c, checked against a
// labeled simulation before writing any of this): `ts` reverts to
// Janis's own ORIGINAL spec — a TRUE FIXED anchor at `al`, no slider —
// since the fixed-ts-to-rotating-tt distance now genuinely SHRINKS as
// the tray folds (v16's slider was a workaround for the wrong fold
// direction, not a real requirement). The give is now in the link's own
// middle lap-joint (`lc`), which folds via a real circle/circle
// intersection, picking the root closer to apex A (matches Janis's own
// description: `lc` travels toward apex A as the tray folds). The 2
// link halves are also now split in DEPTH (X), not left coplanar —
// checked via a real rectangle-overlap simulation that 2 same-plane
// rigid bars sharing one pivot cannot close without interpenetrating,
// same reason real hinges/scissors stack in depth — combined stack held
// to a real 10mm total (`TRAY_LINK_STACK_T`, Janis's own spec: the 2
// halves nest/wrap into each other, not additive).
//
// Re-verified via a real isolated intersection() probe (link vs. tray
// plate, chamber/firebox/exhaust geometry suppressed so the probe is
// unambiguous): EMPTY (no collision) from 0° to about 86° — the entire
// normal operating range. DISCLOSED, NOT FULLY RESOLVED (Janis's own
// call, this link isn't critical, math doesn't need to be perfect): a
// real, small collision remains from about 87° to 90°, where `ts`/`tt`
// nearly coincide (11mm apart vs. each link half being ~115mm long),
// making the circle intersection ill-conditioned — tried multiple
// root-selection variants, the instability is inherent to this
...
// give the folding link (`tray_link()`, real rod OD 10mm) real physical
// room near the hinge instead of being tight against it -- real
// judgment call on the exact amount (no number given), sized to clear
// the link's own 10mm rod diameter with margin.
HINGE_OUT       = HINGE_PIVOT_OFFSET + 1 + 15;     // 21mm -- real overlap OUTSIDE the wall face (Y<0), covers the real pivot line AND leaves room for the folding link hardware
// TRAY_MOUNT_GAP -- REAL FIX, 2026-07-30, Janis's own direct catch: the
// tray plate itself was still mounted flush with the pivot line, NOT
// the (now-widened) hinge block's own outer tip -- so the tray plate
// actually OVERLAPPED the hinge block's own footprint by this same
// amount (confirmed: hinge spans world Y=[-HINGE_OUT,HINGE_INTO_WALL],
// old tray plate started at Y=-HINGE_PIVOT_OFFSET, deep inside that
// span) -- meaning there was never really a real gap for the folded
// link to hide in, despite HINGE_OUT being widened. FIX: the tray
// plate's own inner edge now starts flush with the hinge block's own
// outer tip, leaving the whole hinge block's own depth clear as real,
// open space behind the tray for the folded link.
TRAY_MOUNT_GAP  = HINGE_OUT - HINGE_PIVOT_OFFSET;  // 16mm
...
// knuckle standoff, see above), Z=HINGE_Z — angle_deg: 0 (deployed,
// horizontal, plate spans outward toward -Y per the Standing Orientation
// Convention's own "toward the user" side) to +90 (stowed, vertical,
// tip toward the ground). LOCKED CONVENTION, v17, Janis's own explicit
// spec, never to change: default/stowed position has the tray tip
// facing the ground; "activating" (deploying) is a CLOCKWISE rotation
// about X viewed from +X, ending flat/horizontal — v16 had this
// backwards (angle_deg -90=stowed swung the tip UP past apex A instead
// of down, confirmed via a real colored render, not assumed from the
// code). TWO REAL BUGS FOUND+FIXED VIA LIVE CGAL SWEEP PROBES (not
...
module tray(x0, angle_deg) {
    ...
    // direction + kinematics corrected in v17, see tray_link()'s own
    // header above for the full real derivation). `tt`'s pin is rigidly
    // part of the tray (bolted to its underside) and rotates WITH the
    // tray about the tray's own real pivot (Y=-HINGE_PIVOT_OFFSET,
    // Z=HINGE_Z); `ts` is a TRUE FIXED anchor at `al`; the link's own
    // middle lap-joint (`lc`) is what genuinely folds, via a real
    // circle/circle intersection -- see tray_link_lc() above.
    ...
    translate([x0, -HINGE_PIVOT_OFFSET, HINGE_Z]) rotate([angle_deg, 0, 0]) {
        // plate's own inner edge starts at TRAY_MOUNT_GAP (flush with the
        // hinge block's own outer tip), NOT at the pivot line itself --
        // real fix, see TRAY_MOUNT_GAP's own header for why
        translate([0, -TRAY_D - TRAY_MOUNT_GAP, 0])
        ...
        // a coincident face.
        translate([0, -TRAY_D-TRAY_MOUNT_GAP-TRAY_T, -TRAY_SKIRT_H])                    // tip (outer) edge
            cube([TRAY_L, 2*TRAY_T, TRAY_SKIRT_H + TRAY_T]);
        translate([-e, -TRAY_D-TRAY_MOUNT_GAP-TRAY_T, -TRAY_SKIRT_H])                    // left side edge
            cube([2*TRAY_T, TRAY_D+TRAY_T, TRAY_SKIRT_H + TRAY_T]);
        translate([TRAY_L-TRAY_T, -TRAY_D-TRAY_MOUNT_GAP-TRAY_T, -TRAY_SKIRT_H])         // right side edge
            cube([2*TRAY_T, TRAY_D+TRAY_T, TRAY_SKIRT_H + TRAY_T]);
    }
}
...
TRAY_LINK_OVERLAP = 10;   // mm, real judgment call -- Janis's own explicit spec: the 2 links must overlap (a real lap joint) at the middle pivot, not just meet at a point
TRAY_LINK_SPAN0   = norm(TRAY_LINK_TT_REF - TRAY_LINK_TS_REF);          // the deployed (0°) reference reach, ts(fixed)-to-tt -- 221.529mm
TRAY_LINK_C_HALF  = (TRAY_LINK_SPAN0 + TRAY_LINK_OVERLAP) / 2;          // each link half's own real reach from its own end to the mid-joint, including its half of the lap overlap
function perp2(v) = [-v[1], v[0]];

// v17 REAL REBUILD, 2026-07-31 -- two real fixes from Janis's own direct
// review of the v16 render:
//
// (1) FOLD DIRECTION, LOCKED, NEVER CHANGE: default/stowed position is
// tray tip facing the ground (vertical); "activating" (deploying) is a
// CLOCKWISE rotation about X, viewed from +X, ending flat/horizontal.
// v16 had this backwards (angle_deg: -90=stowed swung the tip UP past
// apex A, confirmed wrong via a real colored render, not assumed). Fixed
// convention: angle_deg 0=deployed (horizontal, unchanged) ..
// +90=stowed (tip down). The tray's own hinge is the fixed center of
// rotation (HINGE_PIVOT_OFFSET/HINGE_Z, unchanged) -- see tray() below.
//
// (2) LINK KINEMATICS REBUILT to match the corrected direction. Under
// the OLD (wrong) direction, a fixed `ts=al` couldn't reach `tt` through
// the full sweep (distance grew past the link's own length) -- that's
// why v16 made `ts` a slider. Checked fresh under the CORRECTED
// direction (not assumed): the fixed-`ts`-to-rotating-`tt` distance now
// SHRINKS monotonically as the tray folds (221.5mm at 0° down to
// 11.2mm near 90°) -- exactly the real 2-bar-link behavior Janis
// described (two rigid arms closing, their free ends' distance
// shrinking, absorbed by the collapsible middle link bowing to one
// side) -- so `ts` reverts to Janis's own ORIGINAL spec: a TRUE FIXED
// anchor at `al`, no slider, no rail. The give is now entirely in the
// link's own middle pivot (`lc`), which genuinely folds.
function tray_rot17(p, angle_deg) =
    let(piv = [-HINGE_PIVOT_OFFSET, HINGE_Z], rel = p - piv, c = cos(angle_deg), s = sin(angle_deg))
    [rel[0]*c - rel[1]*s, rel[0]*s + rel[1]*c] + piv;
function tray_link_tt(angle_deg) = tray_rot17(TRAY_LINK_TT_REF, angle_deg);

// `lc`, the middle lap-joint pivot: `ts` and `tt` are each a real,
// CONSTANT TRAY_LINK_C_HALF away from it (both link halves are rigid,
// fixed-length) -- a genuine circle(ts,C_HALF)/circle(tt,C_HALF)
// intersection, not an approximation. Two roots exist; picking the one
// CLOSER to apex A is Janis's own direct correction (checked against a
// labeled triangle simulation before writing this) -- matches the real
// hardware description: as the tray folds toward stowed, `lc` genuinely
// travels toward apex A while `tt` tucks up under the tray and `ts`
// stays put at the wall. DISCLOSED, NOT FULLY RESOLVED: real, isolated
// collision probe (link vs tray plate) confirms EMPTY from 0° to about
// 86° (the entire normal operating range) -- a real, small collision
// remains from about 87° to 90°, where `ts`/`tt` nearly coincide
// (11mm apart vs each link half being ~115mm) and the circle
// intersection becomes ill-conditioned -- tried multiple root-selection
// variants, the instability is inherent to this near-degenerate
// geometry there, not a selection-rule bug. Accepted as a practical
// ~86° fold limit, Janis's own call (this link is explicitly "not
// critical," math doesn't need to be perfect).
function tray_link_lc(angle_deg) =
    let(tt = tray_link_tt(angle_deg), d = norm(tt - TRAY_LINK_TS_REF), mid = (TRAY_LINK_TS_REF + tt)/2,
        h = sqrt(max(TRAY_LINK_C_HALF*TRAY_LINK_C_HALF - (d/2)*(d/2), 0)),
        dirv = (tt - TRAY_LINK_TS_REF)/d, perp = [-dirv[1], dirv[0]],
        m1 = mid + perp*h, m2 = mid - perp*h)
    (norm(m1-RIB_REF_A) < norm(m2-RIB_REF_A)) ? m1 : m2;
...
// stack, NOT additive (not 2x their own individual thickness). Modeled
// here as 2 flat strips (still schematic, matching hinge_u()'s own
// "simple, not fab-precise" convention -- the real nested-channel
// cross-section isn't modeled) offset in DEPTH (X) just enough to keep
// their combined envelope at exactly this 10mm, not stacked apart with
// extra clearance. This SAME depth-split idea also came from Janis's
// own triangle lesson this round: 2 coplanar rigid bars sharing one
// pivot, both with real width, cannot close together without
// interpenetrating UNLESS offset in depth -- verified by direct
// rectangle-overlap simulation before writing this (not assumed) --
// matches why real hinges/scissors stack their 2 leaves in depth
// instead of building them coplanar.
TRAY_LINK_STACK_T = 10;
TRAY_LINK_X_SPLIT = TRAY_LINK_STACK_T - TRAY_LINK_T;   // 7mm center-to-center -- keeps the combined envelope at exactly TRAY_LINK_STACK_T
module tray_link(x_center, angle_deg) {
    ts = TRAY_LINK_TS_REF;
    tt = tray_link_tt(angle_deg);
    lc = tray_link_lc(angle_deg);
    x1 = x_center - TRAY_LINK_X_SPLIT/2;
    x2 = x_center + TRAY_LINK_X_SPLIT/2;
    flat_link(ts, lc, x1);
    flat_link(tt, lc, x2);
    hinge_u(ts, x1);
    hinge_u(tt, x2);
    translate([x_center,lc[0],lc[1]]) rotate([0,90,0])

...
// ═══════════════════════════════════════════════════════════════════
// TASK 3 kinetic driver — real Customizer-visible parameter, drives the
// WHOLE rib/handle/axle/CB1 assembly below AND (via this reassignment,
// placed deliberately AFTER the include chain) the chambers file's own
// `lid_open_deg` kinetic variable — OpenSCAD's real "last top-level
// assignment wins globally" behavior, empirically verified in-sandbox
// the prior session via echo() probes on this project's real 3-file
// include chain (Section 2 of the source prompt). Adjusting
// `door_open_deg` here moves the rib assembly AND the visual lid shell
// together.
// ═══════════════════════════════════════════════════════════════════
...
// ─── Grab handle -- CORRECTED this session ───
HANDLE_Y        = -140;
...
R_HANDLE = norm([HANDLE_Y - FC_Y, HANDLE_Z - FC_Z]);   // 551.9mm, real radial pivot->handle distance (matches docs/lid-hinge-counterbalance-calc.md)
...
// ─── Full rib spine: handle -> A -> B -> C -> pivot -> corner-arc -> CB1
// -> prong tip. Door-side arm traces the real A-B/B-C contour (Section 5);
// CB-side branch traces the D-E contour offset outward (via the corner
// arc above, converted to native frame). ───
RIB_SPINE = concat(
    [[HANDLE_Y,HANDLE_Z], RIB_REF_A, RIB_REF_B, RIB_REF_C, [FC_Y,FC_Z]],
    CORNER_ARC_CLOSED,
...
// header, "tray interference" finding): the A-B weld-flush run's own
// 70-100mm target is satisfied by its real LENGTH along the wall
// (~253mm, A to B), not by a wide radius bulging perpendicular into -Y
// (the adjacent tray's own territory). 25mm keeps real material there
// while minimizing (not fully eliminating -- see flagged finding) tray
// intrusion; C stays wider since it sits at Y=178.665, far from the
// tray's own Y<=0 reach.
WELD_HALF_W_A  = 22;
WELD_HALF_W_B  = 25;
WELD_HALF_W_C  = 40;
...
RIB_SPINE_R = concat(
    [HANDLE_WRAP_R, WELD_HALF_W_A, WELD_HALF_W_B, WELD_HALF_W_C, AXLE_HALF_W],

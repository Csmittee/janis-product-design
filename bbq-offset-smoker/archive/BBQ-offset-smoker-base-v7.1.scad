...
include <BBQ-understructure-v17.scad>
...
// ─── Pivot (fulcrum) -- v7 REAL FIX: rebuilt from RIB_REF_D, not
// RIB_REF_C. Root cause (Janis's own direct diagnosis against the real
// v6.1 code, this file's own header): apex C (178.665mm) sits BELOW
// DATUM_Y_CENTER(305mm) -- i.e. on the LID's own half of the ridge, not
// the fixed half -- so the axle/bearing built from it was structurally
// attached to the MOVING lid, not the fixed shell (why the bearing
// rendered embedded in the door). Apex D (431.335mm) IS provably fixed-
// side, and is already the CB1 branch's own real anchor -- using it here
// keeps the fixed pivot and the fixed-side CB1 anchor on the same real
// structure. Same real offset CONCEPT as the old formula (15mm along the
// ridge, UCP204-12's real 33.3mm H dimension, UNCHANGED values) --
// direction flipped (measured inward from D toward C, not outward from
// C toward D), since D sits on the opposite side of the ridge from C. ───
FC_Y = RIB_REF_D[0] - 15;          // 416.335mm -- real margin vs DATUM_Y_CENTER(305mm): 111.335mm, comfortably fixed-side
FC_Z = RIB_REF_D[1] + 33.3;        // UCP204-12's real "H" dimension, UNCHANGED value (D and C share the same DATUM_Z_RIDGE, so this numeric value is unchanged even though the reference point moved)
...
R_HANDLE = norm([HANDLE_Y - FC_Y, HANDLE_Z - FC_Z]);   // v7: 685.8mm (was 534.3mm under v6.1's C-based pivot) -- REAL, FLAGGED CONSEQUENCE of the TASK 2 pivot relocation, not a handle change (HANDLE_Y/Z themselves are UNCHANGED, Section 5 DO NOT TOUCH): docs/lid-hinge-counterbalance-calc.md's own swept force curve is now stale AGAIN, not recomputed this round (Janis has deferred stopper/counterbalance review)
...
// Branch bow waypoint -- v7 REAL REBUILD, RETIRES the old 45mm
// corner-arc-around-D (v6/v6.1). Root cause (this file's own header):
// re-tested against the NEW, much-closer pivot (36.522mm from D, was
// ~419mm), the old arc-around-D technique made clearance WORSE (near-0mm
// at several angles) -- arc waypoints placed close to D swing even
// closer to D when rotated about a pivot that's already close to D.
// FIX: a single, precisely-placed bow waypoint. Real method: find the
// closest point to D on the NAIVE straight path from the pivot to CB1's
// own OPEN-state position (the tightest real configuration, confirmed
// via sweep), then push that specific point 50mm directly AWAY from D
// (radially), then convert to native/closed frame via the SAME
// round-trip rotation functions already trusted for CB1/the handle.
// PROVABLE CEILING (see this file's own header): since the pivot itself
// is a mandatory, non-moving point on this path at a fixed 36.522mm from
// D, no construction can push the branch's own centerline clearance past
// that 36.522mm ceiling -- a push of 50mm (comfortably past the point
// where the OLD tight spot stops being the binding constraint) reaches
// that ceiling exactly, confirmed via a real 0.01°-step sweep.
...
// all the way to the new fixed-side pivot near D (v7 TASK 2 -- a much
// longer run than the old C-based pivot). CB-side branch traces from the
...
// bore, this project's own standing convention) -- but see this file's
// own header for the REAL, PROVABLE FINDING this round: with the pivot
// now only 36.522mm from apex D, this 28.5mm pad leaves just 8.022mm net
// clearance to D -- short of the 20mm apex-clearance rule, and NOT
// fixable by further tuning (36.522mm is a hard ceiling given the
// locked FC formula; shrinking this below the 15mm-meat convention was
// deliberately NOT done, to avoid trading one flagged violation for
// another). Flagged for Janis, not silently absorbed.
...
module ucp_bearing(x_pos) {
    // HOUSING_R -- v7 REAL, REDUCED (was AXLE_STUB_OD/2+12=21.525mm):
    // the pivot's new position (36.522mm from apex D, this file's own
    // header) leaves no room for the old, more generous placeholder
    // size. This housing is a "simple bbox/cylinder, no supplier casting
    // detail" placeholder (rules-bbq-fab.md Construction Method), not
    // the rib's own structural pivot pad (AXLE_HALF_W, unchanged, see
    // its own flagged 8.022mm finding above) -- shrinking THIS placeholder
    // is a real, defensible judgment call. Real result: 20.997mm net
    // clearance to D -- clears the 20mm rule for this specific part,
    // though the rib's own structural pivot material still does not.
    HOUSING_R = AXLE_STUB_OD/2 + 6;
    difference() {
        union() {
...
// v7.1 REAL ATTEMPT, NOT CONFIRMED (see this file's own header): stop
...
// path.
show_lid = false;   // reassignment attempt -- SAME unverified mechanism as `lid_open_deg`'s own reassignment; if this ALSO gets Customizer-forced back to true, chambers' own lid() will still fire and this will show a doubled lid. Flagged, not asserted fixed.

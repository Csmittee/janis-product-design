...
// ─── Pivot (fulcrum) -- v7.2 REAL FIX, REVERSES v7's own TASK 2:
// Janis's own direct correction against a REAL reference product photo
// (a real, physically-built hinge on an actual smoker) -- v7's apex-D
// relocation was WRONG despite being geometrically defensible (D IS
// provably on the fixed side of DATUM_Y_CENTER); the real, physically
// correct hinge location is close to apex C after all. cc's own real
// caveat, stated explicitly per this project's own R-014 Verification
// Discipline Rule: real hinge hardware typically mounts via a BRACKET
// bolted to fixed material, which can hold the actual pivot PIN at
// almost any (Y,Z) reachable by that bracket -- the "pivot point itself
// must have Y > DATUM_Y_CENTER" test (v7's own reasoning) checks whether
// a bore sitting AT that exact point has fixed material around it, which
// is a DIFFERENT, stricter question than whether a bracket anchored to
// fixed material can REACH that point. This FC_Y value (208.665mm) does
// NOT pass v7's own point-in-fixed-territory test (208.665mm <
// DATUM_Y_CENTER=305mm) -- flagged explicitly, not silently dropped;
// Janis's own real-world reference takes precedence here, not a
// re-derivation from octagon topology.
// FC_Z UNCHANGED (Janis's own explicit instruction, "Z remains the
// same") -- same UCP204-12 real H-dimension value used in EVERY version
// to date, C_Z and D_Z are numerically identical (both DATUM_Z_RIDGE) so
// this line's own text is unaffected by the C/D reference-point switch.
// FC_Y offset increased from the original v6/v6.1 value: 15mm -> 30mm
// along the ridge from apex C, per Janis's own explicit instruction
// (stated twice, consistently: "30mm away from C in y direction"). ───
FC_Y = RIB_REF_C[0] + 30;          // 208.665mm -- Janis's own real-world-verified location, NOT re-derived by cc
FC_Z = RIB_REF_C[1] + 33.3;        // UCP204-12's real "H" dimension, UNCHANGED value and UNCHANGED formula structure from every prior version
...
R_HANDLE = norm([HANDLE_Y - FC_Y, HANDLE_Z - FC_Z]);   // v7.2: 543.0mm (was 685.8mm under v7's D-based pivot, 534.3mm under v6.1's original C+15 pivot) -- REAL, FLAGGED CONSEQUENCE of the pivot's own real position, not a handle change (HANDLE_Y/Z themselves are UNCHANGED, Section 5 DO NOT TOUCH): docs/lid-hinge-counterbalance-calc.md's own swept force curve is stale YET AGAIN, not recomputed this round (Janis has deferred stopper/counterbalance review)
...
// bore, this project's own standing convention). v7.2: with the pivot
// moved back near apex C, this pad's own static clearance to apex D is
// now ~196.6mm (dist FC-D 225.1mm minus this 28.5mm half-width) --
// comfortably clear, v7's own flagged 8.022mm shortfall at this specific
// pad no longer applies now that the pivot sits far from D again. The
// CB1 BRANCH's own clearance to D is the real open concern now instead
...
module ucp_bearing(x_pos) {
    // HOUSING_R -- v7.2: RESTORED to the original, more generous
    // placeholder size (was shrunk to AXLE_STUB_OD/2+6 in v7 specifically
    // to fit the tight D-based pivot, now reversed) -- with the pivot
    // back near apex C (~225mm from D), there is no tight-clearance
    // reason to keep it undersized. "Simple bbox/cylinder, no supplier
    // casting detail" placeholder (rules-bbq-fab.md Construction Method).

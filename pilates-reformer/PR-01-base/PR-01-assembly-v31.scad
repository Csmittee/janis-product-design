// PR-01-assembly-v31.scad
// Janis Product Design — Pilates Reformer with Full Tower
// Version: v31 — base-file-consolidation (2026-07-03): fixes a file-
// organization deviation from the original PR-01-multifile-split-v25 plan,
// found by Janis during a full child-file review. bell_lock_collar() and
// its 5 helpers (bell_dome_pts/bell_dome/bell_thread/bell_bayonet/
// bell_cap) moved from pole_top.scad to leg_socket.scad — this module
// belongs to the base/leg system, not the top-joint housing, and was
// misplaced during the 2026-07-02 bell-collar-socket-build; had zero
// standalone preview anywhere as a result. Zero visual/dimensional
// change — module bodies byte-identical, only file location changed;
// full-assembly render confirmed byte-identical geometry calls to v30.
// leg_socket.scad's ghost stanza now also calls bell_lock_collar()
// directly (previously unreachable in any standalone context). Source:
// PR-01-assembly-v30.scad.
// Previous header, v30 — bed-height-pad-cutaway-fix (2026-07-02): fixes 3
// visual defects found on first render of v29 (bell-collar/leg-socket
// build), reported by Janis before F6: (1) bed_h 500→600, reconciled to
// leg_socket.scad's leg_h(600mm) so the bed surface sits flush on top of
// the new embedded-socket leg instead of appearing sunk below it —
// requires pole_top.scad's TASK 1a fix (dormant bed_h self-reassignment
// removed) to actually take effect; (2) leg_socket()'s foot pad call
// commented out (leg_socket.scad) — never part of the intended design;
// (3) leg_socket.scad ghost-context cutaway added so the embedded socket
// bore is visible when iterating on that file standalone — full-assembly
// render unaffected. Source: PR-01-assembly-v29.scad.
// Previous header, v29 — FIRST production geometry pass on pole
// holder/socket. Adds bell_lock_collar() (pole_top.scad, REVISED
// CONFIRMED dome profile + ENGINEER-PROPOSED thread/bayonet lock
// mechanism) and leg_socket() (new leg_socket.scad, CONFIRMED embedded
// pass-through socket sleeve, first-time lock). Replaces the old
// external split-clamp collar concept (pole_base_collar() call site
// commented out in pole_top.scad, module kept for history).

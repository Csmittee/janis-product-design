...
// Version: v10 — pole_top() Correct Asymmetric Shape + Through-Bore
// Date: 2026-06-30
...
// split-clamp sleeve (both unchanged this version).
//
// Changes from v9 (per IMG_3314 sketch reference — owner's actual design):
//   - pole_top() FULLY REBUILT. v2–v9 all built it as a symmetric placeholder
//     frustum/cone (body of revolution) — this was never correct. The real
//     part is an asymmetric "foot/boot"-shaped HEAD on top of a cylindrical
//     NECK: flat-ish back edge, curved/rounded front edge, wider than the
//     pole. Built via linear_extrude() of an asymmetric 2D profile (X-Z
//     side-view silhouette) — explicitly NOT a body of revolution, no
//     d1/d2 cylinder/cone anywhere in this module.
//   - Horizontal through-bore (d=33mm, TBD pending physical fit test) added
//     through the HEAD (not the neck) for the crossbar pipe to pass through
//     and lock — axis aligned to the existing xbar_z / crossbar X-orientation.
//   - 2 new screw bosses at the neck (M6 hex countersunk, TBD dims) for the
//     neck-to-pole ASSY joint — a separate 2-screw mechanism, distinct from
//     pole_base_collar()'s 3-bolt/6-bolt split clamp.
//   - top_boss_h increased 70mm → 160mm to give the new head+neck enough
//     vertical room (neck 50mm + head 110mm) to contain the bore with real
//     wall thickness on all sides. This is a numeric resize only — the
//     pole_body() and xbar_z formulas themselves are UNCHANGED code, they
//     just recompute off the new top_boss_h value (pole_body() consequently
//     renders shorter; xbar_z recomputes to 2020mm, still inside the head).
//
...
top_boss_h     = 160;   // total pole_top vertical envelope (neck_h + head_h) — resized from v9's 70mm placeholder to fit the head/bore with real wall thickness
...
neck_r         = neck_d / 2;
head_h         = top_boss_h - neck_h;  // = 110mm
head_back_w    = 20;    // head extent behind neck centerline — flat-ish back edge
head_front_w   = 60;    // head extent in front of neck centerline — curved front edge (wide head)
...
// X-Z side-view profile (NOT a body of revolution — no d1/d2 cylinder/cone). Profile is
// asymmetric: flat-ish back edge (negative local X), curved front bulge (positive local X).
// dir mirrors the profile so the curved front faces consistently per pole position
// (same dir convention as pole_body()'s flat-face cut).
module pole_top_head(cx, cy, z_base, dir) {
    head_profile = [
        [-neck_r,              0],
        [-head_back_w,         head_h * 0.15],
        [-head_back_w * 0.95,  head_h * 0.90],
        [-neck_r * 0.4,        head_h],
        [ head_front_w * 0.3,  head_h],
        [ head_front_w,        head_h * 0.55],
        [ head_front_w * 0.75, head_h * 0.12],
        [ neck_r,              0]
    ];
    translate([cx, cy, z_base])
        scale([dir, 1, 1])

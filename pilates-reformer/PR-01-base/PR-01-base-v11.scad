...
// Version: v11 — pole_top() Orientation Fix
// Date: 2026-06-30
...
// head+neck (replacing the old frustum/cone placeholder) but elongated the
// wrong axis (Z instead of X).
//
// Changes from v10 (Janis QA fix — root cause confirmed):
//   - v10's loft elongated along Z (tall, narrow head, standing upright) —
//     wrong axis. Fixed: head now elongates along X, the same direction the
//     bore/pipe points, and is short in Z. head_back_w 20→40mm, head_front_w
//     60→90mm (head X total = 130mm > head_h = 110mm Z) — this is purely a
//     reshaping of the head_top_head() profile control points; head_h,
//     neck_h, neck_d, top_bore_d, bore position (xbar_z/top_boss_h math),
//     and the 2x M6 screw joint are all UNCHANGED from v10 per this prompt's
//     explicit scope.
//   - Profile rounded further (more control points, helmet-like curve) and
//     a tapered blend added at the neck-to-head transition (fillet_h=15mm)
//     instead of an instant jump from neck_r to full head width — this
//     approximates a fillet via gradual polygon widening (a true CAD
//     fillet/round is Stage 2 precision-fit work, flagged below).
//
...
neck_r         = neck_d / 2;
head_h         = top_boss_h - neck_h;  // = 110mm (short axis — Z, unchanged from v10)
head_back_w    = 40;    // head extent behind neck centerline — flat-ish back edge (v11: widened 20→40 so X is the long axis, toward the bore/pipe)
head_front_w   = 90;    // head extent in front of neck centerline — curved front edge, toward bore exit (v11: widened 60→90; head X total = 130mm > head_h = 110mm Z, fixing v10's wrong elongation axis)
head_depth     = 70;    // head Y-extent (lateral) — exceeds pole_d for bore wall clearance
fillet_h       = 15;    // v11 — tapered blend height at neck-to-head transition (polygon-widening approximation of a fillet, not a true CAD round — Stage 2 precision pass)
...
// X-Z side-view profile (NOT a body of revolution — no d1/d2 cylinder/cone). v11: long
// axis runs X (toward the bore/pipe exit), short axis Z (height) — v10 had this
// backwards. Profile is asymmetric: flat-ish back edge (negative local X), rounded
// front bulge (positive local X, toward the bore), with a tapered fillet_h blend out
...
module pole_top_head(cx, cy, z_base, dir) {
    head_profile = [
        [-neck_r,               0],
        [-neck_r * 1.3,         fillet_h],
        [-head_back_w,          head_h * 0.35],
        [-head_back_w * 0.85,   head_h * 0.85],
        [-head_back_w * 0.3,    head_h],
        [ head_front_w * 0.35,  head_h],
        [ head_front_w,         head_h * 0.60],
        [ head_front_w * 0.6,   head_h * 0.18],
        [ neck_r * 1.3,         fillet_h],
        [ neck_r,               0]
    ];
    translate([cx, cy, z_base])

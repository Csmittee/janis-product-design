...
// Version: v14 — pole_top() ORIENTATION ONLY fix (single-rotate construction)
// Date: 2026-06-30
...
// through it.
//
// v14 — SINGLE-ISSUE FIX, orientation only, per explicit "one job" prompt:
//   - Root cause (this is the SECOND time this exact mistake happened, v10
//     and v13 both): building the bell's exterior and its bore subtraction
//     directly in mixed/partial orientations let inconsistent rotations
//     creep in. Fix: pole_top_bell() now builds the COMPLETE solid (bell
//     exterior via rotate_extrude + bore cylinder, both still along Z, the
//     natural/default axis) first, then wraps the entire finished
//     difference() result in exactly ONE rotate([0,90,0]) as the last
//     operation — never rotating the exterior and the bore cut separately.
//   - After that single rotation the bell's small face sits at local X=0
//     (the pole/neck side) and the large face extends along X (toward the
//     crossbar's incoming direction) — large face = entrance, small face =
//     exit, per Janis's note. scale([dir,1,1]) (same dir convention used
//     pre-v13 for mirroring) then flips which world-X direction the bell
//     extends in per pole corner, so the bell always points toward bed
//     center / the opposite pole, matching the crossbar's actual span.
//   - Bell now sits at world height xbar_z (the same crossbar height used
//     by crossbar_body() — untouched), forming a literal T-junction: the
//     vertical neck sleeve (unchanged from v13) is the stem, the horizontal
//     bell is the crossbar of the T, exactly per Janis's note 3.
//   - Profile silhouette changed from horn (convex, bulging outward) to
//     wine-glass (concave, curving inward) per Janis's note 2 — Bezier
//     control points p1/p2 now pull the curve INSIDE the straight
//     small-to-large line instead of bulging outside it. This is the only
//     shape-related change in this round (the prompt scoped "tuning comes
//     after" but the wine-glass-vs-horn distinction was explicitly listed
//     alongside the orientation fix, so both are addressed together).
//   - DO NOT TOUCH (per prompt) and left exactly as v13: bore diameter
//     (33mm), neck sleeve construction/dimensions, M6 through-bolt holes,
//     pole_body(), pole_base_collar(), crossbar geometry. The neck's z
//     anchor (z0 formula) and the bolt hole z positions are UNCHANGED
//     formulas from v13 — only pole_top_head()/pole_top_bell() and how
//     pole_top() composes neck+bell changed.
//   - ⚑ FLAG: because the neck (still anchored at the v13 z0 formula) and
//     the bell (now anchored at xbar_z, ~70mm above z0) are positioned
//     independently per the "do not touch the neck" constraint, there is an
//     approx. 24mm vertical gap between the top of the neck sleeve and the
//     bottom of the bell's circular envelope at xbar_z. Visually they should
//     still read as a connected T-junction given the bell's ~92mm diameter,
//     but this is a proportion item for Janis to confirm once shape tuning
//     resumes — flagged rather than silently adjusted, since adjusting it
//     would mean touching the neck's z anchor, out of scope this round.
...
module pole_top(cx, cy) {
    ...
    z0  = bed_h + pole_h - top_boss_h;  // neck top anchor — UNCHANGED formula from v13, matches pole_body()'s top
    dir = (cx < bed_l / 2) ? 1 : -1;
    color("#2C3E50", 1.0) {
        difference() {
            pole_top_neck(cx, cy, z0);
            // 2x M6 through-bolt clearance holes through both walls of the sleeve —
            // bolt + nut (not blind/threaded), UNCHANGED positions from v13
            // (neck_h*0.3 / neck_h*0.7 below z0), axis along X

...
// Version: v17 — Neck-bell true alignment + seam blend + bolt edge clearance
// Date: 2026-06-30
...
module pole_top(cx, cy) {
    ...
    color("#2C3E50", 1.0) {
        difference() {
            ...
            // v17: 2x M6 through-bolt clearance holes, repositioned for real edge
            // clearance against the new neck_h=70mm (NOT the old 0.3/0.7 fraction
            // split). neck spans [neck_top-70, neck_top]:
            //   neck_top-20: 20mm from top/seam edge, 50mm from bottom/pole edge
            //   neck_top-50: 50mm from top/seam edge, 20mm from bottom/pole edge
            // both exceed bolt_edge_clearance_min (15mm TBD) with 5mm extra margin
            for (bz = [neck_top - 20, neck_top - 50])
                translate([cx, cy, bz])
                    rotate([90, 0, 0])

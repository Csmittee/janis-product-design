...
// Version: v19 — Item A: pole_top_transition() coordinate dump only, NO geometry
// change (diagnostic per SKILL_joint_construction.md isolation-test discipline).
// Item B: bell profile now wine-glass concave at BOTH ends (large/pipe-entry face
// was a convex horn flare, now mirrors the small face's existing concave curve);
// head_large_r reduced 6mm (46mm -> 40mm), still > head_small_r (23.5mm).
// Date: 2026-06-30
...
// pole_top_transition(cx, cy, dir, neck_top) — v17 NEW. Bridges the neck's straight
// vertical cylindrical wall (axis Z) into the bell's curving horizontal body-of-
// revolution (axis X) with a smooth hull() fillet instead of a hard right-angle seam.
// hull() of: a thin disc at the neck's top cross-section (centered cx,cy,neck_top,
// axis Z, diameter neck_od) and a short same-diameter cylinder slice at the bell's
// small face (centered cx,cy,xbar_z, axis X via rotate([0,90,0])+scale([dir,1,1]),
// length neck_bell_transition_h). Both cross-sections share the same diameter
// (neck_od == head_small_r*2), so the hull is a smooth tapered wedge bridging the
// two orientations. Caller wraps this inside the same difference() that subtracts
// the bore-envelope cylinder from the neck — bore-safety is inherited from that cut,
// not re-derived here, so the transition can never intrude the bore void.
module pole_top_transition(cx, cy, dir, neck_top) {
    // v18: bell-side anchor updated to match pole_top_bell()'s recentered small face
    // (world X = cx - dir*head_h/2, see pole_top_bell() v18 header note) — previously
    // assumed the small face sat at (cx,cy,xbar_z), which is no longer true now that
    // the bell is centered on the neck axis instead of edge-anchored to it.
    bell_small_face_x = cx - dir * head_h / 2;
    hull() {
        translate([cx, cy, neck_top])
            cylinder(h = e, d = neck_od, $fn = 64);
        translate([bell_small_face_x, cy, xbar_z])
            rotate([0, 90, 0])
                scale([dir, 1, 1])
                    cylinder(h = neck_bell_transition_h, d = neck_od, $fn = 64);
    }
}

...
// Version: v23 — v21 F5 QA FAIL fixes (3 issues), per prompts/PR-01-base-v22-housing-fix.md.
// Source: v22 (already in repo from a prior rapid-fire that applied only the
// profile_pts() sin->cos condition fix) — saved forward as v23 per this prompt's
// "if v22 already exists, use v22 as source and save as v23" instruction.
// CHANGES FROM v22:
//   - profile_pts(): already cos(a)>=0 in v22 (confirmed matches this prompt's Task 1) — unchanged.
//   - housing_peak_t: 0.40 -> 0.60 (fat/prominent face now points INWARD toward bar center)
//   - Added module pole_top_neck_blend() — closes 4.5mm neck-to-housing gap
//   - pole_top(): union() now calls housing + neck + neck_blend
//   - crossbar_body(): DEBUG — bar extended 50mm past each pole joint end for
//     visual pass-through verification. REVERT before production.
// Date: 2026-07-01
...
//
// CHANGES FROM v20 (carried from v21/v22 header, still accurate):
...
// v23: cos(a)>=0 condition confirmed carried forward from v22 unchanged —
// dome goes UP in world +Z. rotate([0,90,0]) maps profile -X -> world +Z,
// so cos(a)<0 = top (+Z). Old sin(a) condition mapped dome to world Y (sideways).
function profile_pts(r_top, r_bot, n = 48) = [
    for (i = [0 : n - 1])
        let(a = i * 360 / n, r = cos(a) >= 0 ? r_bot : r_top)
        [r * cos(a), r * sin(a)]
];
    ...
    color("#2C3E50") {
        difference() {
            union() {
                ...
                pole_top_neck_blend(cx, cy);  // v23: neck-to-housing gap bridge
            }

...
// Version: v24 — smooth profile function + remove blend ring, per
// prompts/PR-01-base-v24-smooth-profile.md. Source: v23.
// CHANGES FROM v23:
//   - profile_pts(): binary cos(a)>=0 step -> smooth cosine blend
//     (1-cos(a))/2. Fixes diamond/polygon end-face AND the housing-side
//     gap in one root-cause fix — side radius now (r_bot+r_top)/2≈27mm,
//     which is > neck radius 23.5mm, so the sides cover the neck naturally.
//   - Removed module pole_top_neck_blend() — no longer needed (was a v23
//     patch for the side gap; smooth profile makes it redundant AND it was
//     causing a double-ring z-fighting artifact at the housing base).
//   - pole_top(): union() back to 2 calls (housing + neck), blend call removed.
// Date: 2026-07-01

...
// Changes from v2:
//   Fix 1: crossbar_body() corrected to X-axis (rotate [0,90,0]) — longitudinal along bed length
//           Assembly uses xbar_y_front/xbar_y_rear (Y positions of front/rear pole rows)
//   Fix 2: pole_top_collar() bore corrected to X-axis (rotate [0,90,0], center=true)
//           Bore drills side-to-side (X) to accept longitudinal crossbar — correct orientation
...
// D-shaped cross-section slice: circle clipped on -X side (15% depth cut = convex D-shape)
module d_slice(r, h) {
    intersection() {
...
// Logarithmic taper D-profile loft from r_base (base) to r_top_taper (40% height)

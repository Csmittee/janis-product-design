...
// Version: v8 — Crossbar Seating + Overshoot Fix
// Date: 2026-06-30
...
// shaft, no taper), BASE COLLAR (insert pin to wood socket), WOOD SOCKET
// (plain cylindrical insert in drilled hole).
//
// Changes from v7 (Janis QA FAIL — root cause confirmed by direct code read):
//   - xbar_z now SUBTRACTS top_boss_h*0.5 (was adding) — crossbar seats
//     mid-height inside the pole_top() boss instead of floating above it.
//   - grip_l now spans pole-center to pole-center (bed_l - leg_w), not bed
//     edge to bed edge — crossbar no longer overshoots past the poles.
//   - crossbar_body() X origin starts at leg_w/2 (pole center), not 0 (bed edge).
//   - crossbar_end_cap() calls use actual pole_cx[] positions, not 0/grip_l bed extremes.
//   - $fn=64 set explicitly on pole_top()/pole_base_collar()/pole_wood_socket()
//     cylinders to match pole_body() resolution — removes faceted/knurled look.
//
...
module pole_base_collar(cx, cy) {
    color("#2C3E50", 1.0) {
        translate([cx, cy, bed_h])
            cylinder(h = collar_h, d = collar_d, $fn = 64);

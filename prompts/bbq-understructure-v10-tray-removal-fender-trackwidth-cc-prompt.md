CC PROMPT — bbq-understructure-v10-tray-removal-fender-trackwidth
================================================================
Repo location: bbq-offset-smoker/BBQ-understructure-v10.scad (new, source v9)
...
SCOPE — Janis is running a separate, direct round with cc on those
(the +50mm parting-line change). Do not open or reference any pending
chamber-side change beyond reading its CURRENT live values.
...
BBQ-understructure-v9.scad and whatever chambers file it includes:
`FIREBOX_W` (expect 580mm), `WHEEL_D`/`WHEEL_R` (expect 457.2/228.6mm),
`TREAD_W` (expect 200mm), current `TRACK_WIDTH_FIREBOX_GAP` (expect
100mm, about to change), and confirm `prep_shelf()`/`prep_shelves()`'s
real current callers/consumers before removing anything (R-009 — state
the actual call sites found, don't assume none exist beyond what this
prompt expects).

Task-specific reads:
1. bbq-offset-smoker/BBQ-understructure-v9.scad — LIVE, in full.
2. Whichever BBQ-chambers-vXX.scad file v9 currently includes — LIVE,
   read only for `FIREBOX_W`/`WHEEL_R`/outer shell side-wall real
   world-Y position (needed for TASK 2's fender weld point). Do NOT
   assume the +50mm parting-line change is present — that is a
   separate, parallel round; read whatever is ACTUALLY there right now
   and state it.
...
None. New `BBQ-understructure-v10.scad` (source v9).
...
or mounting bracket in this file. That work is intentionally deferred to
a separate accessories file, out of this prompt's scope.
...
Real CGAL checks required, not assumed: fender clears the wheel itself
(empty overlap vs. the tire), fender clears the rear axle/struts (empty
overlap), fender makes real contact with the firebox outer shell
(non-empty overlap). State all three results explicitly.
...
this new, narrower geometry — state the real result explicitly, do not
assume resolved because the prior round resolved it at a different
width.

## 5. DO NOT TOUCH

- Any chambers file — read only, zero changes, including the pending
  +50mm parting-line change (separate round, not yours to build or
  assume)
...
- [ ] TASK 2: real fender dimensions stated (300mm extension, 548.64mm
      ...
      3 CGAL checks (wheel-clear, axle/strut-clear, shell-contact)
      stated explicitly, confirmed NO front-wheel fender exists
- [ ] TASK 3: real TRACK_WIDTH stated (expect 880mm), real wheel
      Y-positions stated, real collision re-check result stated
- [ ] Full CGAL manifold sweep, `Simple: yes` on every solid
- [ ] Full kinetic sweep (steer/fold full range, lid/door/ash-tray
      states) — `Simple: yes` throughout (note: no more prep-shelf
      fold state to sweep, confirm removed from sweep list too)
...
5. Save as BBQ-understructure-v10.scad (v9 kept); update

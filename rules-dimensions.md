...
| Acrylic/metal split — "DOUBLE SHEET" RE-CHECKED WITH A REAL RENDER (v56, vm01-v56-sensor-bracket-frame-joint-fix, FLAGGED — DESIGN QUESTION, NOT A BUG) | Acrylic pane sits `door_t`=3mm behind the metal skin's own front face (deliberate recess, documented since v48/v49's window-with-inset-pane design) | Janis's screenshot on the merged v55 PR still showed a "double sheet" in the open-door top view — the v52 fix above was arithmetic-only and its own record explicitly said "Janis F6 recheck still required," never actually confirmed via a live render until this session. Re-checked properly this time: rendered the real open-door top view (OpenSCAD binary available) and confirmed no duplicate `#ADD8E6` object exists (`left_front_acrylic()` still dead code, same finding as v52). What's actually visible: the acrylic's intentional `door_t` recess behind the metal skin reads, from directly above, as two adjacent parallel lines/bands (metal edge + recessed glass edge) — this is what a window with an inset pane looks like from a top cross-section, not a stacked-duplicate-solid artifact. Whether Janis wants the pane flush (zero recess) instead of recessed is a design decision, not a provable geometry bug — flagged for her input, not guessed at or changed. |

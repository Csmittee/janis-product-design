...
//
// Changes from v9:
//   Fix 1: Spring zone front panel added — acrylic #ADD8E6 opacity 0.15,
//           full spring tray zone (Z 300-542), width product_w, left hinge.
//           Customer selection window — see-through panel shown closed.
//   Fix 2: Screen Y moved -30mm (protrudes 30mm forward from right compartment
//           front face) so it is clearly visible from front view.
...
screen_h = 100;
screen_center_z = 280;
screen_protrude = 30;   // FIX 2: screen protrudes this far in front of machine face
screen_y = 0 - screen_protrude;   // -30mm — forward of front face
bracket_r = 8;
bezel_t = 3;
qr_w = 40;
qr_h = 30;
...
// FIX 1: Acrylic customer selection panel — spring tray zone Z 300-542.
// Transparent, hinge on left edge, shown closed.
module spring_zone_panel() {
    panel_w = product_w - (skin_t * 2);
    panel_h = tray_zone_h;   // 242mm
    panel_t = door_t;         // 3mm

    // See-through acrylic panel
    color("#ADD8E6", 0.15)
    translate([skin_t, 0, tray_0_z])
        cube([panel_w, panel_t, panel_h]);

    // 3 hinges on left edge
    hinge_zs = [
        tray_0_z + panel_h * 0.2,
        tray_0_z + panel_h * 0.5,
        tray_0_z + panel_h * 0.8
    ];
    for (hz = hinge_zs) {
        color("#888888")
        translate([skin_t, 0, hz])
...
// FIX 2: screen_y = -30mm (protrudes 30mm forward of front face).
module dashboard() {
...
spring_zone_panel();    // FIX 1: acrylic customer selection panel, Z 300-542
tray_zone_frame();      // structural frame around spring display zone
drop_zone_visual();

sensor_strip();         // parallel strips left+right at Z=280

dashboard();            // FIX 2: screen protrudes 30mm forward
acrylic_display();
rear_service_door();

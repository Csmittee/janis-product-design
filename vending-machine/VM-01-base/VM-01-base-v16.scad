...
//
// Changes from v15:
//   Fix 1: acrylic_display() front panel — replaced hull() with flat cube (simpler, no z-fighting)
//   Fix 2: outer_shell() — removed assembly translate([0,0,leg_h]); hollow subtract now at leg_h+skin_t
//   Fix 3: version header corrected to v16
...
module outer_shell() {
    ...
    difference() {
        ...
        // Hollow interior — starts at leg_h+skin_t so base plate covers leg zone
        translate([skin_t, skin_t, leg_h + skin_t])
...
module tray_zone_frame() {
    ...
    union() {
        ...
        translate([product_w - frame_bar, e, tray_0_z])
            cube([frame_bar, skin_t, tray_zone_h]);
    }
}

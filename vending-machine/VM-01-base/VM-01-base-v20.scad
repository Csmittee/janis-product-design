...
//
// Changes from v19:
//   Fix 1: spring_coil() — inner cylinder height spring_l+1 → spring_l-1.
//           Inner was 1mm taller than outer = open top edge = non-manifold.
//   Fix 2: compartment_divider() — height total_h-leg_h → total_h-leg_h-skin_t.
//           Top face was flush with shell top (coplanar) = non-manifold.
//   Fix 3: tray_zone_frame() — side bars shortened to exclude corner regions.
//           Top/bottom bars own full-width corners; side bars no longer share edges there.
...
module tray_zone_frame() {
    color("#AAAAAA")
    union() {
        // Top bar — full width, owns top corners
        translate([0, e, tray_zone_top_z - frame_bar])
            cube([product_w, skin_t, frame_bar]);
        // Bottom bar — full width, owns bottom corners
        translate([0, e, tray_0_z])
            cube([product_w, skin_t, frame_bar]);
        // Left bar — middle only, Z excludes corner regions
        translate([0, e, tray_0_z + frame_bar])
            cube([frame_bar, skin_t, tray_zone_h - (frame_bar * 2)]);
        // Right bar — middle only, Z excludes corner regions
        translate([product_w - frame_bar, e, tray_0_z + frame_bar])
            cube([frame_bar, skin_t, tray_zone_h - (frame_bar * 2)]);
    }
}

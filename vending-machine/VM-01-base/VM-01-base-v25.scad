...
//
// Changes from v24:
//   Fix: spring_tray() — epsilon offsets on motors, springs, partitions.
//        All internal objects had faces coplanar with tray body faces = non-manifold.
//        Motor: rear flush with rear wall (Y=tray_d), bottom flush with floor (Z=tray_floor_t).
//        Spring coil: bottom flush with floor (Z=tray_floor_t).
//        Partition: rear flush with rear wall inner face (Y=tray_d-tray_wall_t), bottom flush with floor.
...
module spring_tray(tray_num) {
    ...
    translate([10, tray_start_d, z]) {
        ...
        for (i = [0:spring_lanes-1]) {
            ...
            // Motor: lifted e off floor, shortened e from rear wall
            color("#555555")
            translate([x-10, tray_d - motor_d + e, tray_floor_t + e])
                cube([20, motor_d - e, 35]);

            // Spring coil: lifted e off floor so bottom face clears tray floor top face
            color("#999999")
            translate([x, tray_d - motor_d, tray_floor_t + spring_od/2 + e])
                rotate([90, 0, 0])
                    spring_coil();

            if (i < spring_lanes-1) {
                // Partition: lifted e off floor, shortened e from rear wall inner face
                color("#BBBBBB")
                translate([x + spring_od/2 + spring_gap/2, tray_wall_t, tray_floor_t + e])

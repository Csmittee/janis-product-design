...
//
// Changes from v28:
//   Task 1: total_w 620 → 640, system_w 185 → 204 (screen board 180mm + bezel×2 + margin).
//           dash_w = 204 - 19 - 4 = 181mm. Screen circuit board 180×100mm now fits with tilt.
//   Task 2: Dashboard undercut cleared — Task 1 width fix resolves right shell corner clip.
//           Clearance = 638 - 610 = 28mm before tilt displacement. No code change in dashboard().
//   Task 3: spring_coil() h = spring_l → spring_l - 2. Gives 2mm clearance between coil front
//           face and motor rear face. Fixes 2-manifold overlap (was 0.02mm over). Visual only.
//   Task 4: Added show_shell_top/left/right debug toggles and outer_shell_debug() module.
//           Assembly now calls outer_shell_debug(). outer_shell() kept as clean reference.
...
module spring_tray(tray_num) {
    ...
    translate([10, tray_start_d, z]) {
        ...
        for (i = [0:spring_lanes-1]) {
            ...
            // Motor: lifted e off floor, pulled e from rear wall
            color("#555555")

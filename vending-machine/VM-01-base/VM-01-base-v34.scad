...
//
// Changes from v33:
//   spring_tray() for-loop: motor cube resized [20, motor_d-e, 35] → [18, motor_d-2, 33].
//   Motor translate X: x-10 → x-9 to re-centre the 18mm wide cube (9mm each side of x).
//   Smaller cube shrinks away from all tray faces, reducing manifold edge-contact risk.
...
module spring_tray(tray_num) {
    ...
    translate([10, tray_start_d, z]) {
        ...
        for (i = [0:spring_lanes-1]) {
            ...
            // Motor: 18×(motor_d-2)×33, centred at x (9mm each side), inside hollow
            color("#555555")

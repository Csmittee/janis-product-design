...
//
// Changes from v34:
//   spring_tray() for-loop: motor cube wrapped in if(false) — hidden for manifold isolation test.
//   Spring and partition positions UNCHANGED — they reference motor_d parameter, not motor object.
//   To restore motor: change if(false) to if(true). No other edits needed.
//
// DEBUG RULE — never comment out a variable declaration line.
//   Commenting out "show_shell_back = true" while outer_shell_debug() still uses it
//   causes "Ignoring unknown variable" warnings. To hide a panel: set the value to false,
//   do not comment the line out.
...
module spring_tray(tray_num) {
    ...
    translate([10, tray_start_d, z]) {
        ...
        for (i = [0:spring_lanes-1]) {
            ...
            // Motor: hidden for manifold isolation test — change if(false) to if(true) to restore
            if (false) {
                color("#555555")
            ...
            // Spring coil: rear face at tray_d-motor_d-2 = 388mm, 2mm clear of motor front face
            color("#999999")
            translate([x, tray_d - motor_d - 2, tray_floor_t + spring_od/2 + e])
                rotate([90, 0, 0])
                    spring_coil();

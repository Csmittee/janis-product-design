...
//
// Changes from v36:
//   Added render_mode for two STL exports:
//     render_mode = "full"  → Export 1: full exterior with outer shell → save as VM-01-v37.stl
//     render_mode = "open"  → Export 2: shell + front panels removed, internals visible → save as VM-01-v37-open.stl
//   Workflow: set render_mode below → F6 → File > Export > STL → switch mode → F6 → Export again.
...
// ─────────────────────────────
// "full" → Export 1: full exterior shell + all components → VM-01-v37.stl
// "open" → Export 2: no shell, no front panels → all internals visible → VM-01-v37-open.stl
render_mode = "full";

// DEBUG TOGGLES — only active in "full" mode
// ─────────────────────────────
...
// Front panels — hidden in open mode so internals are visible
if (render_mode == "full") {
    front_door();           // exit zone door, Z 50-300
    spring_zone_panel();    // acrylic customer panel, Z 300-700
    acrylic_display();      // right upper acrylic zone
}

flap_door();            // customer pickup flap — visible in both modes

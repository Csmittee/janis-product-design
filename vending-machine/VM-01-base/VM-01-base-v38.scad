...
if (render_mode == "full") {
    spring_zone_panel();    // acrylic front panel left zone Z 300-700
    acrylic_display();      // acrylic right upper zone
}

front_door();           // exit zone door Z 50-300 — always visible
flap_door();            // customer pickup flap — always visible

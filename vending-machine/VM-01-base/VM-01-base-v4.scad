...
// Units: mm
// Changes from v3:
//   - tray_h: 116 → 86 (spring OD 66 + 20mm clearance)
//   - spring_tray: front face removed (Y=0 side open)
//   - tray Z: leg_h + exit_door_h + (tray_num * tray_h)
//   - acrylic_panel: right compartment only
//   - left zone: front door cutout only, no acrylic
...
// Front face (Y=0) is OPEN — springs visible from customer side
module spring_tray(tray_num) {
    ...
    translate([10, tray_start_d, z]) {
        ...
        // Tray body — open front (Y=0 face removed)
        color("#CCCCCC")
        difference() {
            ...
            translate([tray_wall, tray_wall, tray_wall])
                cube([tray_w-(tray_wall*2),
                      tray_d-(tray_wall*2),
                      tray_h+1]);
            // Remove front face (Y=0 side open)

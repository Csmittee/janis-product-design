...
// Left-zone front door (v41) — replaces front_door()+flap_door()+spring_zone_panel()
...
// v41: Left-zone front door — replaces front_door()+flap_door()+spring_zone_panel().
// Full left-zone height (door_bot_z..door_top_z). Hinge concealed on the flat
// left-side wall (hinge_y), off the rounded front-left corner, joined to the
// front panel by a return flange that wraps the corner. Contains an inset
// acrylic viewing window and a top-hinged exit flap that swings inward onto
// a static stopper rod. door_open/flap_open pivot the leaf/flap for viewer
// STL export; the flap and stopper travel with the leaf when door_open=true.
module left_zone_door() {
    door_h = door_top_z - door_bot_z;              // 648mm
    door_open_angle = door_open ? door_open_deg : 0;
    flap_top_z = exit_bot_z + exit_h;               // 230mm -- flap hinge line
    flap_angle = flap_open ? flap_open_deg : 0;
    window_x0 = door_left_x + window_margin_side;   // 40mm
    window_x1 = door_right_x - window_margin_side;  // 376mm
    window_z0 = flap_top_z + window_flap_gap;       // 270mm
    window_z1 = door_top_z - window_top_gap;        // 683mm
    stop_y = skin_t + exit_h * sin(flap_open_deg);  // flap's fully-open Y position
    stop_z = flap_top_z - exit_h * cos(flap_open_deg); // flap's fully-open Z position

    // Whole leaf (flange, panel, hinge marks, acrylic, flap, stopper) pivots on door_open
    translate([door_left_x, hinge_y, 0])
    rotate([0, 0, -door_open_angle])
    translate([-door_left_x, -hinge_y, 0]) {

        // 1. Return flange — wraps the rounded front-left corner, joins front panel to hinge line
        color("#888888", 0.9)
        translate([door_left_x, skin_t, door_bot_z])
            cube([door_t, return_depth, door_h]);

        // 2+4+6. Main front panel with top window + exit-flap openings cut
        color("#888888", 0.9)
        difference() {
            translate([door_left_x + return_depth, skin_t, door_bot_z])
                cube([door_right_x - (door_left_x + return_depth), door_t, door_h]);

            translate([window_x0, skin_t - 1, window_z0])
                cube([window_x1 - window_x0, door_t + 2, window_z1 - window_z0]);

            translate([exit_x, skin_t - 1, exit_bot_z])
                cube([exit_w, door_t + 2, exit_h]);
        }

        // 3. Hinge seam indicators — flush marks on the flange at the hinge line, concept placeholder only
        for (pct = [0.1, 0.5, 0.9])
            color("#333333")
            translate([door_left_x - e, hinge_y - hinge_mark_d, door_bot_z + door_h*pct - hinge_mark_h/2])
                cube([door_t + e*2, hinge_mark_d, hinge_mark_h]);

        // 5. Acrylic viewing pane — mounted door_t further inside than the door face; "full" mode only
        if (render_mode == "full")
            color("#ADD8E6", 0.3)
            translate([window_x0 - acrylic_border, skin_t + door_t, window_z0 - acrylic_border])
        ...
        color("#B0B0B0", 0.9)
        translate([0, skin_t, flap_top_z])
        rotate([flap_angle, 0, 0])
        translate([0, -skin_t, -flap_top_z])
            translate([exit_x, skin_t, exit_bot_z])
                cube([exit_w, flap_t, exit_h]);

        // 8. Stopper rod — static relative to the leaf, positioned to catch the flap at flap_open_deg.
        // No proximity switch/sensor geometry — explicitly deferred (Janis).
        color("#555555")
...
// left_zone_door()'s inset acrylic window is gated internally on render_mode=="full" (same rule)
if (render_mode == "full") {
    acrylic_display();      // acrylic right upper zone
}

left_zone_door();       // v41: full-height left-zone door — always visible (structural, like the old front_door()/flap_door())

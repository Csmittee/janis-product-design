...
// §2.3 (v43) -- window/acrylic FROZEN as the door's OWN local constants.
// Numerically identical to v42's DATUM_TRAY_BOT/TOP-derived values (world
// Z 270-512mm) -- only the reference chain changed: the window is the
// door's own feature, not a Cousin reference to the tray zone's naming.
window_x0 = 38;    // local X, door's local origin -> window left edge
window_z0 = 220;   // local Z, door's local origin -> window bottom edge
window_w  = 336;
window_h  = 242;
...
module left_zone_door() {
    ...
    rotate([0, 0, -door_open_angle]) {
        ...
        // show_acrylic must both be true.
...
module sensor_strip() {
    ...
    color("#FF0000", 0.3)
    translate([strip_x0 + strip_t,
               skin_t + drop_zone_d/2,
               strip_z + strip_h/2])
        cube([strip_x1 - (strip_x0 + strip_t), 2, 2]);
}

...
// the 100mm tray-stack shift -- spans the FULL width and depth of the
// tray/product compartment (edge-to-edge), NOT a removable/access panel.
// Positioned at the pre-shift tray_0_z (270mm) -- the anchor the trays
// moved away from, NOT the post-shift tray_stack_z0. Works together with
// exit_compartment_wall() (below) to fully close the reach-through path
// from the exit zone into the tray compartment.
module tray_compartment_partition() {
    partition_t = tray_floor_t;   // 5mm, reuses the tray floor's own thickness convention
    color("#AAAAAA")
    translate([skin_t, skin_t, tray_0_z])
        cube([product_w - (skin_t*2), total_d - (skin_t*2), partition_t]);
}
...
// boundary), full width, floor to tray_compartment_partition()'s top,
// with an e overlap so the two share real volume (Rule M-1), not just a
// touching face -- confirmed ZERO gap by construction (wall Z1 =
// tray_0_z+partition_t+e, one epsilon INTO the partition's own thickness).

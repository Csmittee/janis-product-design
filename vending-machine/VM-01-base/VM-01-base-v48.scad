...
module left_zone_door() {
    ...
    // v48 TASK 3c: the exit flap's full swept volume (0-flap_open_deg,
    // rotating about flap_top_z) overlaps the metal panel below by up to
    // ~451mm² (Python/shapely) -- the fill was a solid slab with no hole
    // cut for the flap's own operating volume (Janis: "the hidden acrylic
    // paste on back blocking the flap door"). Notch X matches the leaf's
    // own flap cutout exactly (flap doesn't move in X). Z covers the
    // flap's closed position up to its swept max: flap_top_z+flap_t is
    // the analytic bound on the free corner's own rotation radius about
    // the top hinge line (+e for real clearance) -- confirmed ≥ the
    // live-computed sweep max (232.46mm world at flap_open_deg=55).
    flap_notch_z0 = exit_bot_local;             // 30mm local -- matches the leaf's own flap cutout
    flap_notch_z1 = flap_top_z + flap_t + e;    // 183.01mm local
    ...
    rotate([0, 0, -door_open_angle]) {
        ...
        // v48 TASK 3: three pieces now, all sharing show_acrylic —
        //   (a) lower metal panel (unchanged Z range, notched for the flap)
        //   (b) upper acrylic (3a: real door_acrylic_t depth; 3b: top
        //       pulled down to acrylic_top_limit_z, clear of the frame's
        //       top weld flange)
        //   (c) top metal cap (3b: thinner metal, not acrylic, occupies the
        //       zone nearest the flange -- see acrylic_top_limit_z above)
        if (show_acrylic) {
            // (a) Lower metal panel — plain flat sheet, no vents/cutouts/
            // branding, UNCHANGED Z range and door_t depth from v46/v47.
            // v48 TASK 3c: difference() notches out the exit flap's full
            // swept volume (flap_notch_z0/z1 above) -- was a solid slab
            // that blocked the flap from opening at all (~451mm² overlap,
            // confirmed via live geometry). Notch cuts clean through this
            // panel's own Y-depth (door_t) with margin either side.
            color("#999999")
            difference() {
                ...
                translate([exit_x_local - 1, front_y - 1, flap_notch_z0])
                    cube([exit_w + 2, door_t + acrylic_border + 2, flap_notch_z1 - flap_notch_z0]);
            }

            // (b) Upper acrylic — v48 TASK 3a: real door_acrylic_t depth
            // (was door_t, same as metal). v48 TASK 3b: top boundary
            // pulled down from win_z1+acrylic_border (698mm world, the
            // door's own absolute top edge, zero clearance) to
            // acrylic_top_limit_z (682.99mm world) -- clear of
            // tray_zone_frame()'s top weld flange, see that constant's
            // derivation above.
            color("#ADD8E6", 0.3)
            translate([win_x0 - acrylic_border, front_y + door_t, split_z_local])
                cube([(win_x1-win_x0) + acrylic_border*2, door_acrylic_t, acrylic_top_limit_z - split_z_local]);

            // (c) Top metal cap — v48 TASK 3b: only the THINNER metal
            // panel (door_t, not door_acrylic_t) occupies the zone nearest
            // the frame's top flange, per Janis's explicit direction --
            // the resulting larger top border (15.01mm, was 5mm) is
            // acceptable/desired. Nudged e down into the acrylic's own
            // top for real shared volume (Rule M-1), not a touching face.
            // FLAG (not fully resolved this round): this cap's OWN door_t
            // depth still has a residual ~1.01mm Y-depth overlap with the
            // flange's front face (frame_y0=7mm vs this cap's back face at
            // 8.01mm world) -- the SAME magnitude the old 3mm acrylic had
            // pre-v48, not new, not eliminated by this fix (which targets
            // the NEW 3.01mm risk 3a introduced, not this pre-existing
            // one) -- see cc_chat_log, no further redesign attempted here.
            color("#999999")
            translate([win_x0 - acrylic_border, front_y + door_t, acrylic_top_limit_z - e])
                cube([(win_x1-win_x0) + acrylic_border*2, door_t, ((win_z1 + acrylic_border) - acrylic_top_limit_z) + e]);
        }
...
module tray_zone_frame() {
    // v48: frame_inset/FLANGE_T promoted to PARAMETERS (shared with
    // left_zone_door()'s TASK 3b clearance calc) -- no longer local here.

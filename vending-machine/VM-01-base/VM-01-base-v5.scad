...
// Units: mm
// Changes from v4:
//   - acrylic_panel: inset window panel with rounded corners (hull+cylinders, r=10)
//   - spring_tray: left and right side walls windowed (3mm frame, open centre)
...
spring_lanes = 5;
tray_count = 2;
tray_h = 86;       // spring OD 66 + 20mm clearance
tray_wall = 3;
partition_h = 40;
...
// Front face (Y=0) OPEN. Left and right side walls windowed (3mm frame, open centre).
module spring_tray(tray_num) {
    ...
    translate([10, tray_start_d, z]) {
        ...
        difference() {
            ...
            // Remove front face (Y=0 open)
            translate([tray_wall, -1, tray_wall])
                cube([tray_w-(tray_wall*2),
                      tray_wall+1,
                      tray_h-(tray_wall)]);

            // Open left side wall — 3mm border frame, centre cut through
            translate([-1, tray_wall, tray_wall])
                cube([tray_wall+2,
                      tray_d-(tray_wall*2),
                      tray_h-(tray_wall*2)]);

            // Open right side wall — 3mm border frame, centre cut through
            translate([tray_w-tray_wall-1, tray_wall, tray_wall])
                cube([tray_wall+2,
                      tray_d-(tray_wall*2),
                      tray_h-(tray_wall*2)]);
        }
...
module outer_shell() {
    ...
    difference() {
        ...
        // Front door cutout — left product zone only (no acrylic)
...
// Acrylic window panel — right compartment, above screen to roof.
// Inset thin panel (acrylic_t) with rounded corners (hull + cylinders, r=acrylic_r).
// Sits on front face of right compartment at Y=skin_t.
module acrylic_panel() {
    acrylic_x = product_w + divider_t;
    acrylic_w = system_w - divider_t - skin_t;
    acrylic_z = screen_z + screen_h;
    acrylic_h = total_h - skin_t - acrylic_z;

    color("#ADD8E6", 0.3)
    translate([acrylic_x, skin_t, acrylic_z])
        // hull of 4 cylinders oriented along Y (panel thickness axis)
        hull() {
            translate([acrylic_r, 0, acrylic_r])
                rotate([-90, 0, 0])
                    cylinder(r=acrylic_r, h=acrylic_t);
            translate([acrylic_w-acrylic_r, 0, acrylic_r])
                rotate([-90, 0, 0])
                    cylinder(r=acrylic_r, h=acrylic_t);
            translate([acrylic_r, 0, acrylic_h-acrylic_r])
                rotate([-90, 0, 0])
                    cylinder(r=acrylic_r, h=acrylic_t);
            translate([acrylic_w-acrylic_r, 0, acrylic_h-acrylic_r])
                rotate([-90, 0, 0])
                    cylinder(r=acrylic_r, h=acrylic_t);
        }
}

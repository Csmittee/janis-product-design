...
//
// NAMING NOTE: prompts/VM-01-corner-frame-redesign.md said "Active file:
// VM-01-base-v48.scad -> save as VM-01-base-v49.scad" -- that name was
// already taken (VM-01-door-flap-acrylic-fix shipped the real v49 first,
// merged before this prompt was executed). Built on the REAL latest file
// (v49) and saved as v50 instead, per "never overwrite, always increment
// version." Flagged explicitly, not silently resolved.
//
    ...
    rotate([0, 0, -door_open_angle]) {
        ...
        difference() {
            ...
            translate([win_x0, front_y - 1, split_z_local])
                cube([win_x1 - win_x0, door_t + 2, win_z1 - split_z_local]);
        ...
        // continuous door_t skin (see the difference() above). Four
        // pieces remain, all sharing show_acrylic:
        ...
        //   (d) top metal cap (v48 TASK 3b, UNCHANGED by this prompt --
        //       out of its "the acrylic" scope, own X-extent and residual
        //       flange overlap both still flagged, not re-addressed here)
        if (show_acrylic) {
            ...
            // (d) Top metal cap — v48 TASK 3b: only the THINNER metal
            ...
            // UNCHANGED by v49 (own X-extent still win_x0-acrylic_border
            // ..win_x1+acrylic_border -- out of THIS prompt's "the
            // acrylic" scope). FLAG (not fully resolved, carried from
            // v48): this cap's OWN door_t depth still has a residual
            // ~1.01mm Y-depth overlap with the flange's front face
            // (frame_y0=7mm vs this cap's back face at 8.01mm world) --
            // the SAME magnitude the old 3mm acrylic had pre-v48, not new,
            // not eliminated here -- see cc_chat_log.
            color("#999999")
...
module tray_zone_frame() {
    ...
    left_bar_inner_x = door_left_x + frame_inset + frame_bar;  // 15mm (was 27mm) -- X0(=door_left_x+frame_inset=7) + frame_bar(8)

    // v50 TASK 2 (caught via live render, not assumed): with frame_bar
    // narrowed to 8mm, left_bar_inner_x(15mm) is now SMALLER than the
    // curve's own natural max X reach at the full 180-270 sweep (21mm,
    // at frame_arc_r=14) -- sweeping the full range and then jumping back
    // to X=15 made the polygon fold back on itself (self-intersecting,
    // confirmed: OpenSCAD logged "mesh not closed" on this exact module in
    // isolation). Fixed generally: the curve now sweeps only until EITHER
    // it reaches Y=frame_y0 OR X=left_bar_inner_x, whichever comes first
    // (min of the two candidate angles) -- for the current values, the
    // X-bound is what actually binds (curve stops at X=15mm, Y~=8.35mm,
    // before reaching angle=270). The closing points below then step down
    // to frame_y0 and back to left_bar_inner_x -- correct (and a
    // zero-length no-op edge, harmless) regardless of which bound binds.
    x_stop_angle = 180 + acos((world_arc_cx - left_bar_inner_x) / frame_arc_r);
    y_stop_angle = 180 + asin((world_arc_cy - frame_y0) / frame_arc_r);
    arc_end_angle = min(x_stop_angle, y_stop_angle);  // ~244.6deg (X-bound, for current values)
    arc_segs = 24;

    left_bar_arc = [for (i = [0:arc_segs]) let(a = 180 + i*(arc_end_angle-180)/arc_segs)
        ...
        translate([0, 0, frame_z0])
            linear_extrude(height = frame_h)
                polygon(concat(left_bar_arc,
                    [[left_bar_inner_x, left_bar_arc[arc_segs][1]],
                     [left_bar_inner_x, frame_y0],
                     [left_bar_inner_x, left_bar_arc[0][1]]]));

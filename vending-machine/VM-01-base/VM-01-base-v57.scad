...
//
// Changes from v56 (vm01-acrylic-curve-hinge-lock-fix — fresh session,
// prior direct-chat session on this topic hung on cloud container resume
// and was abandoned before anything was committed, treated as new; real
// OpenSCAD/CGAL binary installed and used for every finding below —
// renders, STL facet-coordinate/cross-section extraction, and a real
// parametrized collision-bisection sweep, not estimates):
//   TASK 1 (acrylic "double sheet"/step at the TOP edge specifically,
//     FIXED): v56's own "door leaf material thickness" theory was WRONG —
//     Janis's show_acrylic=false test made BOTH lines disappear together,
//     which a leaf-thickness edge (never gated by show_acrylic) can't
//     explain. Real cause, found via a real CGAL cross-section (X=200
//     slice, STL vertex extraction): the acrylic assembly's own top edge
//     and the window cutout's own top edge in the leaf land within 0.01mm
//     of the same Z but at two different Y-depths — reads as 2
//     closely-stacked edges from any oblique angle. LEFT/RIGHT never
//     showed this because their border strips already span a real
//     multi-mm band; only the TOP lacked an equivalent border. Fixed:
//     new acrylic_top_z pulls the pane's real top down 2mm+e (same
//     Global Clearance Tolerance convention as acrylic_x0/x1), new top
//     border piece (e) fills the vacated band — mirrors the already-
//     working left/right construction exactly. Window CUTOUT itself
//     UNCHANGED (border-only fix). Re-verified via real CGAL cross-
//     section + top-down PNG renders (before/after) + full-assembly
//     Simple:yes.
//   TASK 2 (door curve vs. left shell wall tangency + frame X-room,
//     INVESTIGATED, NO code change): real CGAL top-down renders (door
//     leaf isolated, acrylic/frame hidden) confirm the door's own
//     return-flange curve is ALREADY exactly tangent to the shell's
//     interior corner curve (identical center/radius, zero kink) — the
//     visible double-line artifact traces to TASK 1's acrylic geometry
//     and/or the frame-to-shell gap (next finding), not a flaw in the
//     door's own curve. Built a real parametrized CGAL bisection harness
//     testing whether hinge_x/hinge_y offset frees tray_zone_frame()'s
//     frame_arc_r past its 15.96mm cap (v50) — result: the closed-door
//     footprint (door_open_deg=0, the binding/tightest constraint) is
//     mathematically INVARIANT to hinge_x/hinge_y by this file's own
//     "world target minus hinge offset, then translate by hinge offset"
//     construction, confirmed identical collision threshold at hinge_x
//     offsets of 0/4/8/12mm. Frame X-room is NOT achievable via this
//     lever — re-confirms v50's finding with a stronger, independent,
//     live-swept method, stated plainly per this task's own explicit
//     permission rather than forcing an ineffective change.
//   TASK 3 (hinge-pivot rod, REMOVED): the v50 reinforcement cylinder
//     (hinge_od diameter, full door_h height) deleted entirely from
//     left_zone_door() — a removal, not a toggle. Pivot point itself
//     (hinge_x/hinge_y/hinge_z, this module's own translate/rotate
//     origin) untouched, pure rotation math only, door_open/
//     door_open_deg animation unaffected (the cylinder was never part of
//     the rotation math, only added reinforcement mass). Full knuckle-
//     hinge hardware explicitly DEFERRED to a separate future session.
//     Pivot point now recorded as a locked reference in
//     rules-dimensions.md. Re-verified via a real CGAL 11-angle sweep
//     (0-100°, 10° steps): Simple:yes at every angle, zero warnings.
//   TASK 4 (lock system, PROVISIONED — placeholder only): no existing
//     project file (design_scope_of_work_rule.md, rules-dimensions.md,
//     cc_chat_log.md, all searched in full) specifies a lock type,
//     position, or dimensions — flagged explicitly for Janis's input,
//     not invented. Added the most conservative/generic placeholder: a
//     shallow (1.5mm, non-through) 30x40mm recess on the door leaf's
//     RIGHT edge (opposite the hinge), clear of the flap's full swept
//     X-range and the window/acrylic's Z-range (both confirmed live, not
//     guessed) — plus a matching shallow strike-provision recess on
//     compartment_divider()'s own front-left corner, the adjacent fixed
//     structure immediately next to the door's right edge. New
//     show_lock_provision toggle (internal, mirrors show_sensor's
//     pattern) gates both. Re-verified: full-assembly Simple:yes.
//   FULL-ASSEMBLY MANIFOLD CHECK: real CGAL 11-angle sweep (0-100°, 10°
//     steps) covering all 4 tasks above simultaneously — Simple:yes at
//     every angle, zero warnings, zero regressions from v56's baseline
//     (which was itself the first fully-clean version in this file's
//     history).
//
...
module left_zone_door() {
    ...
    // v57 TASK 1 (vm01-acrylic-curve-hinge-lock-fix): ROOT CAUSE of the
    // "double sheet"/step Janis flagged at the TOP edge specifically,
    // confirmed via a real OpenSCAD CGAL cross-section (X=200, world Y-Z
    // plane) at the top boundary, not arithmetic alone: the acrylic
    // assembly's own top edge (fill (a)/(b)/(c) below, world Y 5.01-10.01mm)
    // and the window CUTOUT's own top edge in the leaf (the hole boundary,
    // world Y 2.005-5.005mm) both land within 0.01mm of the SAME Z
    // (682.98 vs 682.99mm world, an intentional Rule M-1 shared-volume
    // nudge) -- but at TWO DIFFERENT Y-depths. From any oblique/top-down
    // viewing angle this reads as two closely-stacked, differently-offset
    // edges (a real double-offset), not the single clean recess line the
    // LEFT/RIGHT sides already show (there, the border strips (b)/(c) run
    // the full window height as a substantial, continuous few-mm band, so
    // the "hole edge" and "fill edge" never coincide in Z the way they do
    // here at the top). CONFIRMED matches Janis's own show_acrylic=false
    // test: with the acrylic assembly hidden, only the leaf's own hole
    // boundary remains (one line, the normal window-opening edge) -- both
    // "double sheet" lines vanish together because both belong to the
    // acrylic assembly (this pane top edge + its own near-coincident
    // mounting edge), exactly as Janis reported.
    // FIX: give the acrylic a genuine TOP BORDER, same 2mm Global
    // Clearance Tolerance convention already used for acrylic_x0/x1 on
    // left/right (not a new number) -- pane's own real top pulled DOWN by
    // 2mm+e so it's no longer near-coincident with the hole's own edge.
    // The vacated 2mm-tall band is filled by a new visible metal border
    // piece below (mirrors (b)/(c) exactly), turning the near-coincident
    // double edge into one continuous metal-border-then-acrylic transition,
    // the same look the left/right sides already have. Considered instead
    // shifting the window CUTOUT itself down (the prompt's other suggested
    // option) -- rejected: that would either bury the (unchanged) acrylic
    // top inside solid leaf material (a new real interference) or require
    // this exact same acrylic-top pullback anyway, so it collapses to the
    // same fix with an added side effect; this version has none.
    acrylic_top_z = acrylic_top_limit_z - 2 - e;  // 630.98mm local -- world 680.98mm, pane's real top
    ...
    rotate([0, 0, -door_open_angle]) {
        ...
        difference() {
            ...
            // v57 TASK 4: lock system PROVISION -- shallow recess only
            // (lock_provision_d, half of door_t, NOT a through-hole), right
            // edge of the door opposite the hinge. PLACEHOLDER -- see
            // PARAMETERS block for the full note and Janis-input flag.
            if (show_lock_provision)
        ...
        // v57 TASK 3 (vm01-acrylic-curve-hinge-lock-fix): the v50 TASK 4
        // hinge-pivot reinforcement cylinder (hinge_od diameter, full
        // door_h height, sitting at this rotation axis) is REMOVED entirely
        // -- Janis's assessment, accepted as this round's design direction:
        // a real hinge is a mating pair of fixed/dynamic knuckles with real
        // gaps, not one continuous rod, and this rod's presence right at
        // the shell's exterior corner was the recurring source of collision
        // ambiguity in this project (v52 arithmetic misattribution, v55
        // "ruled out", re-suspected during this round's curve investigation
        // before being cleared again -- see TASK 2 above). This is a
        // REMOVAL, not a toggle -- no show_hinge_rod added. The pivot
        // POINT itself (hinge_x/hinge_y/hinge_z below, this module's own
        // translate/rotate origin) is untouched and still pure rotation
        // math -- door_open/door_open_deg animation is unaffected, there
        // was never any dependency from the rotation itself on this
        // cylinder's SOLID geometry (it was purely an added reinforcement
        // body, not part of the pivot math). Removing a rod can only
        // reduce collision risk (a dimensionless point can't collide with
        // anything) -- re-verified explicitly via a real CGAL 10-angle
        // sweep rather than assumed, see cc_chat_log. Full knuckle-hinge
        // hardware (mating fixed/dynamic pairs) is a SEPARATE, DEFERRED
        // future task -- not attempted this round. Pivot point now
        // recorded as a locked reference in rules-dimensions.md.
        ...
        if (show_acrylic) {
            ...
            // (was door_t, same as metal). v48 TASK 3b: top boundary
            // originally at acrylic_top_limit_z (682.99mm world), clear of
            ...
            // directly into the frame's left/right verticals. v57 TASK 1:
            // top boundary pulled DOWN from acrylic_top_limit_z to the new
            // acrylic_top_z (680.98mm world, -2mm-e) -- see acrylic_top_z's
            // own derivation above for the full root-cause writeup (double
            // edge at the top, fixed with a genuine top border, piece (e)
            // below).
            color("#ADD8E6", 0.3)
            translate([acrylic_x0, front_y + door_t, split_z_local])
                cube([acrylic_x1 - acrylic_x0, door_acrylic_t, acrylic_top_z - split_z_local]);
            ...
            // (e) v57 TASK 1: NEW top mounting border -- fills the 2mm+e
            // band vacated by the acrylic's pulled-down top (acrylic_top_z
            // above), same door_t depth/plane and same nudge convention as
            // (b)/(c) (outer edge +e clear of the hole's own boundary,
            // inner edge -e overlapping the acrylic pane for a real shared
            // volume, Rule M-1). Spans the pane's own X width (acrylic_x0..
            // acrylic_x1) -- (b)/(c) already run the FULL split_z_local..
            // acrylic_top_limit_z height, so they already cover the two top
            // corners; this piece closes the remaining gap across the top.
            // Together this turns the old near-coincident double edge into
            // one continuous metal-border-then-acrylic transition, matching
            // the single-line look the left/right sides already have.
            color("#999999")
            translate([acrylic_x0, front_y + door_t, acrylic_top_z - e])
                cube([acrylic_x1 - acrylic_x0, door_t, (acrylic_top_limit_z - e) - (acrylic_top_z - e)]);

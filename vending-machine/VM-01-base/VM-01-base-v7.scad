// VM-01 Base Vending Machine
// Janis Product Design
// Version: v7
// Date: 2026-06-27
// Units: mm
// Purpose: Smart donation vending machine — buddha ornaments
// Payment: Online only, no cash/coin mechanism
// Changes from v6:
//   Fix 1: total_h = 700 (was 800) — full audit applied
//   Fix 2: zone stack confirmed — legs 0-50 | exit 50-300 |
//           tray0 300-421 | tray1 421-542 | upper display 542-700
//   Fix 3: exit door cutout Z 50-300, must not exceed 300
//   Fix 4: tray Z formula verified — leg_h + exit_door_h + (tray_num * tray_h)
//   Fix 5: screen center Z=280, recessed Y=skin_t+20, bracket r=8, angle -30
//   Fix 6: acrylic bot Z=542, top Z=698, height=156
//   Fix 7: upper front door cutout Z 542-700 added to shell
//   Fix 8: all dashboard elements verified within Z 50-542

$fn = 64;

// ─────────────────────────────
// PARAMETERS
// ─────────────────────────────

total_w = 620;
total_h = 700;          // FIX 1: was 800
total_d = 550;
corner_r = 20;
skin_t = 2;

// Legs
leg_h = 50;
leg_od = 25;
leg_inset = 40;

// Compartments
product_w = 416;
system_w = 185;
divider_t = 19;

// Spring / Tray
spring_od = 66;
spring_l = 390;
motor_d = 20;
spring_gap = 20;
spring_lanes = 5;
tray_count = 2;
tray_floor_t = 5;
tray_wall_t = 3;
spring_clearance = 50;
tray_h = 121;           // tray_floor_t(5) + spring_od(66) + spring_clearance(50)
partition_h = 40;

// Tray zone — FIX 2 zone stack
tray_zone_h = tray_h * tray_count;         // 242mm
tray_0_z = leg_h + exit_door_h;            // 300mm
tray_zone_top_z = tray_0_z + tray_zone_h;  // 542mm
upper_display_h = total_h - tray_zone_top_z; // 158mm

// Zones from inside floor
drop_zone_d = 138;
tray_start_d = drop_zone_d;

// Exit door — FIX 3
exit_door_h = 250;      // Z range: 50–300
exit_door_w = 400;

// Tray rack
rail_w = 10;
rail_h = 10;
latch_pin_d = 8;
latch_pin_l = 15;

// Front door
door_t = 3;
hinge_od = 12;
hinge_h = 20;

// Frame bars
frame_bar = 20;

// Dashboard — FIX 5 + 8
dash_x = product_w + divider_t + skin_t;
dash_w = system_w - divider_t - (skin_t * 2);
screen_w = 165;
screen_h = 100;
screen_center_z = 280;          // FIX 5: 40% of total_h 700
screen_recess_y = skin_t + 20;  // FIX 5: recessed 30% into panel
bracket_r = 8;                  // FIX 5: was 10
bezel_t = 3;
qr_w = 40;
qr_h = 30;
qr_d = 10;
card_w = 85;
card_h = 8;
card_d = 15;
speaker_w = 60;
speaker_h = 20;
speaker_slots = 5;

// Acrylic zone — FIX 6
acrylic_bot_z = tray_zone_top_z;               // 542mm
acrylic_top_z = total_h - skin_t;              // 698mm
acrylic_zone_h = acrylic_top_z - acrylic_bot_z; // 156mm
acrylic_r = 8;
acrylic_t = skin_t;

// IR sensor
sensor_d = 15;
sensor_h = 8;
sensor_gap = 10;

// ─────────────────────────────
// MODULES
// ─────────────────────────────

module rounded_box(w, h, d, r) {
    hull() {
        translate([r, r, 0]) cylinder(r=r, h=h);
        translate([w-r, r, 0]) cylinder(r=r, h=h);
        translate([r, d-r, 0]) cylinder(r=r, h=h);
        translate([w-r, d-r, 0]) cylinder(r=r, h=h);
    }
}

module leg() {
    color("#AAAAAA")
    cylinder(h=leg_h, d=leg_od);
}

module legs() {
    positions = [
        [leg_inset, leg_inset],
        [total_w-leg_inset, leg_inset],
        [leg_inset, total_d-leg_inset],
        [total_w-leg_inset, total_d-leg_inset]
    ];
    for (p = positions)
        translate([p[0], p[1], 0]) leg();
}

module spring_coil() {
    color("#888888")
    difference() {
        cylinder(h=spring_l, d=spring_od);
        cylinder(h=spring_l+1, d=spring_od-10);
    }
}

// Independent removable tray. Front=OPEN, Top=OPEN, sides=framed window.
// FIX 4: z = leg_h + exit_door_h + (tray_num * tray_h)
module spring_tray(tray_num) {
    tray_w = product_w - 20;
    tray_d = spring_l + motor_d;
    z = leg_h + exit_door_h + (tray_num * tray_h);

    translate([10, tray_start_d, z]) {
        color("#CCCCCC")
        difference() {
            union() {
                cube([tray_w, tray_d, tray_floor_t]);
                translate([0, tray_d - tray_wall_t, 0])
                    cube([tray_w, tray_wall_t, tray_h]);
                cube([tray_wall_t, tray_d, tray_h]);
                translate([tray_w - tray_wall_t, 0, 0])
                    cube([tray_wall_t, tray_d, tray_h]);
            }
            // Left side window (3mm frame, open centre)
            translate([-1, tray_wall_t, tray_floor_t + tray_wall_t])
                cube([tray_wall_t + 2,
                      tray_d - (tray_wall_t * 2),
                      tray_h - tray_floor_t - (tray_wall_t * 2)]);
            // Right side window (3mm frame, open centre)
            translate([tray_w - tray_wall_t - 1, tray_wall_t, tray_floor_t + tray_wall_t])
                cube([tray_wall_t + 2,
                      tray_d - (tray_wall_t * 2),
                      tray_h - tray_floor_t - (tray_wall_t * 2)]);
            // Rear motor cutouts
            for (i = [0:spring_lanes-1]) {
                x = tray_wall_t + spring_od/2 + i*(spring_od + spring_gap);
                translate([x - 11, tray_d - tray_wall_t - 1, tray_floor_t])
                    cube([22, tray_wall_t + 2, 32]);
            }
            // Rear latch hole (d=8mm, centered)
            translate([tray_w/2, tray_d - tray_wall_t - 1, tray_h/2])
                rotate([-90, 0, 0])
                    cylinder(d=latch_pin_d, h=tray_wall_t + 2);
        }

        // Springs + motors + lane partitions (unchanged)
        for (i = [0:spring_lanes-1]) {
            x = tray_wall_t + spring_od/2 + i*(spring_od + spring_gap);
            color("#555555")
            translate([x-10, 0, tray_floor_t])
                cube([20, motor_d, 30]);
            color("#999999")
            translate([x, motor_d + spring_l, tray_floor_t + spring_od/2])
                rotate([90, 0, 0])
                    spring_coil();
            if (i < spring_lanes-1) {
                color("#BBBBBB")
                translate([x + spring_od/2 + spring_gap/2, tray_wall_t, tray_floor_t])
                    cube([2, tray_d-(tray_wall_t*2), partition_h]);
            }
        }
    }
}

// Fixed rack — rails + rear latch pins.
module tray_rack() {
    tray_w = product_w - 20;
    tray_d = spring_l + motor_d;
    color("#AAAAAA")
    for (t = [0:tray_count-1]) {
        z = leg_h + exit_door_h + (t * tray_h);
        translate([10, tray_start_d, z - rail_h])
            cube([rail_w, tray_d, rail_h]);
        translate([10 + tray_w - rail_w, tray_start_d, z - rail_h])
            cube([rail_w, tray_d, rail_h]);
        translate([10 + tray_w/2, tray_start_d + tray_d - tray_wall_t, z + tray_h/2])
            rotate([-90, 0, 0])
                cylinder(d=latch_pin_d, h=latch_pin_l);
    }
}

module outer_shell() {
    color("#C8C8C8", 0.75)
    difference() {
        // FIX 1: total_h = 700
        rounded_box(total_w, total_h, total_d, corner_r);

        // Hollow interior
        translate([skin_t, skin_t, skin_t])
            rounded_box(total_w-(skin_t*2), total_h, total_d-(skin_t*2), corner_r-1);

        // Left product zone front opening — exit door + tray zone (50–542)
        translate([skin_t, -1, leg_h])
            cube([product_w-(skin_t*2), skin_t+2, tray_zone_top_z - leg_h]);

        // FIX 3: Exit door cutout — Z 50–300, width 400mm, centered
        translate([(product_w - exit_door_w)/2, -1, leg_h])
            cube([exit_door_w, skin_t+2, exit_door_h]);

        // FIX 7: Upper front door cutout — Z 542–700 (158mm)
        translate([skin_t, -1, tray_zone_top_z])
            cube([product_w-(skin_t*2), skin_t+2, upper_display_h]);
    }
}

module compartment_divider() {
    color("#AAAAAA", 0.9)
    // FIX 1: total_h - leg_h = 650
    translate([product_w, skin_t, leg_h])
        cube([divider_t, total_d-(skin_t*2), total_h - leg_h]);
}

// Single-panel front door — left hinge, shown closed. Z 50–542.
module front_door() {
    door_h = tray_zone_top_z - leg_h;   // 492mm
    color("#C0C0C0", 0.6)
    translate([skin_t, 0, leg_h])
        cube([product_w-(skin_t*2), door_t, door_h]);
    hinge_positions = [
        leg_h + door_h * 0.15,
        leg_h + door_h * 0.5,
        leg_h + door_h * 0.85
    ];
    for (hz = hinge_positions) {
        color("#888888")
        translate([skin_t, 0, hz])
            cylinder(d=hinge_od, h=hinge_h);
    }
}

// Structural frame around tray zone opening (visible when door open).
module tray_zone_frame() {
    color("#AAAAAA")
    union() {
        translate([0, 0, tray_zone_top_z - frame_bar])
            cube([product_w, skin_t, frame_bar]);
        translate([0, 0, tray_0_z])
            cube([product_w, skin_t, frame_bar]);
        translate([0, 0, tray_0_z])
            cube([frame_bar, skin_t, tray_zone_h]);
        translate([product_w - frame_bar, 0, tray_0_z])
            cube([frame_bar, skin_t, tray_zone_h]);
    }
}

// ATM dashboard — right compartment front face.
// FIX 5: screen_center_z=280, recessed Y=skin_t+20, bracket r=8, angle -30
// FIX 8: all elements verified within Z 50–542
module dashboard() {
    screen_z = screen_center_z - screen_h/2;  // 230mm
    qr_z    = screen_z - 50 - qr_h;           // 150mm
    card_z  = qr_z - 30 - card_h;             // 112mm
    speaker_z = card_z - 20 - speaker_h;       // 72mm
    // All verified: screen 230-330, qr 150-180, card 112-120, speaker 72-92 — all within 50-542

    // Screen — recessed Y = skin_t+20, tilted -30 deg
    color("#1A1A1A")
    translate([product_w + divider_t + 10, screen_recess_y, screen_z])
        rotate([-30, 0, 0])
            cube([screen_w, 5, screen_h]);

    // Screen bezel
    color("#333333", 0.8)
    translate([product_w + divider_t + 10 - bezel_t,
               screen_recess_y - 2, screen_z - bezel_t])
        rotate([-30, 0, 0])
            cube([screen_w + (bezel_t*2), 3, screen_h + (bezel_t*2)]);

    // Curved support bracket under screen — hull r=8mm FIX 5
    color("#444444")
    translate([dash_x + dash_w/2, screen_recess_y, screen_z])
        hull() {
            translate([-screen_w/2, 0, 0])
                rotate([-90, 0, 0]) cylinder(r=bracket_r, h=30);
            translate([screen_w/2, 0, 0])
                rotate([-90, 0, 0]) cylinder(r=bracket_r, h=30);
        }

    // QR scanner (Z=150mm)
    color("#222222")
    translate([dash_x + (dash_w - qr_w)/2, 0, qr_z])
        cube([qr_w, qr_d, qr_h]);

    // ID card reader (Z=112mm)
    color("#333333")
    translate([dash_x + (dash_w - card_w)/2, 0, card_z])
        cube([card_w, card_d, card_h]);

    // Speaker grille — 5 slots (Z=72mm)
    slot_h = 2;
    slot_gap = 2;
    color("#444444")
    for (s = [0:speaker_slots-1]) {
        translate([dash_x + (dash_w - speaker_w)/2,
                   0,
                   speaker_z + s*(slot_h + slot_gap)])
            cube([speaker_w, 3, slot_h]);
    }
}

// Acrylic display — 3 faces, right compartment.
// FIX 6: Z 542–698, height 156mm
module acrylic_display() {
    acrylic_front_w = system_w - divider_t - skin_t;
    acrylic_front_x = product_w + divider_t;

    // Front panel — rounded corners r=8
    color("#ADD8E6", 0.3)
    translate([acrylic_front_x, skin_t, acrylic_bot_z])
        hull() {
            translate([acrylic_r, 0, acrylic_r])
                rotate([-90,0,0]) cylinder(r=acrylic_r, h=acrylic_t);
            translate([acrylic_front_w-acrylic_r, 0, acrylic_r])
                rotate([-90,0,0]) cylinder(r=acrylic_r, h=acrylic_t);
            translate([acrylic_r, 0, acrylic_zone_h-acrylic_r])
                rotate([-90,0,0]) cylinder(r=acrylic_r, h=acrylic_t);
            translate([acrylic_front_w-acrylic_r, 0, acrylic_zone_h-acrylic_r])
                rotate([-90,0,0]) cylinder(r=acrylic_r, h=acrylic_t);
        }

    // Right side panel
    color("#ADD8E6", 0.3)
    translate([total_w - skin_t, skin_t, acrylic_bot_z])
        cube([skin_t, total_d-(skin_t*2), acrylic_zone_h]);

    // Top panel — FIX 1: total_h = 700
    color("#ADD8E6", 0.3)
    translate([acrylic_front_x, skin_t, total_h - skin_t])
        cube([acrylic_front_w, total_d-(skin_t*2), skin_t]);
}

module ir_sensors() {
    sensor_z = leg_h + tray_h - sensor_gap - sensor_h;
    color("#222222")
    translate([skin_t + 5, skin_t + drop_zone_d/2, sensor_z])
        cube([sensor_d, sensor_h, sensor_h]);
    color("#222222")
    translate([product_w - skin_t - 5 - sensor_d,
               skin_t + drop_zone_d/2, sensor_z])
        cube([sensor_d, sensor_h, sensor_h]);
    color("#FF0000", 0.3)
    translate([skin_t + 5 + sensor_d,
               skin_t + drop_zone_d/2 + sensor_h/2,
               sensor_z + sensor_h/2])
        cube([product_w - (skin_t*2) - 10 - (sensor_d*2), 1, 1]);
}

module drop_zone_visual() {
    color("#F0F0F0", 0.15)
    translate([skin_t, skin_t, leg_h])
        cube([product_w-(skin_t*2), drop_zone_d, exit_door_h]);
}

// FIX 1: rear_service_door uses total_h = 700
module rear_service_door() {
    door_w = system_w - divider_t;
    door_h = total_h * 0.5;    // 350mm
    color("#B8B8B8", 0.6)
    translate([product_w + divider_t, total_d - skin_t, leg_h + (total_h*0.25)])
        cube([door_w, skin_t*2, door_h]);
}

// ─────────────────────────────
// ASSEMBLY
// ─────────────────────────────

legs();
translate([0, 0, leg_h]) outer_shell();
compartment_divider();

tray_rack();
for (t = [0:tray_count-1]) spring_tray(t);

front_door();
tray_zone_frame();
drop_zone_visual();

ir_sensors();

dashboard();        // FIX 5 + 8: screen visible, all elements within 50-542
acrylic_display();  // FIX 6: Z 542-698
rear_service_door();

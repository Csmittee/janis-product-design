...
// Changes from v7:
//   Fix 1: spring_tray side walls — restored v6 framed window cutouts
//           (3mm border, centre of each side wall open)
//   Fix 2: screen Y position = skin_t only (was skin_t+20)
//           screen now on front face of right compartment, not inside body
...
screen_h = 100;
screen_center_z = 280;
screen_recess_y = skin_t;       // FIX 2: was skin_t+20 — screen on front face, not inside body
bracket_r = 8;
bezel_t = 3;
qr_w = 40;
qr_h = 30;
...
// FIX 1: side wall window cutouts restored from v6.
// Front=OPEN, Top=OPEN, left/right sides=3mm frame border, centre open.
    ...
    translate([10, tray_start_d, z]) {
        ...
        difference() {
            union() {
                ...
                // Left side wall — full slab (window cut below)
                cube([tray_wall_t, tray_d, tray_h]);
                // Right side wall — full slab (window cut below)
                translate([tray_w - tray_wall_t, 0, 0])
                    cube([tray_wall_t, tray_d, tray_h]);
            }

            // FIX 1: Left side wall window — 3mm border frame, centre open
            translate([-1, tray_wall_t, tray_floor_t + tray_wall_t])
                cube([tray_wall_t + 2,
                      tray_d - (tray_wall_t * 2),
                      tray_h - tray_floor_t - (tray_wall_t * 2)]);

            // FIX 1: Right side wall window — 3mm border frame, centre open
        ...
        // Springs + motors + lane partitions
        for (i = [0:spring_lanes-1]) {
            x = tray_wall_t + spring_od/2 + i*(spring_od + spring_gap);
            color("#555555")
...
// FIX 2: screen_recess_y = skin_t — screen sits on front face, not recessed into body.
// Screen tilted 30deg, visible from front of right compartment.
module dashboard() {
    screen_z = screen_center_z - screen_h/2;  // 230mm bottom of screen
    ...
    // Curved support bracket under screen — hull r=8mm
    color("#444444")
...
// Acrylic display — 3 faces, right compartment, Z 542–698.

...
// Derived from: vending-machine/VM-01-base/VM-01-base-v58.scad — seeded
// as a full copy (same convention as VM-01's own per-version files, a
// complete standalone file, not a diff/include).
//
// GOVERNANCE FLAG (not blocking, see cc_chat_log.md for full note): VM-02
...
module outer_shell() {
    ...
    difference() {
        ...
        // Right compartment front face opening -- VM-02: now full
        // floor-to-ceiling (was capped at tray_zone_top_z in v58, leaving
        // solid shell above it) -- matches the new rear service door's
        // own full-height philosophy (Task B.6) and the removal of
        // acrylic_display() (no separate "display zone" above this cutout
        // anymore).
        translate([product_w + divider_t, -1, 0])
            cube([system_w - divider_t - skin_t, skin_t+2, shell_h]);
